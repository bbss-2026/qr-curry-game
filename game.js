...
// （パスワード未設定の場合はこれまで通り運営への連絡が必要）。1セッションにつき1回だけ表示する。
let _uidReclaimPromptShownThisSession = false;
function maybePromptUidReclaim(err) {
    if (_uidReclaimPromptShownThisSession) return;
    ...
    _uidReclaimPromptShownThisSession = true;
...
function attemptSilentUidReclaimBySecretKey(targetId) {
    ...
    return ensureFirebaseAuth().then(function(){
        ...
        const updates = {};
        updates['players/' + targetId + '/uid'] = currentUid;
...
function attemptUidReclaimByPassword(targetId, guessedPw) {
    ...
    ensureFirebaseAuth().then(function(){
        if (!currentUid) { showCustomAlert('❌ エラー', '認証に失敗しました。時間をおいて再度お試しください。'); return; }
        const updates = {};
        updates['players/' + targetId + '/uid'] = currentUid;
    ...
    showCustomConfirm('🔑 IDとパスワードで復元', msg, function() {
        ...
        ensureFirebaseAuth().then(function(){
            ...
            database.ref('players/' + inputId).once('value').then(function(snap){
                ...
                const updates = {};
                updates['players/' + inputId + '/uid'] = currentUid;
    ...
    if(decoded.playerId && decoded.secretKey) {
        ...
        if(database) {
            ensureFirebaseAuth().then(function(){
                ...
                const updates = {};
                updates['players/' + playerId + '/uid'] = currentUid;
