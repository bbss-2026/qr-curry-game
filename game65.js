...
// ============================================================
(function() {
    function isDebugModeNow() { try { return localStorage.getItem('qr_debug_mode') === '1'; } catch(e) { return false; } }
    // 管理ツール（admin.html）の「デバックログ ON/OFF」でこの同期ログ（画面下部のdebugErrorPanel）
    // だけを非表示にできるようにする（デバッグモード自体は維持したまま、ログ表示のみ消せる）。
    function isDebugLogHidden() { try { return localStorage.getItem('qr_debug_log_hidden') === '1'; } catch(e) { return false; } }
    function showDebugErrorPanel(text) {
        if (!isDebugModeNow()) return;
        if (isDebugLogHidden()) return;
        try {
...
// ============================================================
// localStorageの安全なラッパー（ポータルサイトへのiframe埋め込み対策）
// sandbox属性にallow-same-originが無い埋め込みや、SafariのITP/ストレージ分離環境では
// localStorageへのアクセス自体がSecurityErrorを投げることがある。
// 本来この例外は起動直後のトップレベルコード（window.onload登録より前）で発生しうるため、
// try/catchで囲まないとスクリプト全体の実行がそこで止まり、画面が完全に固まる
// （IDが発行されない・Firebaseにも何も保存されない、という報告の主因と推定される）。
// 失敗時はメモリ上のフォールバックストアで動作を継続する（リロードで消えるがそのプレイ中は使える）。
// ============================================================
const _memoryStorageFallback = {};
function safeLSGet(key, fallback) {
    try { const v = localStorage.getItem(key); return v === null ? (fallback !== undefined ? fallback : null) : v; }
    catch(e) { return (key in _memoryStorageFallback) ? _memoryStorageFallback[key] : (fallback !== undefined ? fallback : null); }
}
function safeLSSet(key, value) {
    try { localStorage.setItem(key, value); }
    catch(e) { _memoryStorageFallback[key] = value; }
}
...
let currentIconFile = migrateIconPath(safeLSGet("selectedPlayerIcon")) || "myimageicon/mayimage01.png";
...
// ===== フェス：仲間bot専用のベース・食器選択（所有物はストックカレー側と共通、選択状態だけ独立） =====
function updateFestAllyTablewareBaseUI() {
    const bEl = document.getElementById('festAllyBaseSelectLabel');
    const tEl = document.getElementById('festAllyTablewareSelectLabel');
    if (bEl) bEl.innerText = festAllySelectedBase;
    if (tEl) tEl.innerText = festAllySelectedTableware;
}
function showFestAllyBaseSelectModal() {
    const owned = getUnlockedBase();
    if (owned.length <= 1) {
        showCustomAlert('ベース', '今は「白米」しか持っていません。');
        return;
    ...
    let html = '<div style="display:flex;flex-direction:column;gap:8px;">';
    owned.forEach(name => {
        const isSel = name === festAllySelectedBase;
        const info = BASE_LIST[name] || { desc: '' };
        html += `<div onclick="selectFestAllyBase('${name}')" style="cursor:pointer;padding:10px;border-radius:6px;border:2px solid ${isSel?'#b88742':'#e0d0b0'};background:${isSel?'#fff8ee':'transparent'};text-align:left;">
            <div style="font-weight:bold;font-size:13px;color:#420000;">${name}${isSel?'（選択中）':''}</div>
            <div style="font-size:11px;color:#888;margin-top:2px;">${info.desc}</div>
        </div>`;
    });
    html += '</div>';
    showCustomAlert('仲間のベースを選ぶ', html);
}
function selectFestAllyBase(name) {
    if (!getUnlockedBase().includes(name)) return;
    festAllySelectedBase = name;
    document.getElementById('customModal').style.display = 'none';
    updateFestAllyTablewareBaseUI();
    festClampAllyHpToMax();
    saveGame();
    if (typeof renderFestAllyUI === 'function') renderFestAllyUI();
}
function showFestAllyTablewareSelectModal() {
    const owned = getUnlockedTableware();
    if (owned.length <= 1) {
        showCustomAlert('食器', '今は「白い皿」しか持っていません。\nショップで他の食器を購入すると選べるようになります。');
        return;
    ...
    let html = '<div style="display:flex;flex-direction:column;gap:8px;">';
    owned.forEach(name => {
        const isSel = name === festAllySelectedTableware;
        const info = TABLEWARE_LIST[name] || { desc: '' };
        html += `<div onclick="selectFestAllyTableware('${name}')" style="cursor:pointer;padding:10px;border-radius:6px;border:2px solid ${isSel?'#b88742':'#e0d0b0'};background:${isSel?'#fff8ee':'transparent'};text-align:left;">
            <div style="font-weight:bold;font-size:13px;color:#420000;">${name}${isSel?'（選択中）':''}</div>
            <div style="font-size:11px;color:#888;margin-top:2px;">${info.desc}</div>
        </div>`;
    });
    html += '</div>';
    showCustomAlert('仲間の食器を選ぶ', html);
}
function selectFestAllyTableware(name) {
    if (!getUnlockedTableware().includes(name)) return;
    festAllySelectedTableware = name;
    document.getElementById('customModal').style.display = 'none';
    updateFestAllyTablewareBaseUI();
    festClampAllyHpToMax();
    saveGame();
...
// 食器・ベースの切替で最大HPが下がった場合、現在HP（festAllyHp）がそれを超えないようクランプする
function festClampAllyHpToMax() {
    if (typeof festHiredAllyName === 'undefined' || !festHiredAllyName) return;
    if (typeof festAllyHp !== 'number') return;
    const stats = getFestAllyCurrentStats(festHiredAllyName);
    if (stats && festAllyHp > stats.hp) festAllyHp = stats.hp;
}
...
// ===== バトル画面：名前横に表示する状態アイコン（全対戦共通） =====
// 以前は毒・幻惑にかかった際に名前の文字色を変更していたが、それを廃止しアイコン表示に統一する。
// 複数の状態が同時に有効な場合は、必ずこの並び順で表示する。
const BATTLE_STATUS_ICON_DEFS = [
    { key: 'illusion',      src: 'battle/bt-icon-01.svg', alt: '幻惑' },
    { key: 'poison',        src: 'battle/bt-icon-02.svg', alt: '毒' },
    { key: 'atkDown',       src: 'battle/bt-icon-03.svg', alt: 'ATKダウン' },
    { key: 'skipTurn',      src: 'battle/bt-icon-04.svg', alt: '次のターン休み' },
    { key: 'defUp',         src: 'battle/bt-icon-05.svg', alt: 'DEFアップ' },
    { key: 'atkUp',         src: 'battle/bt-icon-06.svg', alt: 'ATKアップ' },
    { key: 'hpRegen',       src: 'battle/bt-icon-07.svg', alt: '毎ターンHP回復' },
    { key: 'meatReduce',    src: 'battle/bt-icon-08.svg', alt: '肉系ダメージ軽減' },
    { key: 'vegReduce',     src: 'battle/bt-icon-09.svg', alt: '野菜系ダメージ軽減' },
    { key: 'seafoodReduce', src: 'battle/bt-icon-10.svg', alt: '魚介系ダメージ軽減' },
    { key: 'fruitReduce',   src: 'battle/bt-icon-11.svg', alt: '果物系ダメージ軽減' }
];
// activeKeys: 現在有効な状態キーの配列（順不同で渡してよい。表示順は本関数が上記の固定順で並べ替える）
function renderBattleStatusIcons(activeKeys) {
    if(!activeKeys || !activeKeys.length) return '';
    const activeSet = {};
    activeKeys.forEach(function(k){ if(k) activeSet[k] = true; });
    const html = BATTLE_STATUS_ICON_DEFS.filter(function(d){ return activeSet[d.key]; }).map(function(d){
        return '<img src="' + d.src + '" alt="' + d.alt + '" title="' + d.alt + '" class="battle-status-icon">';
    }).join('');
    if(!html) return '';
    return '<span class="battle-status-icons">' + html + '</span>';
}
// 指定した名前要素の「末尾の子スパン」(id: <baseId>StatusIcons）を作成/取得し、中身を差し替える。
// 名前要素自体は`.innerText = 名前`のような形でバトル開始時に1回だけ設定される作りになっているため、
// その直後に呼べば以降ずっと（次の対戦が始まって名前が再設定されるまで）子スパンとして残り続ける。
// 兄弟要素として追加すると、名前divがブロック要素の場合に改行されて「名前の下」に表示されてしまうため、
// 必ず名前要素の子として末尾に追加し、名前と同じ行に並んで見えるようにする。
function updateBattleStatusIconsFor(nameElId, activeKeys) {
    const nameEl = document.getElementById(nameElId);
    if(!nameEl) return;
    const iconSpanId = nameElId + 'StatusIcons';
    let iconSpan = document.getElementById(iconSpanId);
    if(!iconSpan) {
        iconSpan = document.createElement('span');
        iconSpan.id = iconSpanId;
        nameEl.appendChild(iconSpan);
    }
    iconSpan.innerHTML = renderBattleStatusIcons(activeKeys);
}
// ふわとろオム／世界三大珍味バリアの軽減対象カテゴリ（meat/seafood/vegetable/fruit）を、対応する状態アイコンのキーに変換する
function fluffyCategoryToIconKey(cat) {
    if(cat === 'meat') return 'meatReduce';
    if(cat === 'vegetable') return 'vegReduce';
    if(cat === 'seafood') return 'seafoodReduce';
    if(cat === 'fruit') return 'fruitReduce';
    return null;
}
...
if(firebaseConfig.apiKey !== "YOUR_API_KEY") {
    ...
    // Realtime Database自体への接続確立状況を監視する（.info/connectedはセキュリティルール・
    // 認証状態に一切左右されない、SDKがローカルに把握している「今この端末は接続できているか」の値）。
    // 書き込みの成否に頼らず接続の有無そのものを判定できるため、CSP等でFirebaseへの通信自体が
    // ブロックされているケース（認証やsyncErrorsへの書き込みすら届かない）の切り分けに使う。
    let rtdbConnected = null;
    try {
        database.ref('.info/connected').on('value', function(snap) {
            rtdbConnected = !!(snap && snap.val());
            window.__rtdbConnected = rtdbConnected;
            if (typeof window.__showDebugErrorPanel === 'function') {
                window.__showDebugErrorPanel('RTDB接続状態: ' + (rtdbConnected ? '✅ 接続済み' : '❌ 未接続'));
            }
            if (rtdbConnected) {
                // 接続が確認できたら、以前出していた「未接続」バナーは役目を終えたので消す
                const el = document.getElementById('rtdbDisconnectedBanner');
                if (el) { el.style.display = 'none'; restackTopBanners(); }
            }
        });
        // .info/connectedは「未接続」が一瞬だけ出る立ち上がりの一瞬も含むため、
        // 認証の成否とは無関係に、一定時間（6秒）経っても接続できていなければ
        // 明示的にバナーを出す。認証だけ通ってDB接続自体は生きていない、というケースを
        // showCloudSyncWarningBanner（認証失敗時にしか出ない）とは別に検知するため。
        setTimeout(function() {
            if (rtdbConnected === false) showRtdbDisconnectedBanner();
        }, 6000);
    } catch(e) { /* noop */ }
}

// 認証の成否とは無関係に、Realtime Database自体への接続が確立できていない場合に出す通知。
function showRtdbDisconnectedBanner() {
    if (document.getElementById('rtdbDisconnectedBanner')) return;
    try {
        const el = document.createElement('div');
        el.id = 'rtdbDisconnectedBanner';
        el.className = 'fixedTopBanner';
        el.style.cssText = 'position:fixed;left:0;right:0;z-index:100001;background:#8e44ad;color:#fff;font-size:12px;padding:8px 12px;text-align:center;line-height:1.6;box-shadow:0 2px 6px rgba(0,0,0,0.3);';
        el.innerHTML = '⚠️ サーバー（Realtime Database）への接続自体ができていません。認証状況に関わらず、進行状況はクラウドに保存されません。<br>埋め込み表示の場合は、通常のブラウザで公式ページを開き直してください。'
            + ' <span style="text-decoration:underline;cursor:pointer;font-weight:bold;" onclick="document.getElementById(\'rtdbDisconnectedBanner\').style.display=\'none\';restackTopBanners();">✕閉じる</span>';
...
function showCloudSyncWarningBanner() {
    ...
    try {
        ...
        // .info/connectedの状態が取れていれば、原因が「通信自体がブロックされている」のか
        // 「通信はできるが認証だけ失敗している」のかの切り分けに使えるよう文言に含める。
        const rtdbStatus = window.__rtdbConnected === true ? '（サーバーへの接続自体は成功しています）'
            : window.__rtdbConnected === false ? '（サーバーへの接続自体ができていません）'
            : '';
        el.innerHTML = '⚠️ クラウド同期に接続できていません。進行状況が保存されない可能性があります。' + rtdbStatus + '<br>アプリ内ブラウザ（LINE・Instagram等）やプライベートブラウズを使っている場合は、通常のブラウザで開き直してください。'
...
function showIframeEmbedNotice() {
    ...
    try {
        ...
        // location.hrefは埋め込み方式によっては埋め込み元（プリシー等）のURLを指してしまい、
        // コピーしても同じページに戻るだけになることがあるため、公式URLを固定値で使う。
        const officialUrl = 'https://bbss-2026.github.io/qr-curry-game/game.html';
...
function ensureFirebaseAuth() {
    ...
    _authReadyPromise = new Promise(function(resolve) {
        ...
        let resolved = false;
        // 初回のresolve用ガードとは別に、以降ずっとリスナーを張りっぱなしにしてcurrentUidを常に最新へ追従させる。
        // （以前は初回のuserイベントでcurrentUidを固定していたため、ページ滞在中にFirebase側で
        //   セッションが失われて裏で自動的に再匿名認証された場合、currentUidが古いuidのまま残ってしまい、
        //   UID再紐付け（自己修復）が「新しいuidではなく古いuidを書き込もうとして弾かれる」→ 何度やっても直らない、
        //   という現象の原因になり得ていた。）
...
function grantLvUpReward(lv) {
    ...
    if (reward && reward.items) {
        // レア食材（shop===-3、大根→聖護院大根のような上位互換食材）はQRスキャン以外では
        // 絶対に入手できない仕様のため、このレベルアップ報酬の抽選プールから明示的に除外する。
        // isIngredientAvailable()はレア食材を常にtrueで返す仕様（「レア食材は常に表示」）なので、
        // shopでの除外を入れないとここに混ざってしまう（実際に混ざっていたバグの修正）。
        const ingPool = Object.keys(masterIngredients).filter(k => !["金箔","赤パプリカ","黄パプリカ"].includes(k) && masterIngredients[k].shop !== -3 && isIngredientAvailable(k));
...
const masterIngredients = {
    ...
    "金時にんじん":       { hp: 75, atk: 10, def: 25, spd: 40, emoji: "🥕", icon: "foods_icon/item_56.svg", shop: -3, desc: "京野菜の一つ、金時にんじん（京人参）。鮮やかな赤みと甘みが特徴の希少品種。素早さを大幅に高める。" },
    ...
    "賀茂なす":           { hp: 80, atk: 20, def: 50, spd: 10, emoji: "🍆", icon: "foods_icon/item_62.svg", shop: -3, desc: "京野菜の代表格、丸々とした賀茂なす。とろける食感と上品な甘みでHPと防御力を大幅に高める。" },
    "烏骨鶏の卵":         { hp: 100, atk: 25, def: 25, spd: 20, emoji: "🥚", icon: "foods_icon/item_63.svg", shop: -3, desc: "烏骨鶏が産む幻の卵。濃厚なコクと栄養価でHPと攻撃力を大きく高める。" },
    "ポルチーニ":         { hp: 65, atk: 55, def: 20, spd: 35, emoji: "🍄", icon: "foods_icon/item_64.svg", shop: -3, desc: "森の宝石と称される香り高いキノコの王様。攻撃力と素早さを爆発的に高める。" },
    "島バナナ":           { hp: 60, atk: 20, def: 35, spd: 55, emoji: "🍌", icon: "foods_icon/item_65.svg", shop: -3, desc: "南国育ちの濃厚な甘さを持つ小ぶりなバナナ。俊敏な身のこなしと粘り強い守りを与える。" },
    "白花豆":             { hp: 105, atk: 20, def: 45, spd: 10, emoji: "🫘", icon: "foods_icon/item_66.svg", shop: -3, desc: "大粒でホクホクとした極上の高級豆。圧倒的なHPと防御力を誇る。" },
...
// ============================================================
// 日替わりカレー（旧・PCと対戦（中級）ボタンの場所に統合）
// 曜日ごとに1体、甘口/中辛/辛口の3段階。1日1回勝てば翌日まで再挑戦不可、負けたら即座に何度でも再戦可（難易度も選び直し可）。
// 甘口はガンコ親父〜宮廷カレー長の中間、中辛はドラゴン料理長相当、辛口はそれ以上（対策が必要な強さ）を目安に調整。
// ============================================================
...
const DAILY_CURRY_DEFS = [
    { // 0:日曜日
        ...
        tiers: [
            { hp:300, atk:70, def:45, spd:55, evasionRate:0.25 },
            { hp:370, atk:85, def:55, spd:65, evasionRate:0.35 },
            { hp:450, atk:100, def:65, spd:75, evasionRate:0.45 }
        ],
    ...
    { // 1:月曜日
        ...
        tiers: [
            { hp:320, atk:75, def:50, spd:30, isIllusion:true },
            { hp:390, atk:92, def:60, spd:38, isIllusion:true },
            { hp:470, atk:110, def:70, spd:46, isIllusion:true }
        ],
    ...
    { // 2:火曜日
        ...
        tiers: [
            { hp:310, atk:85, def:40, spd:25, isWanpaku:true, wanpakuMissRate:0.35 },
            { hp:380, atk:104, def:50, spd:32, isWanpaku:true, wanpakuMissRate:0.25 },
            { hp:460, atk:125, def:60, spd:40, isWanpaku:true, wanpakuMissRate:0.15 }
        ],
    ...
    { // 3:水曜日
        ...
        tiers: [
            { hp:300, atk:70, def:55, spd:20, isSeafood:true },
            { hp:370, atk:86, def:68, spd:26, isSeafood:true },
            { hp:450, atk:104, def:82, spd:33, isSeafood:true }
        ],
    ...
    { // 4:木曜日
        ...
        tiers: [
            { hp:320, atk:68, def:48, spd:25, isPoison:true },
            { hp:390, atk:83, def:59, spd:32, isPoison:true },
            { hp:470, atk:100, def:71, spd:40, isPoison:true, isPoisonApple:true }
        ],
    ...
    { // 5:金曜日
        ...
        desc: 'DEFが非常に高い。',
        tiers: [
            { hp:300, atk:60, def:90, spd:15 },
            { hp:370, atk:73, def:110, spd:20 },
            { hp:450, atk:88, def:135, spd:26 }
        ],
    ...
    { // 6:土曜日
        ...
        tiers: [
            { hp:310, atk:65, def:60, spd:20, isSeed:true },
            { hp:380, atk:79, def:74, spd:26, isSeed:true },
            { hp:460, atk:95, def:89, spd:33, isSeed:true }
        ],
        reward: { type:'items', category:'vegetable', normal:[6,9,13], mid:[0,1,0], bonusMidOrHigh:[0,0,1] }
    }
];
function getDailyCurryWeekdayIndex(baseDate) {
    // JST基準の曜日（0=日〜6=土）。既存のgetJSTDateStringと同じJSTオフセット計算を使う。
    const now = baseDate || getGameNow();
    const jstMs = now.getTime() + 9 * 60 * 60000;
    return new Date(jstMs).getUTCDay();
}
function getDailyCurryDef(weekdayIdx) {
    return DAILY_CURRY_DEFS[weekdayIdx] || DAILY_CURRY_DEFS[0];
}
let inventory = {}; let scanHistory = {};
let playerG = 0; let playerEXP = 0; let packTicket = 0; let spicyCoin = 0;
let foodSampleCount = 0; // 食品サンプル所持数（バトル敗北時のストック消滅を身代わりで防ぐアイテム）
// カレーフェス連勝報酬「食品サンプル」の一度きり付与フラグ（20/30/40/50連勝）
let festSampleClaimed20 = false; let festSampleClaimed30 = false; let festSampleClaimed40 = false; let festSampleClaimed50 = false;
...
let festPaused = false;          // 「一時中断」で対戦タブに戻っている間true。festActiveはtrueのまま維持し、再開時にfalseへ戻す
// 一時中断からの再開先。'hub'＝準備画面（ショップ/調理/仲間タブ）に戻ってよいタイミング。
// 'match'＝対戦相手が確定済みで出撃カレー選択～戦闘中（本来は準備画面に戻れない区間）、再開時はマッチング画面を開き直す。
// 'pendingContinue'＝勝利報酬は反映済みだが「次の対戦へ進む／準備画面へ進む」を押す前（これも準備画面には戻れない区間）、
// 再開時はfestBattleContinue()相当の処理までそのまま進める。
let festResumeMode = 'hub';
...
let toppingSlotSpinAudio = null;    // スロット回転中のループ音声（sound/slot_kaiten.mp3）
...
let festPlayerSkipTurn = false;      // 「ぷにぷにプレゼント」でカレー側が次の行動を1回休みか（戦闘またぎでは持続しない）
let festAllySkipTurn = false;        // 同上、仲間側

// トッピングスロットで抽選される効果一覧（絵文字は使わない）。
// type: 'stat'（プレイヤー自身のカレーにのみ常時加算）／'crit'（プレイヤー自身の会心率にのみ常時加算）／
//       'guard'（プレイヤー自身のカレーへの状態異常を100%無効化）／'reduction'（プレイヤー自身のカレーが受ける系統別ダメージを軽減）／
//       'instant'（抽選した瞬間にその場で加算し、festActiveToppingsには残さない＝重複所持にはならない）
// 全項目が同確率（等確率の抽選）。重複して同じ効果を引いた場合、stat/crit/reductionは加算で重ねがけされる。
const FEST_TOPPING_DEFS = [
    { id:'hp20',            label:'HP+20',              type:'stat', stat:'hp',  value:20 },
    { id:'hp30',            label:'HP+30',              type:'stat', stat:'hp',  value:30 },
    { id:'atk20',           label:'ATK+20',             type:'stat', stat:'atk', value:20 },
    { id:'atk30',           label:'ATK+30',             type:'stat', stat:'atk', value:30 },
    { id:'def20',           label:'DEF+20',             type:'stat', stat:'def', value:20 },
    { id:'def30',           label:'DEF+30',             type:'stat', stat:'def', value:30 },
    { id:'spd30',           label:'SPD+30',             type:'stat', stat:'spd', value:30 },
    { id:'spd50',           label:'SPD+50',             type:'stat', stat:'spd', value:50 },
    { id:'crit10',          label:'Spicy Hit率 +10%',   type:'crit', value:0.10 },
    { id:'crit20',          label:'Spicy Hit率 +20%',   type:'crit', value:0.20 },
    { id:'poisonGuard',     label:'毒ガード 100%',       type:'guard', kind:'poison' },
    { id:'illusionGuard',   label:'幻惑ガード 100%',     type:'guard', kind:'illusion' },
    { id:'gold100',         label:'ゴールド +100G',      type:'instant', instant:'gold',       value:100 },
    { id:'gold300',         label:'ゴールド +300G',      type:'instant', instant:'gold',       value:300 },
    { id:'healSpice3',      label:'回復スパイス +3個',    type:'instant', instant:'healSpice',  value:3 },
    { id:'spicySpice3',     label:'辛味スパイス +3個',    type:'instant', instant:'spicySpice', value:3 },
    { id:'expSpice5',       label:'経験スパイス +5個',    type:'instant', instant:'expSpice',   value:5 },
    { id:'fp30',            label:'FP +30',             type:'instant', instant:'fp',         value:30 },
    { id:'meatReduce20',    label:'肉系ダメージ軽減 20%', type:'reduction', category:'meat',      value:0.20 },
    { id:'seafoodReduce20', label:'海鮮系ダメージ軽減 20%', type:'reduction', category:'seafood',  value:0.20 },
    { id:'vegetableReduce20', label:'野菜系ダメージ軽減 20%', type:'reduction', category:'vegetable', value:0.20 },
    { id:'fruitReduce20',   label:'果物系ダメージ軽減 20%', type:'reduction', category:'fruit',    value:0.20 },
    { id:'meatReduce30',    label:'肉系ダメージ軽減 30%', type:'reduction', category:'meat',      value:0.30 },
    { id:'seafoodReduce30', label:'海鮮系ダメージ軽減 30%', type:'reduction', category:'seafood',  value:0.30 },
    { id:'vegetableReduce30', label:'野菜系ダメージ軽減 30%', type:'reduction', category:'vegetable', value:0.30 },
    { id:'fruitReduce30',   label:'果物系ダメージ軽減 30%', type:'reduction', category:'fruit',    value:0.30 },
];
function getFestToppingDef(id) { return FEST_TOPPING_DEFS.find(function(t){ return t.id === id; }) || null; }
// プレイヤー自身のカレーのみに適用するステータス加算合計（仲間botには一切適用しない）
function getFestToppingStatBonus(statKey) {
    return festActiveToppings.reduce(function(sum, id) {
        const def = getFestToppingDef(id);
        return (def && def.type === 'stat' && def.stat === statKey) ? sum + def.value : sum;
    }, 0);
}
// プレイヤー自身の会心率（Spicy Hit率）にのみ加算する合計
function getFestToppingCritBonus() {
    return festActiveToppings.reduce(function(sum, id) {
        const def = getFestToppingDef(id);
        return (def && def.type === 'crit') ? sum + def.value : sum;
    }, 0);
}
// 毒ガード／幻惑ガード（100%）を持っているか
function hasFestToppingGuard(kind) {
    return festActiveToppings.some(function(id) {
        const def = getFestToppingDef(id);
        return def && def.type === 'guard' && def.kind === kind;
    });
}
// 指定した系統（meat/seafood/vegetable/fruit）へのダメージ軽減合計（複数所持していれば加算）
function getFestToppingDamageReduction(category) {
    if(!category) return 0;
    return festActiveToppings.reduce(function(sum, id) {
        const def = getFestToppingDef(id);
        return (def && def.type === 'reduction' && def.category === category) ? sum + def.value : sum;
    }, 0);
}
// プレイヤー自身のカレーが受けるダメージにのみ軽減を適用する（仲間botが受けるダメージには適用しない）。
// 軽減率は80%を上限とし、必ず最低1ダメージは通す（無敵化を防ぐ安全策）。
function applyFestToppingDamageReduction(d, category, targetType) {
    if(targetType !== 'curry' || !category) return d;
    const reduction = getFestToppingDamageReduction(category);
    if(!reduction) return d;
    return Math.max(1, Math.round(d * (1 - Math.min(0.8, reduction))));
}
// 連勝数が新たな閾値（20、以降30・40・50…）に到達した回数を返す（通常は1回勝利につき最大1）。
// 呼び出し側はこの戻り値の回数だけ、その場でトッピングスロットを自動実行する。
function checkFestToppingSlotUnlock() {
    let count = 0;
    while(festWinStreak >= festNextToppingThreshold) {
        festNextToppingThreshold += 10;
        count += 1;
    }
    return count;
}
...
const FEST_ZAKO_DEFS = [
    ...
    // 20連勝（festWinStreak>=20）以降、21戦目から登場する追加雑魚敵。unlockStreakでbuildFestBattleEnemies側から絞り込む。
    { name:'お楽しみ袋ぷにぽん', hp:140, atk:32, def:32, spd:30, image:'botimage/bot24.png', unlockStreak:20,
      curryName:'気まぐれ福袋の新玉ねぎカレー', curryIcon:'foods_icon/item_59.svg',
      skill:{ name:'ぷにぷにプレゼント', levelReq:3, chance:0.30, type:'bonusDamageSkip', desc:'30%で通常攻撃の1.2倍ダメージ＋相手を1回休みにする' } },
];
...
// 現在のレベルに応じた素のステータス（基本値＋(現在Lv−初期Lv)×10ポイントを基本比率で配分。装備補正は含まない）
function getFestAllyRawStats(name) {
    const def = getFestAllyDef(name);
    if(!def) return null;
...
// 素のステータス＋仲間専用のベース・食器補正（festAllySelectedBase/festAllySelectedTableware）。戦闘計算はすべてこちらを使う
function getFestAllyCurrentStats(name) {
    const raw = getFestAllyRawStats(name);
    if(!raw) return null;
    const twInfo = TABLEWARE_LIST[festAllySelectedTableware] || {};
    const bsInfo = BASE_LIST[festAllySelectedBase] || {};
    return {
        name: raw.name,
        level: raw.level,
        hp: Math.max(1, raw.hp + (twInfo.hp || 0) + (bsInfo.hp || 0)),
...
function saveFestState() {
    ...
    const session = {
        festActive, festPaused, festResumeMode, festFP, festInventory, festSpiceInventory, festCurryStock,
        ...
        festNextToppingThreshold, festActiveToppings
    };
...
function loadFestState() {
    ...
    try {
        ...
        festActive = true;
        festPaused = !!session.festPaused;
        festResumeMode = session.festResumeMode || 'hub';
        ...
        festNextToppingThreshold = session.festNextToppingThreshold || 20;
        festActiveToppings = session.festActiveToppings || [];
...
function enterCurryFest() {
    if(festActive) {
        ...
        saveFestState();
        showFestScreen();
        // 一時中断した時点が「本来準備画面に戻れない区間」だった場合は、準備画面で止めず
        // マッチング画面の再表示／続行処理まで自動的に進める
        if(festResumeMode === 'match') {
            openFestMatchModal();
        } else if(festResumeMode === 'pendingContinue') {
            festBattleContinue();
        }
        return;
    }
...
// フェスの実行時状態（フェス開始時にリセットされる各種変数）をまとめてクリアする共通処理。
// フェス開始時（startNewFestRun）だけでなく、フェス終了時（撤退・敗北の両方）でも必ず呼ぶこと。
// これが無いと、フェス終了後もfestActiveToppings等メモリ上の値が残り続け、フェス外の画面とも共有している
// statDisplayWithTableware()等を通じて、フェス外のカレーのステータス表示にフェスで獲得したトッピング効果が
// 混ざって見えてしまう、といった不具合につながる（実際に報告された不具合）。
function clearFestRuntimeState() {
    festFP = 0;
    festInventory = {};
    festSpiceInventory = {};
    festCurryStock = [];
    ...
    festWinStreak = 0;
    festNextToppingThreshold = 20; festActiveToppings = [];
    festSetIndex = 1;
...
function retreatFromFest() {
    const msg = festBattleInProgress
        ? 'フェスを終了し、現時点までの報酬 <span style="color:#2ecc71;">' + festAccumulatedReward + ' G</span> を獲得します。<br>戦闘中のデータは破棄されます。'
        : 'フェスを終了し、現時点までの報酬 <span style="color:#2ecc71;">' + festAccumulatedReward + ' G</span> を獲得します。';
    showCustomConfirm('🚩 撤退', msg, function() {
        maybePromptFestExpSpiceBeforeEnd(function() {
            ...
            clearFestRuntimeState(); // トッピング効果等、フェス専用のメモリ上状態をここで確実にクリアする
            hideFestScreen();
        });
    });
}
// 一時中断＝フェスの状況（食材・カレー・連勝数など）を保存したまま、対戦タブに戻る。報酬は受け取らない。
// festActiveはtrueのまま維持し、festPausedをtrueにすることで「対戦タブ＞カレーフェス」から同じ状態のまま再開できる。
function confirmFestPause() {
    const msg = '報酬は貰わず一時中断します。後で再開できます。';
    showCustomConfirm('⏸️ 一時中断', msg, function() {
        festPaused = true;
        saveFestState();
        stopBattleBGM();
        hideFestScreen();
...
// 「中断」ボタン：即実行はせず、まず「一時中断」「撤退」のどちらを行うか選ばせる
function openFestSuspendChoiceModal() {
    const msg = '<div style="font-size:13px;color:#420000;margin-bottom:16px;line-height:1.6;">どちらを行いますか？</div>'
        + '<button onclick="closeModalThen(confirmFestPause)" style="width:100%;margin-bottom:10px;padding:10px;background:#b88742;color:#efdeb1;border:none;border-radius:4px;font-weight:bold;font-size:13px;cursor:pointer;">⏸️ 一時中断</button>'
        + '<button onclick="closeModalThen(retreatFromFest)" style="width:100%;padding:10px;background:#420000;color:#efdeb1;border:none;border-radius:4px;font-weight:bold;font-size:13px;cursor:pointer;">🚩 撤退</button>';
    document.getElementById("modalTitle").innerText = "🎪 フェスを中断";
    document.getElementById("modalMessage").innerHTML = msg;
    document.getElementById("modalBtnGroup").innerHTML = '<button class="modal-btn modal-btn-cancel" style="width:100%;" onclick="document.getElementById(\'customModal\').style.display=\'none\';">キャンセル</button>';
    document.getElementById("customModal").style.display = "flex";
}
function showFestScreen() {
    ...
    fetchFestWeeklyGlobalRecord();
    updateFestWeeklyBestDisplay();
}
...
// ===== カレーフェス：週間連勝記録（JST日曜0:00〜土曜23:59を1週として集計） =====
// 週ID＝その週の日曜日の日付（YYYY-MM-DD、JST基準）。文字列として辞書順=時系列順になる。
function getFestWeekId(baseDate) {
    const now = baseDate || getGameNow();
    const jstMs = now.getTime() + 9 * 60 * 60000;
    const jst = new Date(jstMs);
    const dow = jst.getUTCDay(); // 0=日曜
    const sunday = new Date(jstMs - dow * 86400000);
...
// 今週の自分のベスト連勝数を返す（週が変わっていれば0扱い。保存はしない＝表示・比較専用）
function getFestWeeklyBestStreak() {
    const s = getStats();
    if((s.festWeeklyBestStreakWeekId || '') !== getFestWeekId()) return 0;
    return s.festWeeklyBestStreak || 0;
}
function updateFestWeeklyBestDisplay() {
...
function updateFestWeeklyGlobalRecordIfHigher() {
    if(!database || !playerId) return;
    if(FEST_ADMIN_EXCLUDED_IDS.includes(playerId)) return; // 管理者キャラの記録は今週の全プレイヤー記録に反映しない
    const weekId = getFestWeekId();
    // 全プレイヤー表示用の最高値（表示専用。単純な「より高ければ更新」でよい）
    const curMax = (festWeeklyGlobalMaxStreakWeekId === weekId) ? festWeeklyGlobalMaxStreak : 0;
    if(festWinStreak > curMax) {
        database.ref('festWeeklyRecord/' + weekId + '/maxStreak').set(festWinStreak).then(function() {
            festWeeklyGlobalMaxStreak = festWinStreak;
            festWeeklyGlobalMaxStreakWeekId = weekId;
            const el = document.getElementById('festWeeklyGlobalRecordStreak');
            if(el) el.innerText = festWeeklyGlobalMaxStreak;
        }).catch(function() { /* 更新失敗時は次回同期時に再試行される */ });
    }
    // 同率タイを含めて報酬対象を正しく判定できるよう、「今週の自分の記録」を各プレイヤーごとに保持する
    // （管理ツール側で全プレイヤー分を読み、最高記録と同値の全員を報酬対象として抽出する）。
    // トランザクションで「自分の既存値より高い場合のみ」更新し、同時書き込みによる競合も安全に処理する。
    database.ref('festWeeklyRecord/' + weekId + '/entries/' + playerId).transaction(function(cur) {
        const curStreak = cur ? cur.streak : 0;
        if(festWinStreak > curStreak) return { streak: festWinStreak, name: playerName || '名無しの料理人' };
        return; // 変更なし（トランザクション中止）
    }).catch(function() { /* 更新失敗時は次回同期時に再試行される */ });
}
// 自分の今週の連勝自己ベストを更新する（週が変わっていれば0からリセットしてから比較。プレイヤーごとにqr_statsへ永続保存）
function festUpdateWeeklyPersonalBest() {
    const weekId = getFestWeekId();
    updateStats(function(s) {
        if((s.festWeeklyBestStreakWeekId || '') !== weekId) {
            s.festWeeklyBestStreakWeekId = weekId;
            s.festWeeklyBestStreak = 0;
        }
        if(festWinStreak > (s.festWeeklyBestStreak || 0)) s.festWeeklyBestStreak = festWinStreak;
    });
    updateFestWeeklyBestDisplay();
}
...
// カレーフェス連勝報酬「食品サンプル」：20/30/40/50連勝でそれぞれ1度きり付与。
// フェス外（通常のバトル・冷蔵庫）で使うアイテムのため、フェス専用のsaveFestState()ではなく
// 通常のsaveGame()で永続化する。到達したその場でPOPを出す（フェス終了時の報酬とは別扱い）。
function checkFestFoodSampleMilestones() {
    let granted = false;
    if(festWinStreak >= 20 && !festSampleClaimed20) {
        festSampleClaimed20 = true; foodSampleCount = (foodSampleCount || 0) + 1; granted = true;
        setTimeout(function(){ showCustomAlert('🎁 カレーフェス20連勝達成！', '🍥 食品サンプルを1個手に入れた！'); }, 1500);
    }
    if(festWinStreak >= 30 && !festSampleClaimed30) {
        festSampleClaimed30 = true; foodSampleCount = (foodSampleCount || 0) + 5; granted = true;
        setTimeout(function(){ showCustomAlert('🎁 カレーフェス30連勝達成！', '🍥 食品サンプルを5個手に入れた！'); }, 1500);
    }
    if(festWinStreak >= 40 && !festSampleClaimed40) {
        festSampleClaimed40 = true; foodSampleCount = (foodSampleCount || 0) + 10; granted = true;
        setTimeout(function(){ showCustomAlert('🎁 カレーフェス40連勝達成！', '🍥 食品サンプルを10個手に入れた！'); }, 1500);
    }
    if(festWinStreak >= 50 && !festSampleClaimed50) {
        festSampleClaimed50 = true; foodSampleCount = (foodSampleCount || 0) + 20; granted = true;
        setTimeout(function(){ showCustomAlert('🎁 カレーフェス50連勝達成！', '🍥 食品サンプルを20個手に入れた！'); }, 1500);
    }
    if(granted) {
        const foodSampleElFest = document.getElementById('globalFoodSample');
        if(foodSampleElFest) foodSampleElFest.innerText = foodSampleCount;
        saveGame();
    }
}
...
function updateFestStatusBar() {
    ...
    updateFestWeeklyGlobalRecordIfHigher();
...
// トッピングスロットで獲得したステータス加算も、プレイヤー自身のカレーにのみここで加算する（仲間botには適用しない）。
function festCurryStatWithTableware(baseVal, statKey) {
    const info = TABLEWARE_LIST[selectedTableware];
    const bsInfo = BASE_LIST[selectedBase];
    const mod = (info ? (info[statKey] || 0) : 0) + (bsInfo ? (bsInfo[statKey] || 0) : 0) + getFestToppingStatBonus(statKey);
    return Math.max(0, baseVal + mod);
}
// フェスの自分のカレーの「最大HP」＝カレー本来のhp＋食器・ベースのhp補正（例：ターリー皿HP+20／ナンHP+20）＋トッピングのHP加算。
// stockデータのcurry.hp自体は書き換えない（食器・ベース・トッピングは戦闘外でいつでも変更できるため、都度この関数で計算する）。
function festCurryMaxHp(curry) {
    if (!curry) return 0;
    const twInfo = TABLEWARE_LIST[selectedTableware] || {};
    const bsInfo = BASE_LIST[selectedBase] || {};
    return Math.max(1, curry.hp + (twInfo.hp || 0) + (bsInfo.hp || 0) + getFestToppingStatBonus('hp'));
}
...
function makeFestEnemyInstance(def, mult, level, isBoss) {
    ...
    return {
        ...
        // お楽しみ袋ぷにぽん「ぷにぷにプレゼント」等、戦闘終了後のアイテムドロップ判定に使う「この戦闘で技を使用したか」フラグ
        namedSkillUsed: false
    };
}
function buildFestBattleEnemies() {
    ...
    if(festBattlePhase <= 3) {
        // unlockStreak（例：お楽しみ袋ぷにぽんのunlockStreak:20）が設定されている敵は、
        // festWinStreakがその値以上になるまで出現候補から除外する
        const pool = FEST_ZAKO_DEFS.filter(function(d){ return festWinStreak >= (d.unlockStreak || 0); });
        const picks = [];
...
function openFestMatchModal() {
    if(festBattleInProgress) return;
    // このマッチング画面〜戦闘中は本来「準備画面」に戻れない区間。一時中断からの再開時に
    // 誤って準備画面へ戻せてしまわないよう、区間に入った時点でfestResumeModeを記録しておく。
    festResumeMode = 'match';
    saveFestState();
    ensureFestCurryHp();
    if(getFestActiveCurryIndex() === -1) {
        showCustomAlert('⚠️ 出撃できるカレーがありません', 'HPが残っているストックカレーがありません。調理してから進んでください。');
...
function startFestBattle() {
    ...
    // 「フェス戦即決」の適用範囲（20戦目まで/30戦目まで）を超えた場合はここでスキップボタンを
    // 非表示にし、スキップモードも強制解除する。これにより直後のbattleDelay(700)が正しい速度で計算される。
    updateFestSkipButtonVisibility();
    ...
    festPlayerSkipTurn = false;
    festAllySkipTurn = false;
    ...
    // 通常PC戦・タッグ戦と同様、戦闘開始時にバトル画面が見えるようスクロールする
    requestAnimationFrame(() => {
        const arena = document.getElementById('festBattleStage');
...
function festUpdateAllHpDisplays() {
    ...
    refreshFestBattleNameIcons();
}
// 名前横の状態アイコン：プレイヤー自身のカレー・雇用中の仲間bot・敵（最大2体）それぞれの現在の状態から算出して反映する。
// festUpdateAllHpDisplays()はHPが変化するたびに（=ほぼ毎ターン）呼ばれているため、ここに載せておけば自然と同期する。
function refreshFestBattleNameIcons() {
    const curry = festCurryStock[festActiveCurryIdx];
    const playerKeys = [];
    if(festPlayerIlluded) playerKeys.push('illusion');
    if(festPlayerPoisoned) playerKeys.push('poison');
    if(festPlayerAtkDownActive || festPlayerAtkDownTurns > 0) playerKeys.push('atkDown');
    if(festPlayerSkipTurn) playerKeys.push('skipTurn');
    // トッピングスロットのステータス加算（HP/ATK/DEF/SPD等）は常時の数値ボーナスであり、アイコン表示の対象にはしない。
    // ATKアップアイコンは辛味スパイスによる一時バフのみを対象とする（DEFアップに相当するトッピングは元々無い）。
    if(festSpicyBuffActive) playerKeys.push('atkUp');
    if(curry && curry.isRatatouille) playerKeys.push('hpRegen');
    const reduceCategories = { meat: false, vegetable: false, seafood: false, fruit: false };
    if(curry && curry.isRatatouille) reduceCategories.meat = true;
    ['meat','vegetable','seafood','fruit'].forEach(function(cat){ if(getFestToppingDamageReduction(cat) > 0) reduceCategories[cat] = true; });
    if(festCurryFluffyCategory && reduceCategories.hasOwnProperty(festCurryFluffyCategory)) reduceCategories[festCurryFluffyCategory] = true; // ふわとろオム／世界三大珍味バリア
    if(reduceCategories.meat) playerKeys.push('meatReduce');
    if(reduceCategories.vegetable) playerKeys.push('vegReduce');
    if(reduceCategories.seafood) playerKeys.push('seafoodReduce');
    if(reduceCategories.fruit) playerKeys.push('fruitReduce');
    updateBattleStatusIconsFor('festPlayerName0', playerKeys);

    const allyKeys = [];
    if(festHiredAllyName) {
        if(festAllyIlluded) allyKeys.push('illusion');
        if(festAllyPoisoned) allyKeys.push('poison');
        if(festAllyAtkDownActive || festAllyAtkDownTurns > 0) allyKeys.push('atkDown');
        if(festAllySkipTurn) allyKeys.push('skipTurn');
        if(festSpicyBuffActive) allyKeys.push('atkUp');
    }
    updateBattleStatusIconsFor('festPlayerName1', allyKeys);

    festBattleEnemies.forEach(function(e, i) {
        const keys = [];
        if(e.isIlluded) keys.push('illusion');
        if(e.isPoisoned) keys.push('poison');
        if(e.skipNextTurn) keys.push('skipTurn');
        updateBattleStatusIconsFor('festEnemyName' + i, keys);
    });
}
...
function festTriggerDamagePop(side, idx, value, color) {
    ...
    if(typeof value === 'string') {
        // 貝の盾カレーのガード演出等、数値ではなく文字（例："Guard"）をそのまま表示したい場合
        pop.innerText = value;
    } else if(isCrit) {
...
function festPlayerActAttack(unitType, curry, allyStats, callback) {
    ...
    // お楽しみ袋ぷにぽん「ぷにぷにプレゼント」で付与された1回休み：この行動を消費して1回休みにする（敵側のskipNextTurn消費と同じ考え方）
    if(unitType === 'curry' && festPlayerSkipTurn) {
        festPlayerSkipTurn = false;
        festLogSet('💤 ' + playerName + 'は行動を1回休んだ！');
        done();
        return;
    }
    if(unitType === 'ally' && festAllySkipTurn) {
        festAllySkipTurn = false;
        festLogSet('💤 ' + festHiredAllyName + 'は行動を1回休んだ！');
        done();
        return;
    }
    if(unitType === 'ally') {
    ...
    // トッピングスロットのSpicy Hit率+はプレイヤー自身のカレーの会心率にのみ加算する（仲間botには適用しない）
    const critRate = getCritRate(spd, target.spd) + (unitType === 'curry' ? getFestToppingCritBonus() : 0);
...
function festEnemyActAttackSimple(e, curry, allyStats) {
    ...
    // 貝の盾カレー（isKaiTate）：通常攻撃を確率でガードしてダメージ0にする
    if(t.type === 'curry' && curry && curry.isKaiTate && rollShieldGuard(targetSpd)) {
        playSoundEffect('sound/guard.mp3');
        festLogAppend('🛡️ ' + targetName + 'が攻撃を貝の盾でガードした。');
        festTriggerDamagePop('player', t.idx, 'Guard', '#3498db');
        festUpdateAllHpDisplays();
        return;
    }
    ...
    d = applyFestToppingDamageReduction(d, e.category, t.type); // トッピングスロットの系統別ダメージ軽減（プレイヤー自身のカレーのみ）
...
function festEnemyActAttack(e, curry, allyStats, enemyIdx, callback) {
    ...
    // お楽しみ袋ぷにぽん「ぷにぷにプレゼント」：通常攻撃の1.2倍ダメージ＋相手を1回休みにする
    const bonusDamageSkipProc = unlocked && skill.type === 'bonusDamageSkip' && Math.random() < skill.chance;
    const anySkillProc = doubleDamageProc || bonusDamageProc || poisonProc || atkDownProc || extraTurnProc || periodicCritProc || illusionProc || bonusDamageSkipProc;
    ...
    function resolveAttack() {
        ...
        if(bonusDamageSkipProc) {
            // 撃破後のドロップ判定用に「この戦闘で技を使用したか」を記録する（ホームランで無効化された場合も発動自体はしているので使用扱い）
            e.namedSkillUsed = true;
            // ホームランカレー（isHomerun）：「ぷにぷにプレゼント」発動時のみ、一定確率で打ち返して無効化できる（スパイス爆発と同じ仕組み）
            if(t.type === 'curry' && curry && curry.isHomerun && isHomerunReflect(festCurryStatWithTableware(curry.spd, 'spd'), e.spd)) {
                homerunNegated = true;
            } else {
                d = Math.round(d*1.2); skillMsg = '「' + skill.name + '」発動！ダメージ1.2倍＆1回休み！';
            }
        }
        if(homerunNegated) {
        ...
        if(poisonProc) {
            if(t.type === 'curry' && hasFestToppingGuard('poison')) {
                festLogAppend('毒ガードの効果で毒を回避した！');
            } else if(targetBaseInfo.poisonAvoid && Math.random() < 0.30) {
        ...
        if(illusionProc) {
            if(t.type === 'curry' && hasFestToppingGuard('illusion')) {
                festLogAppend('幻惑ガードの効果で幻惑を回避した！');
            } else if(targetBaseInfo.illusionAvoid && Math.random() < 0.30) {
        ...
        if(bonusDamageSkipProc) {
            // ホームランで無効化された場合はこの時点まで到達しない（早期returnで抜けている）ため、ここに来た時点で必ず休み状態を付与する
            if(t.type === 'curry') festPlayerSkipTurn = true; else festAllySkipTurn = true;
            festLogAppend('💤 ' + targetName + 'は「' + skill.name + '」で次の行動を1回休みになった！');
        }
        if(extraTurnProc) {
    ...
    // 貝の盾カレー（isKaiTate）：ボス固有技を除く通常攻撃のみ確率でガードしてダメージ0にする
    if(!anySkillProc && t.type === 'curry' && curry && curry.isKaiTate && rollShieldGuard(targetSpd)) {
        playSoundEffect('sound/guard.mp3');
        festLogSet('🛡️ ' + targetName + 'が攻撃を貝の盾でガードした。');
        festTriggerDamagePop('player', t.idx, 'Guard', '#3498db');
        festUpdateAllHpDisplays();
        done();
        return;
...
function chooseFestCurrySwap(idx) {
    ...
    const overlay = document.getElementById('festCurrySwapOverlay');
    if(overlay) overlay.style.display = 'none';
    // 力尽きた前のカレーが受けていたデバフ（毒・幻惑・ATKダウン・1回休み）は、新しく出撃するカレーには引き継がない
    festPlayerPoisoned = false;
    festPlayerIlluded = false;
    festPlayerAtkDownActive = false;
    festPlayerAtkDownTurns = 0;
...
// お楽しみ袋ぷにぽん「ぷにぷにプレゼント」を1度でも使用された戦闘の勝利時にドロップする戦利品を1つ抽選する。
// ノーマル食材1個50%／中級食材1個10%／経験スパイス1〜3個20%／辛味スパイス1〜3個20%。
// フェス内アイテムなので、フェス外のinventoryではなくfestInventory/festExpSpice/festSpicySpiceへ加算する。
function rollFestPunipuponDrop() {
    const r = Math.random();
    if(r < 0.50) {
        const normalPool = Object.keys(masterIngredients).filter(function(n){ return masterIngredients[n].shop === 0 && isIngredientAvailable(n); });
        if(normalPool.length === 0) return null;
        const item = normalPool[Math.floor(Math.random() * normalPool.length)];
        festInventory[item] = (festInventory[item] || 0) + 1;
        return '🎁 「ぷにぷにプレゼント」の戦利品：' + item + 'を1個手に入れた！';
    } else if(r < 0.60) {
        const midPool = Object.keys(masterIngredients).filter(function(n){ return masterIngredients[n].shop === 1 && isIngredientAvailable(n); });
        if(midPool.length === 0) return null;
        const item = midPool[Math.floor(Math.random() * midPool.length)];
        festInventory[item] = (festInventory[item] || 0) + 1;
        return '🎁 「ぷにぷにプレゼント」の戦利品：中級食材「' + item + '」を1個手に入れた！';
    } else if(r < 0.80) {
        const amt = 1 + Math.floor(Math.random() * 3);
        festExpSpice += amt;
        return '🎁 「ぷにぷにプレゼント」の戦利品：経験スパイスを' + amt + '個手に入れた！';
    } else {
        const amt = 1 + Math.floor(Math.random() * 3);
        festSpicySpice += amt;
        return '🎁 「ぷにぷにプレゼント」の戦利品：辛味スパイスを' + amt + '個手に入れた！';
    }
}
function festBattleWin() {
    ...
    festPlayerSkipTurn = false; festAllySkipTurn = false;
    festStopBattleBgm();
    // 勝利報酬は反映されるが「次の対戦へ進む／準備画面へ進む」を押すまでは本来準備画面に戻れない区間。
    // 一時中断からの再開時にfestBattleContinue()相当までそのまま進められるよう記録しておく。
    festResumeMode = 'pendingContinue';
    festWinStreak += 1; // 雑魚戦・ボス戦を問わず、1勝ごとに連勝数+1（報酬額の計算は引き続きセット数ベース）
    // festBattleEnemiesが次戦向けに入れ替わる前に、この戦闘中に「ぷにぷにプレゼント」が使用されたか判定してドロップ抽選する
    const punipuponDropMsg = festBattleEnemies.some(function(e){ return e.namedSkillUsed; }) ? rollFestPunipuponDrop() : null;
    festUpdatePersonalBestAndIconUnlock();
    festUpdateWeeklyPersonalBest();
    checkFestFoodSampleMilestones();
    ...
    festShowBattleResultOverlay('WIN!', '#2ecc71', function() {
        ...
        if(resultText) resultText.innerText = punipuponDropMsg || '';
        ...
        // 20連勝・以降10連勝ごとに、勝利のその場でトッピングスロットを自動実行してから結果画面を表示する
...
function returnToFestHubAfterBattle() {
    festResumeMode = 'hub'; // 準備画面に戻ったので、一時中断からの再開先も準備画面に戻す
    const battlePanel = document.getElementById('festBattlePanel');
    if(battlePanel) battlePanel.style.display = 'none';
...
function finishFestByDefeat() {
    maybePromptFestExpSpiceBeforeEnd(function() {
        ...
        const finalStreak = festWinStreak;
        festActive = false;
        localStorage.removeItem('qr_fest_session');
        clearFestRuntimeState(); // トッピング効果等、フェス専用のメモリ上状態をここで確実にクリアする
        hideFestScreen();
        showCustomAlert('🏁 フェス終了', '合計 <span style="color:#2ecc71;">' + rewardTotal + ' G</span> を受け取りました！（最高連勝：' + finalStreak + '連勝）');
    });
}
...
function renderFestAllyUI() {
    ...
    if(allyBox) {
        if(festHiredAllyName) {
            ...
            const stats = getFestAllyCurrentStats(festHiredAllyName);
            const rawStats = getFestAllyRawStats(festHiredAllyName);
            ...
            allyBox.innerHTML =
                ...
                + 'HP:' + curHp + '/' + festAllyStatDisplayWithTableware('hp', rawStats.hp) + ' ATK:' + festAllyStatDisplayWithTableware('atk', rawStats.atk) + ' DEF:' + festAllyStatDisplayWithTableware('def', rawStats.def) + ' SPD:' + festAllyStatDisplayWithTableware('spd', rawStats.spd) + '<br>'
    ...
    renderFestToppingEffectBox();
    renderFestDebugPanel();
}
// 仲間タブ「トッピング効果」欄：現在有効なトッピング（プレイヤー自身のカレーにのみ適用）の一覧を表示するだけ。
// スロット自体は20連勝・以降10連勝ごとに勝利直後その場で自動的に実行されるため、ここに抽選ボタンは置かない。
function renderFestToppingEffectBox() {
    const box = document.getElementById('festToppingEffectBox');
    if(!box) return;
    const counts = {};
    festActiveToppings.forEach(function(id){ counts[id] = (counts[id] || 0) + 1; });
    const ids = Object.keys(counts);
    if(ids.length === 0) {
        box.innerHTML = '<div style="color:#888;">現在有効なトッピング効果はありません</div>';
        return;
    }
    box.innerHTML = ids.map(function(id){
        const def = getFestToppingDef(id);
        if(!def) return '';
        const label = def.label + (counts[id] > 1 ? '　×' + counts[id] : '');
        return '<div style="padding:2px 0;">' + label + '</div>';
    }).join('');
}
...
// コマの高さは文字の実際の高さぎりぎりまで詰める（コマ枠に対して余白が大きいと、静止時にコマの上下が
// 見切れて見える部分がちょうど「余白（＝何も書かれていない部分）」に当たりやすくなり、回転中に比べて
// 静止時だけやたら行間が広く空いて見えてしまう。文字がコマいっぱいまで詰まっていれば、見切れた部分にも
// 文字そのものが写るようになり、回転中・静止時で見た目の詰まり具合がほぼ揃う）。
const TOPPING_SLOT_ITEM_H = 30;                              // リール1コマの高さ(px)
const TOPPING_SLOT_VIEWPORT_H = TOPPING_SLOT_ITEM_H * 2;      // 中央のコマ＋上下それぞれ半分ずつ見切れるくらいの高さ
const TOPPING_SLOT_CENTER_OFFSET = (TOPPING_SLOT_VIEWPORT_H - TOPPING_SLOT_ITEM_H) / 2; // 中央のコマをビューポート中央に揃えるためのオフセット
const TOPPING_SLOT_FAST_SPEED = 0.6;                          // 高速回転中のスクロール速度(px/ms)
...
function runFestToppingSlotQueue(count, onAllDone) {
    ...
    openFestToppingSlotAuto(function() {
        runFestToppingSlotQueue(count - 1, onAllDone);
    });
}
...
function resetFestToppingSlotState() {
    ...
    // 静止時にindex1が中央に来るようscrollPxを設定する（index0が下に、index2が上にそれぞれ半端に見切れて見える状態にするため。
    // index0を中央にすると「まだ何も通過していない」ため下側に見切れるコマが存在せず、下だけ不自然に空いてしまう）
    _toppingSlotScrollPx = 1 * TOPPING_SLOT_ITEM_H + TOPPING_SLOT_CENTER_OFFSET;
}
...
function renderFestToppingSlotBody() {
    ...
    body.innerHTML =
        ...
        + '<div style="height:' + TOPPING_SLOT_VIEWPORT_H + 'px; overflow:hidden; position:relative; background:#ffffff; border:2px solid #99aee3; border-radius:8px; margin:0 auto 18px; max-width:230px;">'
        + '<div id="toppingSlotTrack" style="position:absolute; left:0; right:0; top:0;"></div>'
        + '<div style="position:absolute; left:0; right:0; top:0; height:8px; background:linear-gradient(to bottom, rgba(255,255,255,0.95), rgba(255,255,255,0)); pointer-events:none;"></div>'
        + '<div style="position:absolute; left:0; right:0; bottom:0; height:8px; background:linear-gradient(to top, rgba(255,255,255,0.95), rgba(255,255,255,0)); pointer-events:none;"></div>'
        + '</div>'
...
function toppingSlotFastFrame(now) {
    ...
    // コマiが中央（またはそれ以降）に到達するのはscrollPxがおおよそi*ITEM_H+CENTER_OFFSETに達した頃なので、常にその少し先まで生成しておく
    ensureFestToppingSlotLabels(Math.ceil((_toppingSlotScrollPx - TOPPING_SLOT_CENTER_OFFSET) / TOPPING_SLOT_ITEM_H) + 8);
    renderFestToppingSlotTrackDom();
...
function stopFestToppingSlot() {
    ...
    const currentIndex = Math.floor((_toppingSlotScrollPx - TOPPING_SLOT_CENTER_OFFSET) / TOPPING_SLOT_ITEM_H);
    ...
    // コマiが中央に来る時のスクロール量は i*ITEM_H+CENTER_OFFSET
    _toppingSlotDecelTargetPx = targetIndex * TOPPING_SLOT_ITEM_H + TOPPING_SLOT_CENTER_OFFSET;
...
function applyFestToppingResult(def) {
    if(def.type === 'instant') {
        if(def.instant === 'gold') { festAccumulatedReward += def.value; }
        else if(def.instant === 'healSpice') { festHealSpice += def.value; }
        else if(def.instant === 'spicySpice') { festSpicySpice += def.value; }
        else if(def.instant === 'expSpice') { festExpSpice += def.value; }
        else if(def.instant === 'fp') { festFP += def.value; }
    } else {
        festActiveToppings.push(def.id);
    }
}
...
let unlockedFoodIconFeature = (safeLSGet('qr_food_icon_unlocked') === '1'); // 実績10個達成で食材アイコンをプレイヤーアイコンに使えるようになる
...
const INGREDIENT_CATEGORY = {
    ...
    vegetable: ["ナス", "トマト", "レンコン", "ピーマン", "赤パプリカ", "黄パプリカ", "玉ねぎ", "にんじん", "大根", "マッシュルーム", "オクラ", "パクチー", "トリュフ", "アスパラガス",
                "フルーツトマト", "メークイン", "金時にんじん", "聖護院大根", "新玉ねぎ", "賀茂なす", "ポルチーニ"],
    fruit:     ["りんご", "バナナ", "レーズン", "ココナッツ", "シャインマスカット", "島バナナ"]
};
...
// 日替わりカレー：報酬用の食材抽選・付与
// ============================================================
// categoryKey: 'meat'/'seafood'/'vegetable'/'fruit'/'fruitOrOther'（果物系＋系統未設定＝その他系）
// shopTier: 0=ノーマル, 1=中級, 2=高級
function getDailyCurryIngredientPool(categoryKey, shopTier) {
    return Object.keys(masterIngredients).filter(function(k) {
        if(masterIngredients[k].shop !== shopTier) return false;
        if(!isIngredientAvailable(k)) return false;
        const cat = getIngredientCategory(k);
        if(categoryKey === 'fruitOrOther') return cat === 'fruit' || cat === null;
        return cat === categoryKey;
    });
}
// スパイス用（masterSpices）。categoryKeyは不要（スパイスに系統分けはないため）。
function getDailyCurrySpicePool(shopTier) {
    return Object.keys(masterSpices).filter(function(k) {
        return masterSpices[k].shop === shopTier && isIngredientAvailable(k);
    });
}
// pool からランダムに count 個（重複あり）を選び、inventoryに加算する。取得した食材名の配列を返す。
function grantRandomFromPool(pool, count) {
    const got = [];
    if(!pool || pool.length === 0 || count <= 0) return got;
...
// 取得した食材名リストから「アイコン画像つきの簡易表示テキスト」を作る（種類ごとにまとめて「食材名×n」表示）
function dailyCurryLootHtml(gotList) {
    if(!gotList || gotList.length === 0) return '';
    const counts = {};
    gotList.forEach(function(n){ counts[n] = (counts[n]||0) + 1; });
    return Object.keys(counts).map(function(n){
        const d = masterIngredients[n] || masterSpices[n];
        const icon = d && d.icon ? `<img src="${d.icon}" style="width:1.2em;height:1.2em;vertical-align:middle;object-fit:contain;">` : '';
        return `${icon}${n}×${counts[n]}`;
    }).join(' / ');
}
...
function grantDailyCurryReward(weekdayIdx, tier) {
    ...
    let got = [];
    const normalCount = (r.normal && r.normal[tier]) || 0;
    if(normalCount > 0) {
        const pool = isSpice ? getDailyCurrySpicePool(0) : getDailyCurryIngredientPool(r.category, 0);
        got = got.concat(grantRandomFromPool(pool, normalCount));
    }
    const midCount = (r.mid && r.mid[tier]) || 0;
    if(midCount > 0) {
        const pool = isSpice ? getDailyCurrySpicePool(1) : getDailyCurryIngredientPool(r.category, 1);
        got = got.concat(grantRandomFromPool(pool, midCount));
    }
    const highCount = (r.high && r.high[tier]) || 0;
    if(highCount > 0) {
        const pool = isSpice ? getDailyCurrySpicePool(2) : getDailyCurryIngredientPool(r.category, 2);
        got = got.concat(grantRandomFromPool(pool, highCount));
    }
    const bonusCount = (r.bonusMidOrHigh && r.bonusMidOrHigh[tier]) || 0;
    if(bonusCount > 0) {
        // 中級or高級のどちらか1つをランダムに選ぶ（2つの候補プールを合体してまとめて抽選）
        const midPool = isSpice ? getDailyCurrySpicePool(1) : getDailyCurryIngredientPool(r.category, 1);
...
const RARE_TO_NORMAL_MAP = {
    ...
    "クルマエビ":"小エビ", "本マグロ":"ツナ",
    "賀茂なす":"ナス", "烏骨鶏の卵":"タマゴ", "ポルチーニ":"マッシュルーム",
    "島バナナ":"バナナ", "白花豆":"ひよこ豆"
};
...
const BASE_LIST = {
    ...
    'ターメリックライス': { hp: 0,  atk: 0, def: 0, spd: 30, desc: 'SPDが少し上がる。' }
};
...
// ステータス表示に、装備中食器＋ベース＋（フェス中のみ）トッピングスロット効果の補正を「合算した1つの数値」で色付き併記するヘルパー
// （例: 食器+20・ベース+20 → "50+40"(青字)）。個別に+20+20と両方表示するのではなく、必ず合計してから1回だけ表示する。
// フェス以外ではfestActiveToppingsは常に空のため、トッピング分は0として無視される（通常の食器・ベースのみの挙動と変わらない）。
...
// フェスの仲間bot専用のベース・食器選択（ストックカレー側のselectedBase/selectedTablewareとは独立。
// 所有物（getUnlockedBase/getUnlockedTableware）は共通だが、装備の選択状態だけ別に持つ）
let festAllySelectedBase = '白米';
let festAllySelectedTableware = '白い皿';
...
// 咖喱図書館：book1〜8のボスを全て討伐した報酬として解放される特別なベース（story.js側から付与）
let unlockTurmericRiceBase = false;
// フェス戦即決（スキップ）機能の購入フラグ。スパイシーコインで購入する有償アンロックのため、
// 他の食器・ベースの解放フラグと同様にクラウド同期・端末間引き継ぎの対象にする。
let unlockFestSkip20 = false;
let unlockFestSkip30 = false;
// フェスバトル中のみ有効な「スキップ」モードのON/OFF（フェスの実行時状態なのでrunごとにリセットする）
let festSkipModeActive = false;
...
function getUnlockedBase() {
    ...
    if (unlockTurmericRiceBase) list.push('ターメリックライス');
    return list;
}
...
let bookBossCurrySwapResolve = null; // 咖喱図書館：本のボス戦で出撃カレーが力尽きた時、次のカレー選択／あきらめるの結果を受け取るコールバック
let isMuted = (safeLSGet('qr_muted') === '1');
let isDebugMode = (safeLSGet('qr_debug_mode') === '1');
...
let playerId = safeLSGet('qr_player_id') || '';
let playerSecretKey = safeLSGet('qr_secret_key') || '';
...
let dailyCurryLastWinDate = ''; // 日替わりカレー：最後に勝利したJST日付（YYYY-MM-DD）。今日勝っていれば翌日まで再挑戦不可。
window.onload = function() { showIframeEmbedNotice(); setTimeout(enableGameStart, 5000); loadGame(); updateTablewareBaseUI(); updateCookSelects(); setupCookCategorySwipe(); refreshRecipeBookUI(); updateShopButtons(); listenToRooms(); updateMatchCurrySelects(); setupLoadedIconUI(); updateLvDisplay(); loadNotice(); initQuest(); loadTopNotice(); updateMuteIcon(); initCookAnimSetting(); initGuideCharSetting(); initPlayerId(function(){ enableGameStart(); syncDebugModeToCloud(); }); loadEventEnabledStatus(); setupModalScrollLock(); setupGuideCharLongPress(); setupTutorialSwipe(); updateLunchtimeBanner(); setInterval(updateLunchtimeBanner, 60000); checkBingoNewCardNotification(); renderShopInfoList(); initTopLayout(); playIntroAnimation(); loadFestState(); updateDailyCurryButtonImage(); setInterval(updateDailyCurryButtonImage, 60000); };
...
function proceedGameStart() {
    ...
    // ただし「一時中断」でfestPaused===trueの間は、対戦タブに戻れている＝タブメニューは既に表示済みのはずなので、
    // ここで強制的にフェス画面へ引き戻さない（一時中断した意味がなくなってしまうため）。
    if(festActive && !festPaused) {
        showFestScreen();
        loadNotice();
        onGameStartShowPost();
        return;
...
function goToTop() {
    ...
    // フェス画面を実際に表示している間はロゴタップでそのままトップ画面へ離脱させない。ヘッダーの
    // 「中断」ボタンと同じ選択モーダル（一時中断／撤退）を表示し、必ずどちらかを選んでもらう。
    // これにより、戦闘中・結果画面中にロゴから離脱してfestBattleContinue()等を経由せず
    // 準備画面に戻れてしまう（倒したはずのボスと際限なく連戦できる等の）不具合も併せて防げる。
    // ただしfestPaused（一時中断）で既に対戦タブ等フェス外に戻っている間は、pageFestが非表示
    // なのでこの選択モーダルは出さず、ロゴタップは通常通りトップ画面へ戻る動作にする。
    const pageFestEl = document.getElementById("pageFest");
    if(festActive && pageFestEl && pageFestEl.classList.contains("active")) { openFestSuspendChoiceModal(); return; }
    // 初期画面を表示
    hideCookStatPreview();
    ...
    const qrShortcutBtnTop = document.getElementById("qrShortcutBtn");
    if(qrShortcutBtnTop) qrShortcutBtnTop.style.display = "none";
    // ボタン・お知らせを確実に表示
...
// ⛩️京野菜カレー専用特技「ぶぶ漬けでも、いかがどす？」演出設定（熱々ブレスと同じDEF無視固定ダメージ）
const KYOYASAI_SKILL_CONFIG = {
    charaImg: 'battle/kyoto_chara1.png', chara2Img: 'battle/kyoto_chara2.png',
    bgImg: 'battle/homerun_bg.png',
    charaTop: '55%', charaWidth: '60%', charaMaxScale: 1.2
};
// ルーム戦で敵（対人戦の相手側）が使う場合の演出設定（bg2）
const KYOYASAI_SKILL_CONFIG_ENEMY = {
    charaImg: 'battle/kyoto_chara1.png', chara2Img: 'battle/kyoto_chara2.png',
    bgImg: 'battle/homerun_bg2.png',
    charaTop: '55%', charaWidth: '60%', charaMaxScale: 1.2
};
// 🧀チーズまみれカレー専用特技「チーズキャノン」演出設定（DEF無視ダメージ）
const CHEESE_SKILL_CONFIG = {
    charaImg: 'battle/cheese_chara1.png', chara2Img: 'battle/cheese_chara2.png',
    bgImg: 'battle/homerun_bg.png',
    charaTop: '55%', charaWidth: '60%', charaMaxScale: 1.2
};
// ルーム戦で敵（対人戦の相手側）が使う場合の演出設定（bg2）
const CHEESE_SKILL_CONFIG_ENEMY = {
    charaImg: 'battle/cheese_chara1.png', chara2Img: 'battle/cheese_chara2.png',
    bgImg: 'battle/homerun_bg2.png',
    charaTop: '55%', charaWidth: '60%', charaMaxScale: 1.2
};
// 🍤小えびスピードカレー専用特技「小エビよ！突撃だ！」演出設定（ATKではなくSPDで通常攻撃）
const SHRIMP_SKILL_CONFIG = {
    charaImg: 'battle/shrimp_chara1.png', chara2Img: 'battle/shrimp_chara2.png',
    bgImg: 'battle/homerun_bg.png',
    charaTop: '55%', charaWidth: '60%', charaMaxScale: 1.2
};
// ルーム戦で敵（対人戦の相手側）が使う場合の演出設定（bg2）
const SHRIMP_SKILL_CONFIG_ENEMY = {
    charaImg: 'battle/shrimp_chara1.png', chara2Img: 'battle/shrimp_chara2.png',
    bgImg: 'battle/homerun_bg2.png',
    charaTop: '55%', charaWidth: '60%', charaMaxScale: 1.2
};
    ...
    if(tagArena && tagArena.style.display === "block") {
        ...
        if(!tagOverlayShowing && !tagResultShowing) {
            ...
            if(tagBattleFighters && tagBattleFighters[0] && tagBattleFighters[0].curry) {
                // ※.curryは食器補正適用後の別オブジェクトのため参照が一致しない。元の参照(originalCurry)で探す。
                const myIdx = curryStock.indexOf(tagBattleFighters[0].originalCurry || tagBattleFighters[0].curry);
                // タブ切替による強制終了＝強制敗北扱いのため食品サンプルによる身代わり判定を行う
                if(myIdx !== -1) tryConsumeCurryOnLoss(myIdx);
...
// システム・設定「案内人」オン/オフ（デフォルトはオン）
function isGuideCharEnabled() {
    return localStorage.getItem('qr_guide_char_enabled') !== '0';
}
function setGuideCharEnabled(enabled) {
    localStorage.setItem('qr_guide_char_enabled', enabled ? '1' : '0');
    const onBtn  = document.getElementById('guideCharOnBtn');
    const offBtn = document.getElementById('guideCharOffBtn');
    if(onBtn && offBtn) {
        onBtn.classList.toggle('active', enabled);
        offBtn.classList.toggle('active', !enabled);
    }
    if(!enabled) hideQuestGuideChar(); // オフにしたら即座に非表示
}
function initGuideCharSetting() {
    setGuideCharEnabled(isGuideCharEnabled());
}
...
function saveGame() {
    ...
    localStorage.setItem('qr_curry_foodsample', foodSampleCount);
    ...
    localStorage.setItem('qr_fest_ally_selected_base', festAllySelectedBase);
    localStorage.setItem('qr_fest_ally_selected_tableware', festAllySelectedTableware);
    localStorage.setItem('qr_daily_curry_last_win_date', dailyCurryLastWinDate || '');
    ...
    localStorage.setItem('qr_unlock_turmeric_rice_base', unlockTurmericRiceBase ? '1' : '0');
    localStorage.setItem('qr_unlock_fest_skip_20', unlockFestSkip20 ? '1' : '0');
    localStorage.setItem('qr_unlock_fest_skip_30', unlockFestSkip30 ? '1' : '0');
    localStorage.setItem('qr_fest_sample_claimed_20', festSampleClaimed20 ? '1' : '0');
    localStorage.setItem('qr_fest_sample_claimed_30', festSampleClaimed30 ? '1' : '0');
    localStorage.setItem('qr_fest_sample_claimed_40', festSampleClaimed40 ? '1' : '0');
    localStorage.setItem('qr_fest_sample_claimed_50', festSampleClaimed50 ? '1' : '0');
    ...
    const foodSampleElSave = document.getElementById("globalFoodSample"); if(foodSampleElSave) foodSampleElSave.innerText = foodSampleCount;
    updateShopButtons();
    updateLvDisplay();
    syncSaveDataToCloud();
}
...
function syncSaveDataToCloud() {
    ...
    try {
        ...
        database.ref('players/' + playerId + '/savedata').set(data).then(function(){
            ...
            // UIDのズレが原因か、savedata側の増加量上限（不正対策）による拒否かは、
            // Firebaseのエラーメッセージ（PERMISSION_DENIED）だけでは区別できない。
            // 失敗した瞬間のサーバー上のuidと現在の認証uidを直接比較し、結果を追加でログに残す。
            database.ref('players/' + playerId + '/uid').once('value').then(function(snap){
                const serverUid = snap.val();
                const myUid = getFreshestAuthUid();
                if (serverUid && myUid && serverUid === myUid) {
                    logSyncEvent('syncFail_uidMatched_likelyCapIssue', 'uid一致（UIDのズレではなく増加量上限側の問題の可能性が高い）');
                } else {
                    logSyncEvent('syncFail_uidMismatch', 'server=' + String(serverUid) + ' local=' + String(myUid));
                }
            }).catch(function(){ /* 診断読み取り自体の失敗は無視 */ });
            maybePromptUidReclaim(err);
        });
    } catch(e) {
...
// クラウド同期が「権限エラー」で失敗し続けている場合の自己解決導線。
// 「IDとパスワードで復元」等を一度も使っていなくても、ブラウザ側の事情（プライベートブラウズ扱い・
// ストレージのパーティション分離・アプリ内ブラウザ等）でFirebase認証のUIDが以前と変わってしまい、
// サーバー側に保存済みのuidと一致しなくなるケースがある。これが起きるとそれ以降ずっと
// syncSaveDataToCloudのPERMISSION_DENIEDが続き、これまでは管理者がFirebase Console側で
// 手動リセット（UID紐付けをリセット）するしか直す手段がなかった。
// ここでは本人がパスワードを設定していれば、その場で入力するだけで自己解決できるようにする
// （パスワード未設定の場合はこれまで通り運営への連絡が必要）。
// なお「静かな自己修復（secretKeyによる無言リトライ）」は毎回の失敗ごとに試す。1回失敗＝もう二度と直らない、
// ではなく、認証側の一時的なズレ（再認証イベントの取りこぼし等）で一度失敗しても次の機会に直る可能性があるため。
// 一方、プレイヤーに見える「パスワード入力」ダイアログは何度も出すと煩わしいので、こちらは1セッションにつき1回だけに絞る。
let _uidReclaimPasswordPromptShownThisSession = false;
let _uidReclaimSilentInFlight = false; // 直前の静かな自己修復がまだ進行中の間に、連続失敗で多重発火しないようにするガード
// このセッション内で「静かな自己修復」が成功した回数。UIDのズレが一度直った後も同じ理由で
// また成功扱いになるということは、実はUIDはすでに合っていて（＝直すことは何もしていない）、
// 別の理由（savedata側の増加量上限など）で保存自体が弾かれ続けている可能性が高いサイン。
let _uidReclaimSilentSuccessCount = 0;
// 上記のサインを検知した際、現在のローカル保存内容を「再同期依頼」として退避する処理を
// 1セッションにつき1回だけ行うためのガード（何度も無駄に書き込まないように）。
let _pendingResyncStagedThisSession = false;
function maybePromptUidReclaim(err) {
    if (!playerId || !database) return;
    const msgText = (err && (err.message || err.code)) ? String(err.message || err.code) : String(err || '');
    if (!/permission[_ ]denied/i.test(msgText)) return; // 権限エラー以外（通信不調等）では出さない
    if (_uidReclaimSilentInFlight) return;
    _uidReclaimSilentInFlight = true;
    // まずプレイヤーに何も聞かず、このブラウザに残っているsecretKey（バックアップコード）だけで
    // 静かに自己解決を試みる。ブラウザ側の事情（ストレージのパーティション分離・IndexedDBの自動退避など）で
    // Firebase認証のUIDだけが変わり、localStorage自体は無事というケースがほとんどのため、
    // 本人が何も操作しなくても直ることが多い。プレイヤーに毎回パスワード入力を求める負担をなくすための対応。
    attemptSilentUidReclaimBySecretKey(playerId).then(function(ok) {
        _uidReclaimSilentInFlight = false;
        if (ok) {
            _uidReclaimSilentSuccessCount++;
            logSyncEvent('uidReclaim_silent_success', playerId);
            // 同じセッション内で2回目以降も「成功」扱いになった場合、UIDはすでに合っているのに
            // 保存そのものが別の理由（増加量上限など）で弾かれ続けている可能性が高い。
            // プレイヤーの操作を止めず、現在のローカル保存内容を上限なしの退避先に置いておき、
            // 管理者がadmin.html側で内容を確認したうえで反映できるようにする。
            if (_uidReclaimSilentSuccessCount >= 2 && !_pendingResyncStagedThisSession) {
                _pendingResyncStagedThisSession = true;
                stagePendingResyncRequest();
            }
            syncSaveDataToCloud(); // 直った状態で同期をやり直す
            return;
        }
        logSyncEvent('uidReclaim_silent_failed', playerId);
        if (_uidReclaimPasswordPromptShownThisSession) return; // パスワード入力ダイアログは1セッション1回まで
        _uidReclaimPasswordPromptShownThisSession = true;
        // secretKeyが無い／一致しない場合のみ、従来通りパスワード入力を促す
        setTimeout(function() {
            showCustomConfirm(
                '⚠️ クラウド同期に失敗しています',
                '<div style="font-size:12px;color:#420000;line-height:1.6;">このブラウザでの進行状況の保存がうまくいっていない可能性があります。<br>以前パスワードを設定したことがあれば、入力すると自己解決できます。</div>'
                + '<input type="text" id="uidReclaimPwInput" placeholder="パスワード" style="width:100%;margin-top:8px;padding:10px;border:1px solid #b88742;border-radius:4px;font-size:16px;box-sizing:border-box;background:#f5e9c8;color:#420000;">',
                function() {
                    const inp = document.getElementById('uidReclaimPwInput');
                    const pw = inp ? inp.value.trim() : '';
                    if (!pw) { showCustomAlert('ℹ️ お知らせ', 'パスワード未設定の場合は運営までご連絡ください。'); return; }
                    attemptUidReclaimByPassword(playerId, pw);
                },
                null,
                '再連携する'
            );
...
// このブラウザに残っているsecretKey（アカウント作成時に発行されたバックアップコード）を使い、
// プレイヤーに何も聞かずUID再紐付けを試みる。secretKeyが無い、またはサーバー側と一致しない場合はfalseを返すだけで、
// エラー表示は一切出さない（呼び出し元がパスワード入力プロンプトへフォールバックする）。
// 書き込み直前の「今まさにサーバーが見るauth.uid」を可能な限り正確に取得する。
// currentUid（onAuthStateChangedで更新されるキャッシュ）はイベント配信の遅延がゼロではないため、
// SDKが持つ firebase.auth().currentUser を優先的に見て、無ければcurrentUidにフォールバックする。
function getFreshestAuthUid() {
    try {
...
function attemptSilentUidReclaimBySecretKey(targetId) {
    if (!database || !targetId || !playerSecretKey) return Promise.resolve(false);
    return ensureFirebaseAuth().then(function(){
        const uid = getFreshestAuthUid();
        if (!uid) return false;
        const updates = {};
        updates['players/' + targetId + '/uid'] = uid;
        updates['playerSecrets/' + targetId + '/_reclaimGuessKey'] = playerSecretKey;
        return database.ref().update(updates).then(function(){ return true; }).catch(function(){ return false; });
    }).catch(function(){ return false; });
}

function attemptUidReclaimByPassword(targetId, guessedPw) {
    if (!database || !targetId) return;
    ensureFirebaseAuth().then(function(){
        const uid = getFreshestAuthUid();
        if (!uid) { showCustomAlert('❌ エラー', '認証に失敗しました。時間をおいて再度お試しください。'); return; }
        const updates = {};
        updates['players/' + targetId + '/uid'] = uid;
        updates['playerSecrets/' + targetId + '/_reclaimGuess'] = guessedPw;
        database.ref().update(updates).then(function(){
            showCustomAlert('✅ 再連携完了', 'クラウド同期を再開しました。');
            syncSaveDataToCloud();
        }).catch(function(){
            showCustomAlert('❌ エラー', 'パスワードが正しくないか、パスワードが未設定です。運営までご連絡ください。');
        });
    });
}

// クラウド同期が長期間失敗し続けていた場合、UIDが直った後もsavedata側の増加量上限
// （不正対策）によって通常の保存が弾かれ続けることがある。その際に現在のローカル保存内容を
// そのまま「再同期依頼」として上限なしの専用ノードへ退避しておき、後で管理者がadmin.html側で
// 内容を確認したうえで、管理者権限（上限チェック免除）でplayers/<id>/savedataへ反映できるようにする。
// プレイヤー本人には何も表示せず、裏側で静かに退避するだけ（プレイの妨げにならないように）。
function stagePendingResyncRequest() {
    if (!database || !playerId) return;
    try {
        const data = buildSaveDataObject();
        data.stagedAt = Date.now();
        data.stagedReason = 'repeated_write_failed_after_uid_reclaim';
        database.ref('resyncRequests/' + playerId).set(data).then(function(){
            logSyncEvent('resyncRequest_staged', playerId);
        }).catch(function(err2){
            logSyncEvent('resyncRequest_stage_failed', err2 && err2.message ? err2.message : String(err2));
        });
    } catch(e) {
        logSyncEvent('resyncRequest_stage_exception', e && e.message ? e.message : String(e));
    }
}
...
function buildAnnouncementPostOpts(a, announceId) {
    ...
    return {
        ...
        rewardIcon: a.rewardIcon || null,
        rewardFoodSample: a.rewardFoodSample || 0,
        rewardItems: a.rewardItems || []
    };
}

// 管理ツールからの「誤送信・不具合のあった個別手紙の取り消し」を検知する。
// 件名が一致する手紙のうち、exceptAnnounceId（修正版として新しく送った手紙のID）以外を削除する。
// タイムスタンプ比較ではなくID指定にすることで、端末間の時刻ズレや処理順序に関わらず、
// 修正版の新しい手紙を誤って消してしまうことがないようにしている。
function checkMailRevokeQueue() {
    if(!database || !playerId) return;
    const ref = database.ref('mailRevokeQueue/' + playerId);
    ref.on('child_added', function(childSnap){
        const r = childSnap.val();
        if(r && r.subject) {
            const before = postMessages.length;
            postMessages = postMessages.filter(m => !(m.subject === r.subject && m.announceId !== r.exceptAnnounceId));
            const removedCount = before - postMessages.length;
            logSyncEvent('mailRevokeProcessed', 'subject=' + r.subject + ' exceptId=' + r.exceptAnnounceId + ' removed=' + removedCount + ' before=' + before);
            if(postMessages.length !== before) {
                savePostMessages();
                updatePostButtonUI();
                const listOverlay = document.getElementById('postListOverlay');
        ...
        ref.child(childSnap.key).remove().catch(function(e){
            logSyncEvent('mailRevokeQueue_remove_failed', e && e.message ? e.message : String(e));
        });
    });
}
...
function initPlayerIdAfterAuth(onReady) {
    if (playerId) {
        ...
        if (database) {
            database.ref('players/' + playerId).once('value').then(function(snap) {
                if (!snap.exists()) {
                    // secretKey・passwordは公開読み取りされない別ノード（playerSecrets）に保存する
                    database.ref('players/' + playerId).set({ name: playerName, createdAt: Date.now(), uid: currentUid });
                    database.ref('playerSecrets/' + playerId + '/secretKey').set(playerSecretKey).catch(function(){});
                } else {
            ...
            checkMailRevokeQueue(); // 管理ツールからの個別手紙の取り消し（誤送信・不具合の修正用）を確認
    ...
    if (!database) {
        ...
        safeLSSet('qr_player_id', playerId);
        updatePlayerIdDisplay();
        if(onReady) onReady();
        return;
    }
    ...
    function tryAssign() {
        ...
        // ポータルサイトへのiframe埋め込み等でCSPがFirebaseへの通信をブロックしていると、
        // once('value')がresolveもrejectもせずそのまま固まり「発行中...」から進めなくなることがある。
        // 一定時間で諦めてローカルのみでID発行し、ゲーム進行自体は止めないようにする。
        const assignTimeout = setTimeout(function() {
            if (settledLocally) return;
            settledLocally = true;
            if (typeof logSyncEvent === 'function') logSyncEvent('playerIdAssignTimeout', 'candidate=' + candidate);
            playerId = candidate;
            safeLSSet('qr_player_id', playerId);
            updatePlayerIdDisplay();
            if(onReady) onReady();
        ...
        database.ref('players/' + candidate).once('value').then(function(snap) {
            if (settledLocally) return; // タイムアウトで既に確定済みなら何もしない
            if (snap.exists()) {
                clearTimeout(assignTimeout);
                tryAssign(); // 重複したら再試行（極めて低確率）
            } else {
                database.ref('players/' + candidate).set({ name: playerName, createdAt: Date.now(), uid: currentUid }).then(function() {
                    if (settledLocally) return;
                    clearTimeout(assignTimeout);
                    settledLocally = true;
                    playerId = candidate;
                    ...
                    database.ref('globalAnnouncements').once('value').then(function(snap2){
                        ...
                        if(items) {
                            ...
                            if(keys.length > 0) safeLSSet('qr_last_global_announce', keys[keys.length - 1]);
                        }
                        if(onReady) onReady();
                    ...
                    clearTimeout(assignTimeout);
                    settledLocally = true;
                    // Firebase書き込み失敗時はローカルのみでID発行（auth未確立によるpermission_deniedが多い想定）
                    if (typeof logSyncEvent === 'function') logSyncEvent('playerCreateWriteFailed', 'candidate=' + candidate + ' uid=' + currentUid + ' err=' + (e && e.message ? e.message : String(e)));
                    playerId = candidate;
                    safeLSSet('qr_player_id', playerId);
                    updatePlayerIdDisplay();
                    if(onReady) onReady();
            ...
            if (typeof logSyncEvent === 'function') logSyncEvent('playerIdReadFailed', 'candidate=' + candidate + ' err=' + (e && e.message ? e.message : String(e)));
            playerId = candidate;
            safeLSSet('qr_player_id', playerId);
            updatePlayerIdDisplay();
            if(onReady) onReady();
...
function loadGame() {
    ...
    const savedFoodSample = localStorage.getItem('qr_curry_foodsample');
    ...
    if (savedFoodSample !== null) foodSampleCount = parseInt(savedFoodSample, 10);
    ...
    const savedFestAllyBase = localStorage.getItem('qr_fest_ally_selected_base');
    const savedFestAllyTableware = localStorage.getItem('qr_fest_ally_selected_tableware');
    if (savedFestAllyBase) festAllySelectedBase = savedFestAllyBase;
    if (savedFestAllyTableware) festAllySelectedTableware = savedFestAllyTableware;
    dailyCurryLastWinDate = localStorage.getItem('qr_daily_curry_last_win_date') || '';
    ...
    unlockTurmericRiceBase = localStorage.getItem('qr_unlock_turmeric_rice_base') === '1';
    unlockFestSkip20 = localStorage.getItem('qr_unlock_fest_skip_20') === '1';
    unlockFestSkip30 = localStorage.getItem('qr_unlock_fest_skip_30') === '1';
    festSampleClaimed20 = localStorage.getItem('qr_fest_sample_claimed_20') === '1';
    festSampleClaimed30 = localStorage.getItem('qr_fest_sample_claimed_30') === '1';
    festSampleClaimed40 = localStorage.getItem('qr_fest_sample_claimed_40') === '1';
    festSampleClaimed50 = localStorage.getItem('qr_fest_sample_claimed_50') === '1';
...
function addPostMessage(opts) {
    postMessages.push({
        ...
        rewardIcon: opts.rewardIcon || null,
        rewardFoodSample: opts.rewardFoodSample || 0,
...
function onGameStartShowPost() {
    ...
    const qrShortcutBtn = document.getElementById('qrShortcutBtn');
    if(qrShortcutBtn) qrShortcutBtn.style.display = 'flex';
    updatePostButtonUI();
    const unread = getUnreadPostCount();
    if(unread > 0) {
...
function renderPostList() {
    ...
    const hasAttachment = m => !m.rewardClaimed && ((m.rewardItems && m.rewardItems.length > 0) || m.rewardGold > 0 || m.rewardPack > 0 || m.rewardSpicyCoin > 0 || m.rewardFoodSample > 0 || !!m.rewardIcon);
...
function renderPostDetailRewardArea(m, idx) {
    ...
    const hasReward = m.rewardPack || m.rewardGold || m.rewardSpicyCoin || m.rewardIcon || m.rewardFoodSample || (m.rewardItems && m.rewardItems.length > 0);
    ...
    if(m.rewardFoodSample) parts.push(`🍥食品サンプル${m.rewardFoodSample}個`);
    if(m.rewardIcon) parts.push(`<img src="${m.rewardIcon}" style="width:1.2em;height:1.2em;vertical-align:middle;border-radius:50%;object-fit:cover;">特別アイコン`);
...
function claimPostReward(idx) {
    ...
    if(m.rewardFoodSample) {
        foodSampleCount = (foodSampleCount || 0) + m.rewardFoodSample;
        const foodSampleElMail = document.getElementById('globalFoodSample'); if(foodSampleElMail) foodSampleElMail.innerText = foodSampleCount;
        receivedParts.push(`🍥食品サンプル${m.rewardFoodSample}個`);
    }
    if(m.rewardItems && m.rewardItems.length > 0) {
    ...
    let iconNewlyUnlocked = false;
    if(m.rewardIcon) {
        iconNewlyUnlocked = unlockIcon(m.rewardIcon);
        receivedParts.push(`特別アイコン`);
    }
    ...
    if(m.rewardIcon && iconNewlyUnlocked) {
        setTimeout(() => showIconGetOverlay(m.rewardIcon, m.subject || '特別報酬'), 500);
    }
}
...
const ACHIEVEMENTS = [
    ...
    { id:'kaitate_c',    cat:'curry', text:'盾でガードだ！',        desc:'貝の盾カレー完成おめでとう！',                 reward:100, check: d => !!d.gotKaiTate },
    { id:'tricaviar_c', cat:'curry', text:'珍味の王様！',            desc:'世界三大珍味カレー完成おめでとう！',           reward:100, check: d => !!d.gotTriCaviar },
    { id:'kyoyasai_c',  cat:'curry', text:'京の心、おもてなし',      desc:'京野菜カレー完成おめでとう！',                 reward:100, check: d => !!d.gotKyoyasai },
    { id:'cheese_c',    cat:'curry', text:'チーズにまみれたい',      desc:'チーズまみれカレー完成おめでとう！',           reward:100, check: d => !!d.gotCheeseCurry },
    { id:'shrimp_c',    cat:'curry', text:'えびちゃん',              desc:'小えびスピードカレー完成おめでとう！',         reward:100, check: d => !!d.gotShrimpCurry },
    { id:'banana_c',    cat:'curry', text:'２つの意味ですべるカレー', desc:'そんなバナナカレー完成おめでとう！',           reward:100, check: d => !!d.gotBananaCurry },
...
function getStats() {
    ...
    return s ? JSON.parse(s) : {
        ...
        gotTonTonTon:false, gotPoisonApple:false, gotFluffyOmelette:false, gotGreenCurry:false, gotTriCaviar:false, gotKaiTate:false, gotKyoyasai:false, gotCheeseCurry:false, gotShrimpCurry:false, gotBananaCurry:false,
        gotDraw:false, gotDoubleBingo:false, gotTripleBingo:false,
        // バトルカレーフェス：参加回数・自己ベスト連勝数（フェスをまたいで永続）
        festJoinCount: 0, festBestStreak: 0, festWeeklyBestStreak: 0, festWeeklyBestStreakWeekId: '',
...
function onCookDone(curry) {
    updateStats(s => {
        ...
        if(curry.isKaiTate) s.gotKaiTate = true;
        if(curry.isKyoyasai) s.gotKyoyasai = true;
        if(curry.isCheeseCurry) s.gotCheeseCurry = true;
        if(curry.isShrimpCurry) s.gotShrimpCurry = true;
        if(curry.isBananaCurry) s.gotBananaCurry = true;
        if(curry.curryType) {
    ...
    const isSpecial = curry.isPoison||curry.isIllusion||curry.isSeed||curry.isMargherita||curry.isSeafood||curry.isTonTonTon||curry.isSticky||curry.isWanpaku||curry.isRatatouille||curry.isHomerun||curry.isFluffyOmelette||curry.isGreenCurry||curry.isTriCaviar||curry.isKaiTate||curry.isKyoyasai||curry.isCheeseCurry||curry.isShrimpCurry||curry.isBananaCurry;
    ...
    if(curry.isKaiTate) incrementGlobalStat('cook/kaitate');
    if(curry.isKyoyasai) incrementGlobalStat('cook/kyoyasai');
    if(curry.isCheeseCurry) incrementGlobalStat('cook/cheese');
    if(curry.isShrimpCurry) incrementGlobalStat('cook/shrimp');
    if(curry.isBananaCurry) incrementGlobalStat('cook/banana');
...
function buildSaveDataObject() {
    ...
    return {
        inventory, scanHistory: safeScanHistory, playerG, playerEXP, packTicket, spicyCoin, foodSampleCount,
        ...
        unlockGenmaiBase, unlockSaffronRiceBase, unlockJasmineRiceBase, unlockNanBase, unlockTurmericRiceBase,
        unlockFestSkip20, unlockFestSkip30,
        festSampleClaimed20, festSampleClaimed30, festSampleClaimed40, festSampleClaimed50,
        ...
        // 咖喱図書館（story.js側で管理）：本棚の解放状況・想いの欠片・読了/撃破履歴。
        // story.js側は今まで通りlocalStorageへ同期的に読み書きするだけで、ここで他の永続データと
        // 同じ仕組みに乗せることでクラウド保存（players/<id>/savedata）にも含める。
        storyUnlocked: localStorage.getItem('qr_story_library_unlocked') || '0',
        storyKakera: localStorage.getItem('qr_story_kakera_count') || '0',
        storyKakeraOnboard: localStorage.getItem('qr_story_kakera_onboard_stage') || '0',
        storyKakeraInfoSeen: localStorage.getItem('qr_story_kakera_info_seen') || '0',
        storyUnlockedChapters: localStorage.getItem('qr_story_unlocked_chapters') || '[]',
        storyReadChapters: localStorage.getItem('qr_story_read_chapters') || '[]',
        storyClearedChapters: localStorage.getItem('qr_story_cleared_chapters') || '[]',
        storyBookBossIntroSeen: localStorage.getItem('qr_story_book_boss_intro_seen') || '0',
        playerId: playerId || '',
...
function applySaveDataObject(decoded) {
    ...
    foodSampleCount = decoded.foodSampleCount || 0;
    playerName = decoded.playerName || '名無しの料理人';
    // 復元操作直後は復元前の名前がヘッダー・入力欄に残ったままになり、本人から見て
    // 「本当に復元できたのか」が分かりにくいという声があったため、ここで即座にUIへ反映する。
    const _restoredHeaderNameEl = document.getElementById('headerPlayerName');
    if(_restoredHeaderNameEl) _restoredHeaderNameEl.innerText = playerName;
    const _restoredNameInputEl = document.getElementById('playerNameInput');
    if(_restoredNameInputEl) _restoredNameInputEl.value = playerName;
    ...
    festAllySelectedBase = decoded.festAllySelectedBase || '白米';
    ...
    unlockTurmericRiceBase = !!decoded.unlockTurmericRiceBase;
    ...
    unlockFestSkip20 = !!decoded.unlockFestSkip20;
    unlockFestSkip30 = !!decoded.unlockFestSkip30;
    festSampleClaimed20 = !!decoded.festSampleClaimed20;
    festSampleClaimed30 = !!decoded.festSampleClaimed30;
    festSampleClaimed40 = !!decoded.festSampleClaimed40;
    festSampleClaimed50 = !!decoded.festSampleClaimed50;
    ...
    // 咖喱図書館（story.js側で管理）：復元時も他の永続データと同じ扱いでlocalStorageへ書き戻す。
    // story.js側はこれらのキーを今まで通り同期的に読み書きするだけで、変更は不要。
    if(decoded.storyUnlocked !== undefined) localStorage.setItem('qr_story_library_unlocked', decoded.storyUnlocked);
    if(decoded.storyKakera !== undefined) localStorage.setItem('qr_story_kakera_count', decoded.storyKakera);
    if(decoded.storyKakeraOnboard !== undefined) localStorage.setItem('qr_story_kakera_onboard_stage', decoded.storyKakeraOnboard);
    if(decoded.storyKakeraInfoSeen !== undefined) localStorage.setItem('qr_story_kakera_info_seen', decoded.storyKakeraInfoSeen);
    if(decoded.storyUnlockedChapters !== undefined) localStorage.setItem('qr_story_unlocked_chapters', decoded.storyUnlockedChapters);
    if(decoded.storyReadChapters !== undefined) localStorage.setItem('qr_story_read_chapters', decoded.storyReadChapters);
    if(decoded.storyClearedChapters !== undefined) localStorage.setItem('qr_story_cleared_chapters', decoded.storyClearedChapters);
    if(decoded.storyBookBossIntroSeen !== undefined) localStorage.setItem('qr_story_book_boss_intro_seen', decoded.storyBookBossIntroSeen);
    localStorage.setItem('selectedPlayerIcon', currentIconFile);
    migrateLegacyExpCap();
}
...
function showPasswordSetup() {
    ...
    const msg = '<div style="font-size:13px;color:#420000;margin-bottom:10px;line-height:1.7;">'
        ...
        + '</div>'
        + '<div style="font-size:11px;color:#7a5c1e;margin-bottom:6px;">※半角英数字・記号のみで設定してください（全角・日本語は使用不可）</div>'
        + '<input type="text" id="pwSetupInput" placeholder="パスワードを入力（半角英数字）" maxlength="20" style="width:100%;padding:10px;border:1px solid #b88742;border-radius:4px;font-size:16px;box-sizing:border-box;background:#f5e9c8;color:#420000;">';
    showCustomConfirm('🔒 パスワード設定', msg, function() {
        const inp = document.getElementById('pwSetupInput');
        // Unicode正規化(NFC)をかけておく。濁点付きカナ等はNFC/NFD(合成済み/分解形)で
        // 見た目が完全に同じでも内部のコードポイント列が異なる場合があり、正規化しないまま
        // サーバー側で文字列比較すると、目視では区別できない不一致でパスワード復元が失敗しうるため。
        const pw = inp ? inp.value.trim().normalize('NFC') : '';
        if(!pw) { showCustomAlert('❌ エラー', 'パスワードを入力してください。'); return; }
        // 半角英数字・記号のみを新規設定分から強制する（全角・日本語はNFC/NFD表記ゆれで
        // 復元時に不可解な不一致を起こしうるため）。既存プレイヤーが既に全角等で
        // 設定済みのパスワードには一切影響しない（比較・保存のロジック自体は変更していない）。
        if(!/^[\x21-\x7E]+$/.test(pw)) {
            showCustomAlert('❌ エラー', 'パスワードは半角英数字・記号のみで設定してください。');
            return;
        }
        if(!database) { showCustomAlert('❌ エラー', '通信に問題があり、パスワードを保存できませんでした。'); return; }
        database.ref('playerSecrets/' + playerId + '/password').set(pw).then(function(){
...
// ===== 現在の認証UID表示（サポート用デバッグ機能） =====
// Firebase Authenticationの匿名ユーザー一覧から「最新のもの」を目視で推測して手動紐付けすると、
// 同時期に他プレイヤーの匿名サインインが発生している場合に誤ったUIDを選んでしまう恐れがある。
// この端末・ブラウザが実際に今使っているUIDを直接表示することで、運営が確実に正しい値で
// players/{id}/uid を手動修復できるようにする。
function showCurrentAuthUidDebug() {
    const uid = getFreshestAuthUid();
    if(!uid) { showCustomAlert('❌ エラー', '認証情報を取得できませんでした。時間をおいて再度お試しください。'); return; }
    const msg = '<div style="font-size:12px;color:#420000;margin-bottom:10px;line-height:1.6;">このデバイス・ブラウザが現在使っている認証UIDです。<br>データ復元がうまくいかない場合、この値を運営に伝えてください。</div>'
        + `<textarea id="currentUidDebugArea" style="width:100%;height:50px;font-size:14px;padding:8px;border:1px solid #b88742;border-radius:4px;box-sizing:border-box;background:#f5e9c8;color:#420000;resize:none;" readonly>${uid}</textarea>`
        + `<button onclick="copyCurrentUidDebug()" style="width:100%;margin-top:8px;padding:8px;background:#b88742;color:#efdeb1;border:none;border-radius:4px;font-weight:bold;cursor:pointer;">📋 コピーする</button>`;
    showCustomAlert('🆔 現在の認証UID', msg);
}

function copyCurrentUidDebug() {
    const ta = document.getElementById('currentUidDebugArea');
    if(!ta) return;
    ta.select(); ta.setSelectionRange(0, 99999);
    navigator.clipboard.writeText(ta.value).then(() => {
        showCustomAlert('✅ コピー完了', 'UIDをコピーしました！');
    }).catch(() => {
...
function showIdPasswordRestore() {
    ...
    showCustomConfirm('🔑 IDとパスワードで復元', msg, function() {
        ...
        // パスワード設定側と同じくNFC正規化してから比較用に送る（濁点付きカナ等のNFC/NFD不一致対策）。
        const inputPw = pwInput ? pwInput.value.trim().normalize('NFC') : '';
        ...
        // パスワードの正誤はクライアント側では判定しない（players以下からpasswordを直接読める
        // 状態は「誰でも他人のパスワードを読める」ことを意味するため廃止した）。
        // 代わりに「uidの再紐付け＋パスワードの自己申告」を同一の書き込みリクエストとして送り、
        // サーバー側のルールが申告値と保存値の一致を照合する。一致すれば書き込みが成功し、
        // その成功自体が「パスワード一致＝本人確認OK」の証明になる（値そのものはクライアントに返らない）。
        ensureFirebaseAuth().then(function(){
            const uidForReclaim = getFreshestAuthUid();
            if(!uidForReclaim) { showCustomAlert('❌ エラー', '認証に失敗しました。時間をおいて再度お試しください。'); return; }
            database.ref('players/' + inputId).once('value').then(function(snap){
                if(!snap.exists()) { showCustomAlert('❌ エラー', 'そのIDのプレイヤーは見つかりません。'); return; }
                const updates = {};
                updates['players/' + inputId + '/uid'] = uidForReclaim;
                updates['playerSecrets/' + inputId + '/_reclaimGuess'] = inputPw;
                database.ref().update(updates).then(function(){
                    // 再紐付け成功＝パスワード一致。続けてsavedataを取得して復元
                    database.ref('players/' + inputId + '/savedata').once('value').then(function(sdSnap){
                        ...
                        setupLoadedIconUI(); updateLvDisplay(); initQuest();
                        ensureDeliveryCurrySynced();
                        // このIDのsecretKeyもこの端末に反映しておく（今後の再紐付け・バックアップコード用）
                        database.ref('playerSecrets/' + inputId + '/secretKey').once('value').then(function(skSnap){
                            const sk = skSnap.val();
                            if(sk) { playerSecretKey = sk; localStorage.setItem('qr_secret_key', playerSecretKey); }
                        }).catch(function(){});
                        showCustomAlert('✅ 復元完了', 'IDとパスワードでデータを復元しました！');
                    });
                }).catch(function(e){
                    logSyncEvent('idPasswordRestore_write_failed', (e && (e.message || e.code)) ? String(e.message || e.code) : String(e));
                    showCustomAlert('❌ エラー', 'パスワードが正しくないか、このIDにはパスワードが設定されていません。バックアップコードでの復元をお試しください。');
                });
            }).catch(function(e){
                logSyncEvent('idPasswordRestore_read_failed', (e && (e.message || e.code)) ? String(e.message || e.code) : String(e));
...
function performBackupRestore(decoded) {
    ...
    if(decoded.playerId && decoded.secretKey) {
        ...
        localStorage.setItem('qr_secret_key', playerSecretKey);
        updatePlayerIdDisplay();
        // バックアップコードに含まれるsecretKey（本人しか持ち得ないランダム値）を「合言葉」として
        // uidの再紐付けも同時に行う。以前はここでnameだけ書き込んでuidの再紐付けをしていなかったため、
        // 別端末でバックアップコード復元した場合に以後ずっとクラウド同期が失敗し続ける不具合があった。
        if(database) {
            ensureFirebaseAuth().then(function(){
                const uidForReclaim = getFreshestAuthUid();
                if(!uidForReclaim) return;
                const updates = {};
                updates['players/' + playerId + '/uid'] = uidForReclaim;
                updates['players/' + playerId + '/name'] = playerName;
                updates['playerSecrets/' + playerId + '/_reclaimGuessKey'] = playerSecretKey;
                database.ref().update(updates).catch(function(e){
                    logSyncEvent('performBackupRestore_uid_reclaim_failed', e && e.message ? e.message : String(e));
                });
            });
        }
    }
...
// ===== カレー店情報 =====
// ★店舗は新しい順（上が最新）に並べる。今後追加する店舗は配列の先頭に足していくこと。
const CURRY_SHOPS = [
    {
        id: 'shop10',
        name: 'AKARI CURRY － Gallery & Bar －',
        location: '大阪 桜ノ宮',
        photos: ['shop/shop-10-1.png', 'shop/shop-10-2.png'],
        desc: 'JR桜ノ宮駅から徒歩1分\nさらさら系欧風カレーのお店です🍛\nお昼はカレー夜は小皿とお酒、日常にAKARIをテーマに日々頑張ります🍛',
        links: [{ label: 'Instagram', url: 'https://www.instagram.com/akaricurry/' }]
    },
    {
        id: 'shop9',
        name: 'さぼてん食堂',
        location: '大阪 肥後橋',
        photos: ['shop/shop-9-1.png', 'shop/shop-9-2.png'],
        desc: '肥後橋駅8番出口から徒歩5分弱\n欧風キーマカレー 専門店\nぴあ究極のカレー殿堂入り/Retty人気店/食べログ百名店（全国版）選出',
        links: [
            { label: 'Instagram', url: 'https://www.instagram.com/saboten.osaka/' },
            { label: 'X', url: 'https://x.com/sabo10_osaka' }
        ]
    },
    {
        id: 'shop8',
        name: '創作カレー ツキノワ',
        location: '大阪 堺筋本町',
        photos: ['shop/shop-8-1.png', 'shop/shop-8-2.png'],
        desc: '大阪メトロ「堺筋本町駅」の3番出口から徒歩5分\n香り弾ける鮮烈なスパイス感溢れる\n和風出汁ベースの創作系カレー',
        links: [
            { label: 'Instagram', url: 'https://www.instagram.com/tsukinowa_curry/' },
            { label: 'X', url: 'https://x.com/tsukinowa_curry' }
        ]
    },
    {
        id: 'shop7',
        name: 'グーグー藤カレー',
        location: '大阪 高槻',
        photos: ['shop/shop-7-1.png', 'shop/shop-7-2.png'],
        desc: 'JR高槻から徒歩2分\nお腹も心も満たすカレーを😁\n化学調味料、小麦粉、白砂糖不使用👍\n天日塩、インドから空輸のスパイス、バスマティ米',
        links: [{ label: 'Instagram', url: 'https://www.instagram.com/fujicurry.takatsuki/' }]
    },
    {
        id: 'shop6',
        name: '間借りde咖喱屋PARAMAX',
        location: '大阪 正雀',
        photos: ['shop/shop-6-1.png', 'shop/shop-6-2.png'],
        desc: '月に一度だけ現れる間借り欧風スパイス咖喱屋\n18種のスパイスを研究し続ける店主の一皿\n無添加・グルテンフリーで生まれる深いコク',
        links: [{ label: 'Instagram', url: 'https://www.instagram.com/paramax_curry/' }]
    },
    {
        id: 'shop5',
        name: 'スパイスのこうちゃん',
        location: '兵庫県 塚口',
        photos: ['shop/shop-5-1.png', 'shop/shop-5-2.png'],
        desc: 'クローブの匂いが一番好きです。\n多彩なスパイスを混ぜたカレー。\n美味しく完成したらそれは奇跡。',
        links: [
            { label: 'X', url: 'https://x.com/kirakuniikouya1' },
            { label: 'Instagram', url: 'https://www.instagram.com/spice_no_kouchan/' }
        ]
    },
    {
        id: 'shop4',
        name: 'HeavEN 24/7 東三国店',
        location: '大阪 東三国',
        photos: ['shop/shop-4-1.png', 'shop/shop-4-2.png'],
        desc: '東三国駅から徒歩約7～8分\n24の恵みと７つの魔法 羽根が導く天国カレー\n昼はカレーとクラフト🍻\n夜はクラフト🍻とおばんざい\n〆はカレーうどん',
        links: [{ label: 'Instagram', url: 'https://www.instagram.com/heaven.higashimikuni/' }]
    },
    {
        id: 'shop3',
        name: 'らんらんルー',
...
const SHOP_ITEMS = [
    ...
    },
    {
        id: 'fest_skip_20',
        name: 'フェス戦即決(20戦目まで)',
        desc: 'カレーフェスの戦闘20戦目まで限定で、戦闘過程をスキップしてすぐに勝敗結果を見れるようになります。',
        priceType: 'coin', price: 1,
        oneTimeOnly: true, festSkipUnlock: 20
    },
    {
        id: 'fest_skip_30',
        name: 'フェス戦即決(30戦目まで)',
        desc: 'カレーフェスの戦闘30戦目まで限定で、戦闘過程をスキップしてすぐに勝敗結果を見れるようになります。\n※「フェス戦即決(20戦目まで)」を購入済みでないと購入できません。',
        priceType: 'coin', price: 1,
        oneTimeOnly: true, festSkipUnlock: 30
    },
    {
        id: 'food_sample_10',
        name: '食品サンプル10個',
        desc: 'ストックカレーの「食品サンプル設定」で使用するアイテム。設定したカレーが対戦でHP0になり敗北した場合、代わりに1個消費してストックからの消滅を防ぎます。',
        priceType: 'coin', price: 1,
        oneTimeOnly: false, foodSampleGrant: 10
    }
];
...
function isShopItemPurchased(item) {
    ...
    if(item.festSkipUnlock === 20) return unlockFestSkip20;
    if(item.festSkipUnlock === 30) return unlockFestSkip30;
    return false;
}
...
function confirmShopPurchase() {
    ...
    // フェス戦即決(30戦目まで)専用：フェス戦即決(20戦目まで)を先に購入していないと購入不可
    if(item.id === 'fest_skip_30' && !unlockFestSkip20) {
        showCustomAlert('⚠️ 購入できません', '「フェス戦即決(20戦目まで)」を先に購入してください。');
        return;
    }
    // 購入処理
    ...
    } else if(item.oneTimeOnly && item.festSkipUnlock === 20) {
        unlockFestSkip20 = true;
    } else if(item.oneTimeOnly && item.festSkipUnlock === 30) {
        unlockFestSkip30 = true;
    } else if(item.foodSampleGrant) {
        foodSampleCount = (foodSampleCount || 0) + item.foodSampleGrant;
        const foodSampleElShop = document.getElementById('globalFoodSample'); if(foodSampleElShop) foodSampleElShop.innerText = foodSampleCount;
    }
    saveGame();
    if(item.id === 'bingo_maru') {
...
function openShop1Link() {
    ...
    if(pendingShopQR) {
        ...
        else if(pq.shopId === 'shop4') rewardMsg = giveShop4Reward(pq.text);
        else if(pq.shopId === 'shop5') rewardMsg = giveShop5Reward(pq.text);
        else if(pq.shopId === 'shop6') rewardMsg = giveShop6Reward(pq.text);
        else if(pq.shopId === 'shop7') rewardMsg = giveShop7Reward(pq.text);
        else if(pq.shopId === 'shop8') rewardMsg = giveShop8Reward(pq.text);
        else if(pq.shopId === 'shop9') rewardMsg = giveShop9Reward(pq.text);
        else if(pq.shopId === 'shop10') rewardMsg = giveShop10Reward(pq.text);
    }
...
function processShop2QR(text, matchedKeyword) {
    try {
        // 修正：SNSリンクURLとpendingShopQRの設定がshop1/shop3側と異なり漏れていたため、ここで追加している
        // （これが無いと「SNSを開く」を押してもリンクが開かず、giveShop2Rewardの報酬付与も動かない）
        currentShop1LinkUrl = SHOP2_KEYWORD_URLS[matchedKeyword] || '';
        pendingShopQR = { shopId: 'shop2', text };
        document.getElementById("getBoxTitle").innerText = "店舗情報";
        document.getElementById("bonusBadge").style.display = "none";
        document.getElementById("getItemEmoji").style.display = "none";
        document.getElementById("getItemName").style.display = "none";
        ...
        document.getElementById("shopQrLocation").innerText = '大阪 堺筋本町';
...
// 実店舗「HeavEN 24/7 東三国店」専用QR処理
const SHOP4_KEYWORD_URLS = {
    "heaven.higashimikuni": "https://www.instagram.com/heaven.higashimikuni/"
};
function giveShop4Reward(text) {
    const isFirstTime = localStorage.getItem('qr_shop4_scanned') !== '1';
    const today = getJSTDateString();
    const isFirstToday = scanHistory[text] !== today;
    scanHistory[text] = today;
    const rewardArea = document.getElementById("shopQrRewardArea");
    let rewardMsg = null;
    if(isFirstTime) {
        localStorage.setItem('qr_shop4_scanned', '1');
        packTicket = (packTicket || 0) + 1;
        rewardMsg = '🎫 パック券 1枚 GET！';
        if(rewardArea) rewardArea.innerHTML = `<div style="font-size:14px; font-weight:bold; color:#e67e22;">${rewardMsg}</div>`;
        const pkEl = document.getElementById('globalPackTicket'); if(pkEl) pkEl.innerText = packTicket;
...
function processShop4QR(text, matchedKeyword) {
    try {
        currentShop1LinkUrl = SHOP4_KEYWORD_URLS[matchedKeyword] || '';
        pendingShopQR = { shopId: 'shop4', text };
        document.getElementById("getBoxTitle").innerText = "店舗情報";
        document.getElementById("bonusBadge").style.display = "none";
        document.getElementById("getItemEmoji").style.display = "none";
        document.getElementById("getItemName").style.display = "none";
        document.getElementById("shopQrArea").style.display = "block";
        document.getElementById("shopQrShopName").innerText = 'HeavEN 24/7 東三国店';
        document.getElementById("shopQrLocation").innerText = '大阪 東三国';
        document.getElementById("shopQrPhotos").innerHTML = '<img src="shop/shop-4-1.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;"><img src="shop/shop-4-2.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;">';
        document.getElementById("shopQrDesc").innerHTML = '東三国駅から徒歩約7～8分<br>24の恵みと７つの魔法 羽根が導く天国カレー<br>昼はカレーとクラフト🍻<br>夜はクラフト🍻とおばんざい<br>〆はカレーうどん';
        document.getElementById("shopQrOpenLinkBtn").style.display = 'inline-block';
        const _r4ft = localStorage.getItem('qr_shop4_scanned') !== '1';
        const _r4td = scanHistory[text] !== getJSTDateString();
        document.getElementById("shopQrRewardArea").innerHTML = (_r4ft || _r4td)
            ? `<div style="font-size:12px; color:#9a6e30; font-weight:bold; margin-top:4px;">📱 SNSを開くと報酬GET！</div>` : '';
        document.getElementById("getBox").style.display = "block";
    } catch(e) { console.error('processShop4QR error:', e); }
}
// 実店舗「スパイスのこうちゃん」専用QR処理
const SHOP5_KEYWORD_URLS = {
    "x.com/kirakuniikouya1":         "https://x.com/kirakuniikouya1",
    "instagram.com/spice_no_kouchan": "https://www.instagram.com/spice_no_kouchan/"
};
function giveShop5Reward(text) {
    const isFirstTime = localStorage.getItem('qr_shop5_scanned') !== '1';
    const today = getJSTDateString();
    const isFirstToday = scanHistory[text] !== today;
    scanHistory[text] = today;
    const rewardArea = document.getElementById("shopQrRewardArea");
    let rewardMsg = null;
    if(isFirstTime) {
        localStorage.setItem('qr_shop5_scanned', '1');
        packTicket = (packTicket || 0) + 1;
        rewardMsg = '🎫 パック券 1枚 GET！';
        if(rewardArea) rewardArea.innerHTML = `<div style="font-size:14px; font-weight:bold; color:#e67e22;">${rewardMsg}</div>`;
        const pkEl = document.getElementById('globalPackTicket'); if(pkEl) pkEl.innerText = packTicket;
...
function processShop5QR(text, matchedKeyword) {
    try {
        currentShop1LinkUrl = SHOP5_KEYWORD_URLS[matchedKeyword] || '';
        pendingShopQR = { shopId: 'shop5', text };
        document.getElementById("getBoxTitle").innerText = "店舗情報";
        document.getElementById("bonusBadge").style.display = "none";
        document.getElementById("getItemEmoji").style.display = "none";
        document.getElementById("getItemName").style.display = "none";
        document.getElementById("shopQrArea").style.display = "block";
        document.getElementById("shopQrShopName").innerText = 'スパイスのこうちゃん';
        document.getElementById("shopQrLocation").innerText = '兵庫県 塚口';
        document.getElementById("shopQrPhotos").innerHTML = '<img src="shop/shop-5-1.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;"><img src="shop/shop-5-2.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;">';
        document.getElementById("shopQrDesc").innerHTML = 'クローブの匂いが一番好きです。<br>多彩なスパイスを混ぜたカレー。<br>美味しく完成したらそれは奇跡。<br>SNSのフォローよろしくお願いします！';
        document.getElementById("shopQrOpenLinkBtn").style.display = 'inline-block';
        const _r5ft = localStorage.getItem('qr_shop5_scanned') !== '1';
        const _r5td = scanHistory[text] !== getJSTDateString();
        document.getElementById("shopQrRewardArea").innerHTML = (_r5ft || _r5td)
            ? `<div style="font-size:12px; color:#9a6e30; font-weight:bold; margin-top:4px;">📱 SNSを開くと報酬GET！</div>` : '';
        document.getElementById("getBox").style.display = "block";
    } catch(e) { console.error('processShop5QR error:', e); }
}
// 実店舗「間借りde咖喱屋PARAMAX」専用QR処理
const SHOP6_KEYWORD_URLS = {
    "instagram.com/paramax_curry": "https://www.instagram.com/paramax_curry/"
};
function giveShop6Reward(text) {
    const isFirstTime = localStorage.getItem('qr_shop6_scanned') !== '1';
    const today = getJSTDateString();
    const isFirstToday = scanHistory[text] !== today;
    scanHistory[text] = today;
    const rewardArea = document.getElementById("shopQrRewardArea");
    let rewardMsg = null;
    if(isFirstTime) {
        localStorage.setItem('qr_shop6_scanned', '1');
        packTicket = (packTicket || 0) + 1;
        rewardMsg = '🎫 パック券 1枚 GET！';
        if(rewardArea) rewardArea.innerHTML = `<div style="font-size:14px; font-weight:bold; color:#e67e22;">${rewardMsg}</div>`;
        const pkEl = document.getElementById('globalPackTicket'); if(pkEl) pkEl.innerText = packTicket;
...
function processShop6QR(text, matchedKeyword) {
    try {
        currentShop1LinkUrl = SHOP6_KEYWORD_URLS[matchedKeyword] || '';
        pendingShopQR = { shopId: 'shop6', text };
        document.getElementById("getBoxTitle").innerText = "店舗情報";
        document.getElementById("bonusBadge").style.display = "none";
        document.getElementById("getItemEmoji").style.display = "none";
        document.getElementById("getItemName").style.display = "none";
        document.getElementById("shopQrArea").style.display = "block";
        document.getElementById("shopQrShopName").innerText = '間借りde咖喱屋PARAMAX';
        document.getElementById("shopQrLocation").innerText = '大阪 正雀';
        document.getElementById("shopQrPhotos").innerHTML = '<img src="shop/shop-6-1.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;"><img src="shop/shop-6-2.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;">';
        document.getElementById("shopQrDesc").innerHTML = '月に一度だけ現れる間借り欧風スパイス咖喱屋<br>18種のスパイスを研究し続ける店主の一皿<br>無添加・グルテンフリーで生まれる深いコク<br>SNSのフォローよろしくお願いします！';
        document.getElementById("shopQrOpenLinkBtn").style.display = 'inline-block';
        const _r6ft = localStorage.getItem('qr_shop6_scanned') !== '1';
        const _r6td = scanHistory[text] !== getJSTDateString();
        document.getElementById("shopQrRewardArea").innerHTML = (_r6ft || _r6td)
            ? `<div style="font-size:12px; color:#9a6e30; font-weight:bold; margin-top:4px;">📱 SNSを開くと報酬GET！</div>` : '';
        document.getElementById("getBox").style.display = "block";
    } catch(e) { console.error('processShop6QR error:', e); }
}
// 実店舗「グーグー藤カレー」専用QR処理
const SHOP7_KEYWORD_URLS = {
    "instagram.com/fujicurry.takatsuki": "https://www.instagram.com/fujicurry.takatsuki/"
};
function giveShop7Reward(text) {
    const isFirstTime = localStorage.getItem('qr_shop7_scanned') !== '1';
    const today = getJSTDateString();
    const isFirstToday = scanHistory[text] !== today;
    scanHistory[text] = today;
    const rewardArea = document.getElementById("shopQrRewardArea");
    let rewardMsg = null;
    if(isFirstTime) {
        localStorage.setItem('qr_shop7_scanned', '1');
        packTicket = (packTicket || 0) + 1;
        rewardMsg = '🎫 パック券 1枚 GET！';
        if(rewardArea) rewardArea.innerHTML = `<div style="font-size:14px; font-weight:bold; color:#e67e22;">${rewardMsg}</div>`;
        const pkEl = document.getElementById('globalPackTicket'); if(pkEl) pkEl.innerText = packTicket;
...
function processShop7QR(text, matchedKeyword) {
    try {
        currentShop1LinkUrl = SHOP7_KEYWORD_URLS[matchedKeyword] || '';
        pendingShopQR = { shopId: 'shop7', text };
        document.getElementById("getBoxTitle").innerText = "店舗情報";
        document.getElementById("bonusBadge").style.display = "none";
        document.getElementById("getItemEmoji").style.display = "none";
        document.getElementById("getItemName").style.display = "none";
        document.getElementById("shopQrArea").style.display = "block";
        document.getElementById("shopQrShopName").innerText = 'グーグー藤カレー';
        document.getElementById("shopQrLocation").innerText = '大阪 高槻';
        document.getElementById("shopQrPhotos").innerHTML = '<img src="shop/shop-7-1.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;"><img src="shop/shop-7-2.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;">';
        document.getElementById("shopQrDesc").innerHTML = 'JR高槻から徒歩2分<br>お腹も心も満たすカレーを😁<br>化学調味料、小麦粉、白砂糖不使用👍<br>天日塩、インドから空輸のスパイス、バスマティ米<br>SNSのフォローよろしくお願いします！';
        document.getElementById("shopQrOpenLinkBtn").style.display = 'inline-block';
        const _r7ft = localStorage.getItem('qr_shop7_scanned') !== '1';
        const _r7td = scanHistory[text] !== getJSTDateString();
        document.getElementById("shopQrRewardArea").innerHTML = (_r7ft || _r7td)
            ? `<div style="font-size:12px; color:#9a6e30; font-weight:bold; margin-top:4px;">📱 SNSを開くと報酬GET！</div>` : '';
        document.getElementById("getBox").style.display = "block";
    } catch(e) { console.error('processShop7QR error:', e); }
}
// 実店舗「創作カレー ツキノワ」専用QR処理
const SHOP8_KEYWORD_URLS = {
    "instagram.com/tsukinowa_curry": "https://www.instagram.com/tsukinowa_curry/",
    "x.com/tsukinowa_curry":         "https://x.com/tsukinowa_curry"
};
function giveShop8Reward(text) {
    const isFirstTime = localStorage.getItem('qr_shop8_scanned') !== '1';
    const today = getJSTDateString();
    const isFirstToday = scanHistory[text] !== today;
    scanHistory[text] = today;
    const rewardArea = document.getElementById("shopQrRewardArea");
    let rewardMsg = null;
    if(isFirstTime) {
        localStorage.setItem('qr_shop8_scanned', '1');
        packTicket = (packTicket || 0) + 1;
        rewardMsg = '🎫 パック券 1枚 GET！';
        if(rewardArea) rewardArea.innerHTML = `<div style="font-size:14px; font-weight:bold; color:#e67e22;">${rewardMsg}</div>`;
        const pkEl = document.getElementById('globalPackTicket'); if(pkEl) pkEl.innerText = packTicket;
...
function processShop8QR(text, matchedKeyword) {
    try {
        currentShop1LinkUrl = SHOP8_KEYWORD_URLS[matchedKeyword] || '';
        pendingShopQR = { shopId: 'shop8', text };
        document.getElementById("getBoxTitle").innerText = "店舗情報";
        document.getElementById("bonusBadge").style.display = "none";
        document.getElementById("getItemEmoji").style.display = "none";
        document.getElementById("getItemName").style.display = "none";
        document.getElementById("shopQrArea").style.display = "block";
        document.getElementById("shopQrShopName").innerText = '創作カレー ツキノワ';
        document.getElementById("shopQrLocation").innerText = '大阪 堺筋本町';
        document.getElementById("shopQrPhotos").innerHTML = '<img src="shop/shop-8-1.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;"><img src="shop/shop-8-2.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;">';
        document.getElementById("shopQrDesc").innerHTML = '大阪メトロ「堺筋本町駅」の3番出口から徒歩5分<br>香り弾ける鮮烈なスパイス感溢れる<br>和風出汁ベースの創作系カレー<br>SNSのフォローよろしくお願いします！';
        document.getElementById("shopQrOpenLinkBtn").style.display = 'inline-block';
        const _r8ft = localStorage.getItem('qr_shop8_scanned') !== '1';
        const _r8td = scanHistory[text] !== getJSTDateString();
        document.getElementById("shopQrRewardArea").innerHTML = (_r8ft || _r8td)
            ? `<div style="font-size:12px; color:#9a6e30; font-weight:bold; margin-top:4px;">📱 SNSを開くと報酬GET！</div>` : '';
        document.getElementById("getBox").style.display = "block";
    } catch(e) { console.error('processShop8QR error:', e); }
}
// 実店舗「さぼてん食堂」専用QR処理
const SHOP9_KEYWORD_URLS = {
    "instagram.com/saboten.osaka": "https://www.instagram.com/saboten.osaka/",
    "x.com/sabo10_osaka":          "https://x.com/sabo10_osaka"
};
function giveShop9Reward(text) {
    const isFirstTime = localStorage.getItem('qr_shop9_scanned') !== '1';
    const today = getJSTDateString();
    const isFirstToday = scanHistory[text] !== today;
    scanHistory[text] = today;
    const rewardArea = document.getElementById("shopQrRewardArea");
    let rewardMsg = null;
    if(isFirstTime) {
        localStorage.setItem('qr_shop9_scanned', '1');
        packTicket = (packTicket || 0) + 1;
        rewardMsg = '🎫 パック券 1枚 GET！';
        if(rewardArea) rewardArea.innerHTML = `<div style="font-size:14px; font-weight:bold; color:#e67e22;">${rewardMsg}</div>`;
        const pkEl = document.getElementById('globalPackTicket'); if(pkEl) pkEl.innerText = packTicket;
...
function processShop9QR(text, matchedKeyword) {
    try {
        currentShop1LinkUrl = SHOP9_KEYWORD_URLS[matchedKeyword] || '';
        pendingShopQR = { shopId: 'shop9', text };
        document.getElementById("getBoxTitle").innerText = "店舗情報";
        document.getElementById("bonusBadge").style.display = "none";
        document.getElementById("getItemEmoji").style.display = "none";
        document.getElementById("getItemName").style.display = "none";
        document.getElementById("shopQrArea").style.display = "block";
        document.getElementById("shopQrShopName").innerText = 'さぼてん食堂';
        document.getElementById("shopQrLocation").innerText = '大阪 肥後橋';
        document.getElementById("shopQrPhotos").innerHTML = '<img src="shop/shop-9-1.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;"><img src="shop/shop-9-2.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;">';
        document.getElementById("shopQrDesc").innerHTML = '肥後橋駅8番出口から徒歩5分弱<br>欧風キーマカレー 専門店<br>ぴあ究極のカレー殿堂入り/Retty人気店/食べログ百名店（全国版）選出<br>SNSのフォローよろしくお願いします！';
        document.getElementById("shopQrOpenLinkBtn").style.display = 'inline-block';
        const _r9ft = localStorage.getItem('qr_shop9_scanned') !== '1';
        const _r9td = scanHistory[text] !== getJSTDateString();
        document.getElementById("shopQrRewardArea").innerHTML = (_r9ft || _r9td)
            ? `<div style="font-size:12px; color:#9a6e30; font-weight:bold; margin-top:4px;">📱 SNSを開くと報酬GET！</div>` : '';
        document.getElementById("getBox").style.display = "block";
    } catch(e) { console.error('processShop9QR error:', e); }
}
// 実店舗「AKARI CURRY － Gallery & Bar －」専用QR処理
const SHOP10_KEYWORD_URLS = {
    "instagram.com/akaricurry": "https://www.instagram.com/akaricurry/"
};
function giveShop10Reward(text) {
    const isFirstTime = localStorage.getItem('qr_shop10_scanned') !== '1';
    const today = getJSTDateString();
    const isFirstToday = scanHistory[text] !== today;
    scanHistory[text] = today;
    const rewardArea = document.getElementById("shopQrRewardArea");
    let rewardMsg = null;
    if(isFirstTime) {
        localStorage.setItem('qr_shop10_scanned', '1');
        packTicket = (packTicket || 0) + 1;
        rewardMsg = '🎫 パック券 1枚 GET！';
        if(rewardArea) rewardArea.innerHTML = `<div style="font-size:14px; font-weight:bold; color:#e67e22;">${rewardMsg}</div>`;
        const pkEl = document.getElementById('globalPackTicket'); if(pkEl) pkEl.innerText = packTicket;
...
function processShop10QR(text, matchedKeyword) {
    try {
        currentShop1LinkUrl = SHOP10_KEYWORD_URLS[matchedKeyword] || '';
        pendingShopQR = { shopId: 'shop10', text };
        document.getElementById("getBoxTitle").innerText = "店舗情報";
        document.getElementById("bonusBadge").style.display = "none";
        document.getElementById("getItemEmoji").style.display = "none";
        document.getElementById("getItemName").style.display = "none";
        document.getElementById("shopQrArea").style.display = "block";
        document.getElementById("shopQrShopName").innerText = 'AKARI CURRY － Gallery & Bar －';
        document.getElementById("shopQrLocation").innerText = '大阪 桜ノ宮';
        document.getElementById("shopQrPhotos").innerHTML = '<img src="shop/shop-10-1.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;"><img src="shop/shop-10-2.png" style="width:48%; max-width:160px; border-radius:6px; object-fit:cover;">';
        document.getElementById("shopQrDesc").innerHTML = 'JR桜ノ宮駅から徒歩1分<br>さらさら系欧風カレーのお店です🍛<br>お昼はカレー夜は小皿とお酒、日常にAKARIをテーマに日々頑張ります🍛<br>SNSのフォローよろしくお願いします！';
        document.getElementById("shopQrOpenLinkBtn").style.display = 'inline-block';
        const _r10ft = localStorage.getItem('qr_shop10_scanned') !== '1';
        const _r10td = scanHistory[text] !== getJSTDateString();
        document.getElementById("shopQrRewardArea").innerHTML = (_r10ft || _r10td)
            ? `<div style="font-size:12px; color:#9a6e30; font-weight:bold; margin-top:4px;">📱 SNSを開くと報酬GET！</div>` : '';
        document.getElementById("getBox").style.display = "block";
    } catch(e) { console.error('processShop10QR error:', e); }
}
function processQRData(text, r, g, b) {
    try {
        ...
        const matchedShop4Keyword = Object.keys(SHOP4_KEYWORD_URLS).find(kw => text.includes(kw) || decodedText.includes(kw));
        if(matchedShop4Keyword) {
            processShop4QR(text, matchedShop4Keyword);
            return;
        }
        const matchedShop5Keyword = Object.keys(SHOP5_KEYWORD_URLS).find(kw => text.includes(kw) || decodedText.includes(kw));
        if(matchedShop5Keyword) {
            processShop5QR(text, matchedShop5Keyword);
            return;
        }
        const matchedShop6Keyword = Object.keys(SHOP6_KEYWORD_URLS).find(kw => text.includes(kw) || decodedText.includes(kw));
        if(matchedShop6Keyword) {
            processShop6QR(text, matchedShop6Keyword);
            return;
        }
        const matchedShop7Keyword = Object.keys(SHOP7_KEYWORD_URLS).find(kw => text.includes(kw) || decodedText.includes(kw));
        if(matchedShop7Keyword) {
            processShop7QR(text, matchedShop7Keyword);
            return;
        }
        const matchedShop8Keyword = Object.keys(SHOP8_KEYWORD_URLS).find(kw => text.includes(kw) || decodedText.includes(kw));
        if(matchedShop8Keyword) {
            processShop8QR(text, matchedShop8Keyword);
            return;
        }
        const matchedShop9Keyword = Object.keys(SHOP9_KEYWORD_URLS).find(kw => text.includes(kw) || decodedText.includes(kw));
        if(matchedShop9Keyword) {
            processShop9QR(text, matchedShop9Keyword);
            return;
        }
        const matchedShop10Keyword = Object.keys(SHOP10_KEYWORD_URLS).find(kw => text.includes(kw) || decodedText.includes(kw));
        if(matchedShop10Keyword) {
            processShop10QR(text, matchedShop10Keyword);
            return;
        }
        const today = getJSTDateString();
...
const CURRY_SKILL_INFO = [
    ...
    { flag: 'isKaiTate',    name: '🛡️ 貝の盾',       desc: '通常攻撃を確率でガードしてダメージ0にする。SPDが高いほどガード率アップ（最低15%〜最高35%）。' },
    { flag: 'isKyoyasai',   name: '⛩️ 京野菜',       desc: '時々「ぶぶ漬けでも、いかがどす？」でDEF無視80ダメージ。敵の技を打ち返すホームランも併せ持つ。' },
    { flag: 'isCheeseCurry', name: '🧀 チーズキャノン', desc: '時々DEF無視でATKの1〜2倍のダメージを与える「チーズキャノン」を放つ。' },
    { flag: 'isShrimpCurry', name: '🍤 小エビよ！突撃だ！', desc: '時々通常攻撃をATKではなくSPDの数値で行う。' },
    { flag: 'isBananaCurry', name: '🍌 バナナトラップ', desc: '戦闘開始時にバナナトラップを設置。相手は30%の確率で自分のターンに行動不能になる。' }
];
...
function updateFridgeCurryStockGrid() {
    ...
    curryStock.forEach((curry, idx) => {
        ...
        // 「Delivering...」（宅配設定）と「サンプル設定済」（食品サンプル設定）は同時に成立しない（宅配カレーはサンプル対象外）
        const deliveringLabel = curry.isDelivering ? '<div class="delivering-label">Delivering...</div>' : (curry.foodSampleSet ? '<div class="sample-set-label">サンプル設定済</div>' : '');
...
function showStockCurryDetail(title, message, idx) {
    ...
    if(curryStock[idx] && !curryStock[idx].isDelivering) {
        ...
        sampleBtn.className = "modal-btn";
        sampleBtn.style.background = curryStock[idx].foodSampleSet ? "#d35400" : "#b88742";
        sampleBtn.style.color = "#fff";
        sampleBtn.innerText = curryStock[idx].foodSampleSet ? `🍥 食品サンプル設定: ON` : `🍥 食品サンプル設定: OFF`;
        sampleBtn.onclick = function() {
            toggleFoodSampleSetting(idx);
        };
...
// 食品サンプル設定：ONにしておくと、そのカレーが対戦でHP0になり敗北した場合、バトル終了後に
// ストックから消滅させる代わりに食品サンプルを1つ消費して手元に戻す（所持数0の場合は通常通り消費される）。
function toggleFoodSampleSetting(idx) {
    if(!curryStock[idx] || curryStock[idx].isDelivering) return;
    curryStock[idx].foodSampleSet = !curryStock[idx].foodSampleSet;
    saveGame();
    updateFridgeUI();
    // モーダルは開いたまま、ボタンの表示だけ最新の状態に更新する
    const title = document.getElementById("modalTitle").innerHTML;
    const message = document.getElementById("modalMessage").innerHTML;
    showStockCurryDetail(title, message, idx);
}
// バトルが敗北で終わった際の消費処理をまとめたヘルパー。
// idx番目のカレーが「食品サンプル設定済」かつ食品サンプルを1つ以上所持していれば、
// ストックからは消さずに食品サンプルを1つ消費して身代わりにし、trueを返す。
// それ以外（設定なし／サンプル0個）の場合は通常通りcurryStockから消滅させ、falseを返す。
// ※カレーフェスでは呼び出さない（フェスは食品サンプル導入不要のため）。
function tryConsumeCurryOnLoss(idx) {
    const c = curryStock[idx];
    if(c && c.foodSampleSet && (foodSampleCount || 0) > 0) {
        foodSampleCount -= 1;
        const foodSampleElBattle = document.getElementById('globalFoodSample');
        if(foodSampleElBattle) foodSampleElBattle.innerText = foodSampleCount;
        const sampleCurryName = c.name;
        setTimeout(() => showCustomAlert('🍥 食品サンプル使用', `食品サンプルが代わりに消費され、${sampleCurryName}はストックに返品されました。`), 500);
        return true;
    }
...
const CURRY_SPECIAL_IMAGES = {
    ...
    kaitate:        'currymonster/curry_kaitate.png',
    kyoyasai:       'currymonster/curry_kyoyasai.png',
    cheese:         'currymonster/curry_cheese.png',
    shrimp:         'currymonster/curry_shrimp.png',
    banana:         'currymonster/curry_banana.png'
};
function getCurryImage(curry) {
    if(!curry) return CURRY_TYPE_IMAGES.balance;
    // 🍤小えびスピードカレーは全食材が海鮮のためisSeafoodも同時にtrueになるが、専用画像を優先させる
    if(curry.isShrimpCurry)    return CURRY_SPECIAL_IMAGES.shrimp;
    ...
    if(curry.isKyoyasai)       return CURRY_SPECIAL_IMAGES.kyoyasai;
    if(curry.isCheeseCurry)    return CURRY_SPECIAL_IMAGES.cheese;
    if(curry.isBananaCurry)    return CURRY_SPECIAL_IMAGES.banana;
    ...
    if(curry.isKaiTate)        return CURRY_SPECIAL_IMAGES.kaitate;
...
function buildCurryFromMaterials(materialNames, spiceName) {
    ...
    // ⛩️京野菜カレー：聖護院大根＋金時にんじん＋賀茂なす（京の伝統野菜3つ）。常にホームランも併せ持つ
    let isKyoyasai = acts.length === 3 && acts.includes("聖護院大根") && acts.includes("金時にんじん") && acts.includes("賀茂なす");
    // 🧀チーズまみれカレー：チーズ3つ
    let isCheeseCurry = acts.filter(x => x === "チーズ").length === 3;
    // 🍤小えびスピードカレー：小エビ3つ
    let isShrimpCurry = acts.filter(x => x === "小エビ").length === 3;
    // 🍌そんなバナナカレー：バナナ3つ
    let isBananaCurry = acts.filter(x => x === "バナナ").length === 3;
    ...
    // 🛡️貝の盾カレー：（ホタテ or 牡蠣）＋海鮮食材がもう1種（＝海鮮食材が合計2種以上、うち1つは貝殻系）
    const SHELL_LIST = ["ホタテ","牡蠣"];
    let hasShell = acts.some(x => SHELL_LIST.includes(x));
    let shieldSeafoodCount = acts.filter(x => SEAFOOD_LIST.includes(x)).length;
    let isKaiTate = hasShell && shieldSeafoodCount >= 2;
    let kaiTateShellName = acts.includes("ホタテ") ? "ホタテ" : "牡蠣";
    ...
    else if(isKyoyasai) poisonChosen = "kyoyasai"; // 京野菜カレー：3つのレア京野菜すべてを使う厳しい条件のため最優先扱いする
    ...
    if(!poisonChosen) {
        ...
        if(isKaiTate) exclusiveCandidates.push("kaitate");
        if(isCheeseCurry) exclusiveCandidates.push("cheese");
        if(isShrimpCurry) exclusiveCandidates.push("shrimp");
        if(isBananaCurry) exclusiveCandidates.push("banana");
    }
    ...
    isPoison=false; isMargherita=false; isTonTonTon=false; isIllusion=false; isSticky=false; isSeed=false; isWanpaku=false; isRatatouille=false; isHomerun=false; isPoisonApple=false; isFluffyOmelette=false; isGreenCurry=false; isTriCaviar=false; isKaiTate=false; isKyoyasai=false; isCheeseCurry=false; isShrimpCurry=false; isBananaCurry=false;
    ...
    else if(chosen === "kyoyasai") { cName = "⛩️京野菜カレー⛩️"; isKyoyasai = true; isHomerun = true; }
    ...
    else if(chosen === "kaitate") { cName = `🛡️${kaiTateShellName}の盾カレー🛡️`; isKaiTate = true; }
    else if(chosen === "cheese") { cName = "🧀チーズまみれカレー🧀"; isCheeseCurry = true; }
    else if(chosen === "shrimp") { cName = "🍤小えびスピードカレー🍤"; isShrimpCurry = true; }
    else if(chosen === "banana") { cName = "🍌そんなバナナカレー🍌"; isBananaCurry = true; }
    ...
    return { name: cName, visual: vis, hp: hp, atk: atk, def: def, spd: spd, isPoison, hasGold, isMargherita, isTonTonTon, isSeafood, isIllusion, isSticky, isSeed, isWanpaku, isRatatouille, isHomerun, isPoisonApple, isFluffyOmelette, isGreenCurry, isTriCaviar, isKaiTate, isKyoyasai, isCheeseCurry, isShrimpCurry, isBananaCurry, isCritical, materials: acts.length ? acts : [], spice: sp || "", curryType: curryType };
}
...
// イベント：特盛りモンスターから街を守れ！
// ============================================================
const EVENT_MONSTER_DATA = { hp: 5000, atk: 100, def: 90, spd: 60, name: "水面カレーシャーク" };
// 特盛モンスター第二弾で新規追加：割り込みキャラ（HPバー・名前表示なし、攻撃対象にもならない）
const SHARK_SON_DATA = {
    long: { name: "シャーク長男", atk: 100, spd: 55 },
    second: { name: "シャーク次男", atk: 70, spd: 10 }
};
...
function loadEventEnabledStatus() {
    ...
    database.ref('events/tokumori002/enabled').once('value').then(function(snap){
...
function showGuideChar(imageSet) {
    if(!isGuideCharEnabled()) return; // システム・設定で案内人がオフなら表示しない
...
function checkEventBannerVisibility() {
    ...
    if(replayBtn) {
        const hasWatched = localStorage.getItem('qr_event_movie_watched_tokumori002') === '1';
...
function showTokumori001Results() {
    const ranking = [
        { rank: 1, name: "天然アフロ", dmg: 33436 },
        { rank: 2, name: "サフラン", dmg: 23017 },
        { rank: 3, name: "おにぎり", dmg: 9576 },
        { rank: 4, name: "🦊さん", dmg: 8194 },
        { rank: 5, name: "カレーガチ勢🍛", dmg: 3955 }
    ];
    const medal = { 1: "🥇", 2: "🥈", 3: "🥉" };
    let rows = "";
    ranking.forEach(r => {
        rows += `<div style="display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid rgba(66,0,0,0.15);">`
            + `<span>${medal[r.rank] || (r.rank + "位")} ${r.name}</span>`
            + `<span style="font-weight:bold;">${r.dmg.toLocaleString()}ダメージ</span>`
            + `</div>`;
    });
    const msg = '<div style="text-align:left;font-size:13px;line-height:1.6;color:#420000;">'
        + '討伐にご協力いただきありがとうございました！<br><br>'
        + rows
        + '</div>';
    showCustomAlert('【第1弾 浪花のカレーライスベイビー 結果】', msg);
}
...
function showFestInfo() {
    const msg = '<div style="text-align:left;font-size:13px;line-height:1.8;color:#420000;">'
        ...
        + '<b>●中断</b><br>'
        + '画面右上の「中断」ボタンから「一時中断」「撤退」を選べます。<br>'
        + '「一時中断」はフェスの状況を保存したまま対戦タブに戻れます。フェス外のプレイも通常通り行え、「カレーフェス」からいつでも再開できます（報酬は受け取りません）。<br>'
...
function startEventBattle() {
    ...
    if(!hasWatched) {
        playEventMovie(() => {
            localStorage.setItem('qr_event_movie_watched_tokumori002', '1');
            checkEventBannerVisibility();
            showEventVsScreen();
        });
    } else {
...
function showEventVsScreen() {
    ...
    const oppCurryData = {
        ...
        isEventBoss: true, isBotImage: false, foodCategory: 'seafood'
    };
...
// ===== 日替わりカレー =====
function updateDailyCurryButtonImage() {
    const img = document.getElementById('dailyCurryBtnImg');
    if(!img) return;
    const def = getDailyCurryDef(getDailyCurryWeekdayIndex());
    img.src = def.btnImg;
    img.alt = '日替わりカレー（' + def.name + '）';
}
function isDailyCurryWonToday() {
    return !!dailyCurryLastWinDate && dailyCurryLastWinDate === getJSTDateString();
}
function openDailyCurryModal() {
    if (!hasUsableCurryForBattle()) { alertNoUsableCurry(); return; }
    document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
    document.querySelectorAll('.tab-btn').forEach(b => b.classList.remove('active'));
    document.getElementById('dailyCurryFullscreen').classList.add('active');
    hideQuestGuideChar();
    renderDailyCurryModal();
}
function closeDailyCurryModal() {
    document.getElementById('dailyCurryFullscreen').classList.remove('active');
...
function renderDailyCurryModal() {
    ...
    const def = getDailyCurryDef(weekdayIdx);
    const body = document.getElementById('dailyCurryBody');
    ...
    const wonToday = isDailyCurryWonToday();
...
function launchDailyCurryBattle(tier) {
    if (!hasUsableCurryForBattle()) { alertNoUsableCurry(); return; }
    if (isDailyCurryWonToday()) { showCustomAlert('⚠️ 本日は挑戦済みです', '日替わりカレーは1日1回、勝利すると翌日まで再挑戦できません。'); return; }
    if (audioCtx.state === 'suspended') audioCtx.resume();
    isBotMatch = true; currentRoomId = null;
    const weekdayIdx = getDailyCurryWeekdayIndex();
    const def = getDailyCurryDef(weekdayIdx);
...
function openBotDifficultyChooser() {
    if (!hasUsableCurryForBattle()) { alertNoUsableCurry(); return; }
    document.getElementById('botDifficultyChooserOverlay').style.display = 'flex';
}
function closeBotDifficultyChooser() {
    document.getElementById('botDifficultyChooserOverlay').style.display = 'none';
}
...
// フェス戦即決（スキップ）が有効な間は、フェスバトル中に限りウェイトを0にして
// ターンの経過演出を実質すっ飛ばす。festBattleInProgressで囲っているため、
// 通常対戦・タッグ対戦など他のバトルには一切影響しない。
function battleDelay(ms) {
    if (typeof festBattleInProgress !== 'undefined' && festBattleInProgress && festSkipModeActive) return 0;
    return Math.round(ms * battleSpeedMultiplier);
}
// バトル速度切り替えボタン群（.tag-speed-btnクラスは音量オンオフ・調理アニメーション・案内人等の
// 設定ボタンとも共用しているため、active切り替えはこのIDリストのみに限定し、他の設定ボタンの表示に影響しないようにする）
const BATTLE_SPEED_BTN_IDS = [
    'tagSpeedBtn1', 'tagSpeedBtn2', 'tagSpeedBtn4',
    'normalSpeedBtn1', 'normalSpeedBtn2', 'normalSpeedBtn4',
    'settingsSpeedBtn1', 'settingsSpeedBtn2', 'settingsSpeedBtn4',
    'festSpeedBtn1', 'festSpeedBtn2', 'festSpeedBtn4',
    'festSkipBtn'
];
function setBattleSpeed(mult) {
    battleSpeedMultiplier = mult;
    festSkipModeActive = false; // 速度ボタンを選んだらスキップモードは解除する
    const suffix = mult === 1 ? '1' : (mult === 0.5 ? '2' : '4');
    BATTLE_SPEED_BTN_IDS.forEach(id => { const b = document.getElementById(id); if(b) b.classList.remove('active'); });
...
// フェス戦即決（スキップ）モードに切り替える。フェスバトル専用の速度ボタン群にのみ存在する。
function setFestSkipMode() {
    festSkipModeActive = true;
    BATTLE_SPEED_BTN_IDS.forEach(id => { const b = document.getElementById(id); if(b) b.classList.remove('active'); });
    const btn = document.getElementById('festSkipBtn');
    if(btn) btn.classList.add('active');
}
// 現在所持している「フェス戦即決」の適用上限（未購入なら0、20戦目まで/30戦目までのいずれか）
function getFestSkipMaxBattle() {
    if (unlockFestSkip30) return 30;
    if (unlockFestSkip20) return 20;
    return 0;
}
// フェスバトル開始時に呼び、現在の戦闘数が「フェス戦即決」の適用範囲内かどうかでスキップボタンの
// 表示・非表示を切り替える。適用範囲外になった場合はスキップモードも強制的に解除する。
function updateFestSkipButtonVisibility() {
    const btn = document.getElementById('festSkipBtn');
    if(!btn) return;
    const maxSkip = getFestSkipMaxBattle();
    const battleNum = (festWinStreak || 0) + 1;
    const allowed = maxSkip > 0 && battleNum <= maxSkip;
...
function tagUpdateAllHpDisplays() {
    ...
    refreshTagBattleNameIcons();
}
// 名前横の状態アイコン：4人のファイター（自分・宅配仲間・敵2体）それぞれの現在の状態から算出して反映する。
// tagUpdateAllHpDisplays()はHPが変化するたびに（=ほぼ毎ターン）呼ばれているため、ここに載せておけば
// 個々の状態変化箇所すべてに個別のアイコン更新処理を書かなくても自然と同期する。
function refreshTagBattleNameIcons() {
    if(!tagBattleFighters) return;
    tagBattleFighters.forEach(f => {
        if(!f) return;
        const keys = [];
        if(f.isIlluded) keys.push('illusion');
        if(f.isPoisoned) keys.push('poison');
        if(f.defBuffTurns > 0) keys.push('defUp');
        if(f.curry && f.curry.isRatatouille) { keys.push('hpRegen'); keys.push('meatReduce'); }
        const fluffyIconKey = fluffyCategoryToIconKey(f.fluffyCategory);
        if(fluffyIconKey) keys.push(fluffyIconKey);
        const elId = (f.side === 'player' ? 'tagPlayerName' : 'tagEnemyName') + f.idx;
        updateBattleStatusIconsFor(elId, keys);
    });
}
...
function launchTagBattle(myCurry, deliveryCand, enemies) {
    // 【不具合修正】applyTablewareModifiersはObject.assignで別オブジェクトを作って返すため、
    // 以前はここでmyCurryを丸ごと差し替えてしまい、戦闘終了時のcurryStock.indexOf(myFighter.curry)が
    // 参照不一致で常に-1になり、出撃カレーが消費されない不具合が起きていた。
    // 消費判定用に、食器補正を適用する前の元オブジェクト参照を別途保持しておく。
    const originalMyCurry = myCurry;
    ...
    // 4ファイター構築
    const myFighter = {
        side: 'player', idx: 0, name: playerName, curry: myCurry, originalCurry: originalMyCurry,
...
function tagExecuteAction(actor, myTeam, enemyTeam, callback) {
    ...
    if(target.curry && target.curry.isKaiTate && rollShieldGuard(target.curry.spd)) {
        playSoundEffect('sound/guard.mp3');
        tagTriggerDamagePop('enemy', target.idx, 'Guard', '#3498db');
        tagLogAppend(`🛡️ ${target.name} が攻撃を貝の盾でガードした。`);
        callback();
        return;
    }
...
function tagEnemyNormalAttack(actor, myTeam, callback) {
    ...
    if(target.curry && target.curry.isKaiTate && rollShieldGuard(target.curry.spd)) {
        playSoundEffect('sound/guard.mp3');
        tagTriggerDamagePop('player', target.idx, 'Guard', '#3498db');
        tagLogAppend(`🛡️ ${target.name} が攻撃を貝の盾でガードした。`);
        callback();
        return;
    }
...
function tagFinishBattle(result) {
    ...
    // ※myFighter.curryは食器補正適用後の別オブジェクトのため参照が一致しない。元の参照(originalCurry)で探す。
    const myIdx = curryStock.indexOf(myFighter.originalCurry || myFighter.curry);
    if(myIdx !== -1) {
        // 敗北時のみ食品サンプルによる身代わり判定。勝利・引き分けは通常通り消費する。
        if(isWin || result === 'draw') { curryStock.splice(myIdx, 1); }
        else { tryConsumeCurryOnLoss(myIdx); }
    }
    if(selectedCurryIndex >= curryStock.length) selectedCurryIndex = curryStock.length - 1;
    saveGame(); updateFridgeUI(); updateCookSelects(); updateMatchCurrySelects();
}
...
// わんぱくカレー：通常攻撃ダメージ1〜2倍の振れ幅、30%でミス
// rateを渡すとその確率を使う（日替わりカレー「火曜日の衝撃」の甘口/中辛/辛口ごとの可変ミス率用）。
// 省略時は既存仕様通り固定30%（プレイヤーのわんぱくカレー・悪ガキサタン君など既存の挙動は一切変わらない）。
function getWanpakuDamageMultiplier() { return 1 + Math.random() * 0.5; } // 1.0〜1.5
function isWanpakuMiss(rate) { return Math.random() < (typeof rate === 'number' ? rate : 0.30); }
...
// 貝の盾カレー：通常攻撃を確率でガードしてダメージ0にする。ガード率はSPDで変動
// （SPD50以下で最低15%、SPD200以上で最高35%、その間は線形に補間）。
function getShieldGuardRate(spd) {
    if(spd <= 50) return 0.15;
    if(spd >= 200) return 0.35;
    return 0.15 + (spd - 50) / (200 - 50) * (0.35 - 0.15);
}
function rollShieldGuard(spd) { return Math.random() < getShieldGuardRate(spd); }
...
function calcAllBattleTurns(myC, oppC, oppN, myName) {
    ...
    if(myC.isBananaCurry) statusLines.push(`🍌バナナトラップが仕掛けられた🍌`);
    if(oppC.isBananaCurry) statusLines.push(`🍌バナナトラップが仕掛けられた🍌`);
    let myTurn;
    ...
    for(let round = 1; round <= 50; round++) {
        ...
        if(myTurn) {
            ...
            // 🍌そんなバナナカレー：相手が仕掛けたバナナトラップに30%でかかり、このターンは行動不能
            if(oppC.isBananaCurry && Math.random() < 0.3) {
                turn.log+=`
🍌 {{ACTOR}}はバナナトラップにかかってしまった！
🍌 そんなバナナ〜🍌`;
                turn.hostHP = pHP; turn.guestHP = oHP; myTurn=!myTurn; turns.push(turn);
                continue;
            }
...
💨 わんぱくが暴れすぎて攻撃が外れた！`;
            ...
            } else if(myC.isKyoyasai && Math.random()<0.3) {
                // ⛩️京野菜カレー専用特技「ぶぶ漬けでも、いかがどす？」：熱々ブレスと同じDEF無視固定80ダメージ
                turn.log+=`
🍵 {{ACTOR}}の「ぶぶ漬けでも、いかがどす？」！`;
                if(oppC.isHomerun && isHomerunReflect(oppC.spd, myC.spd)) {
                    turn.log+=`
🏏 ホームラン！{{ACTOR}}のぶぶ漬けを打ち返して無効化！`;
                    turn.effects.push({type:'homerun-reflect'});
                } else {
                    oHP = Math.max(0, oHP - 80);
                    turn.log+=`
🔥 {{DMGLABEL}}: DEF無視80ダメージ！`;
                    turn.effects.push({type:'enemy', dmg:80, breath:true});
                }
            } else if(myC.isCheeseCurry && Math.random()<0.3) {
                // 🧀チーズまみれカレー専用特技「チーズキャノン」：DEF無視でATK1〜2倍のダメージ
                turn.log+=`
🧀 {{ACTOR}}の「チーズキャノン」！`;
                if(oppC.isHomerun && isHomerunReflect(oppC.spd, myC.spd)) {
                    turn.log+=`
🏏 ホームラン！{{ACTOR}}のチーズキャノンを打ち返して無効化！`;
                    turn.effects.push({type:'homerun-reflect'});
                } else {
                    let d = Math.round(myC.atk * (1 + Math.random()));
                    oHP = Math.max(0, oHP - d);
                    turn.log+=`
🔥 {{DMGLABEL}}: DEF無視${d}ダメージ！`;
                    turn.effects.push({type:'enemy', dmg:d, cheeseCannon:true});
                }
            } else if(myC.isShrimpCurry && Math.random()<0.5) {
                // 🍤小えびスピードカレー専用特技「小エビよ！突撃だ！」：通常攻撃をATKではなくSPDの数値で行う
                turn.log+=`
🦐 {{ACTOR}}の「小エビよ！突撃だ！」！`;
                const isCrit = Math.random() < getCritRate(myC.spd, oppC.spd);
                let d = isCrit ? Math.max(8, myC.spd) : Math.max(8, myC.spd - Math.floor(oppC.def/2));
                d = Math.round(d*(0.9+Math.random()*0.2));
                let myFluffyBlockedShrimp = false;
                if(oppFluffyCategory && curryHasCategory(myC, oppFluffyCategory)) { d=Math.round(d*0.7); myFluffyBlockedShrimp = true; }
                oHP = Math.max(0, oHP - d);
                if(isCrit){turn.log+=`
🌶️ Spicy Hit!!!!!!!! {{DMGLABEL}}: ${d}`;turn.effects.push({type:'enemy-crit',dmg:d,shrimp:true});}
                else{turn.log+=`
{{DMGLABEL}}: ${d}`;turn.effects.push({type:'enemy',dmg:d,shrimp:true});}
                if(myFluffyBlockedShrimp) { turn.log+=`
...
🌶️ 反動で{{ACTOR}}にも${selfDmg}ダメージ！`;
            ...
            } else if(oppC.isKaiTate && rollShieldGuard(oppC.spd)) {
                turn.log+=`
🛡️ 攻撃を貝の盾でガードした。`;
                turn.effects.push({type:'enemy-guard'});
            } else {
                const isCrit = Math.random() < getCritRate(myC.spd, oppC.spd);
...
☠️ 毒のダメージが増幅`; turn.effects.push({type:'poison-amplify-enemy'}); }
            ...
            turn.log = `{{TURNICON}} {{ACTOR}} のターン！`;
            // 🍌そんなバナナカレー：自分が仕掛けたバナナトラップに相手が30%でかかり、このターンは行動不能
            if(myC.isBananaCurry && Math.random() < 0.3) {
                turn.log+=`
🍌 {{ACTOR}}はバナナトラップにかかってしまった！
🍌 そんなバナナ〜🍌`;
                turn.hostHP = pHP; turn.guestHP = oHP; myTurn=!myTurn; turns.push(turn);
...
🏏 ホームラン！{{ACTOR}}の種連続発射を打ち返して無効化！`;
            ...
            } else if(oppC.isKyoyasai && Math.random()<0.3) {
                // ⛩️京野菜カレー専用特技「ぶぶ漬けでも、いかがどす？」：熱々ブレスと同じDEF無視固定80ダメージ
                turn.log+=`
🍵 {{ACTOR}}の「ぶぶ漬けでも、いかがどす？」！`;
                if(myC.isHomerun && isHomerunReflect(myC.spd, oppC.spd)) {
...
🏏 ホームラン！{{ACTOR}}のぶぶ漬けを打ち返して無効化！`;
                    ...
                    turn.log+=`
🔥 {{DMGLABEL}}: DEF無視80ダメージ！`;
                    turn.effects.push({type:'player', dmg:80, breath:true});
                }
            } else if(oppC.isCheeseCurry && Math.random()<0.3) {
                // 🧀チーズまみれカレー専用特技「チーズキャノン」：DEF無視でATK1〜2倍のダメージ
                turn.log+=`
🧀 {{ACTOR}}の「チーズキャノン」！`;
                if(myC.isHomerun && isHomerunReflect(myC.spd, oppC.spd)) {
...
🏏 ホームラン！{{ACTOR}}のチーズキャノンを打ち返して無効化！`;
                    ...
                    let d = Math.round(oppC.atk * (1 + Math.random()));
                    pHP = Math.max(0, pHP - d);
                    turn.log+=`
🔥 {{DMGLABEL}}: DEF無視${d}ダメージ！`;
                    turn.effects.push({type:'player', dmg:d, cheeseCannon:true});
                }
            } else if(oppC.isShrimpCurry && Math.random()<0.5) {
                // 🍤小えびスピードカレー専用特技「小エビよ！突撃だ！」：通常攻撃をATKではなくSPDの数値で行う
                turn.log+=`
🦐 {{ACTOR}}の「小エビよ！突撃だ！」！`;
                const isCrit = Math.random() < getCritRate(oppC.spd, myC.spd);
                let d = isCrit ? Math.max(8, oppC.spd) : Math.max(8, oppC.spd - Math.floor(myC.def/2));
                d = Math.round(d*(0.9+Math.random()*0.2));
                let oppFluffyBlockedShrimp = false;
                if(myFluffyCategory && curryHasCategory(oppC, myFluffyCategory)) { d=Math.round(d*0.7); oppFluffyBlockedShrimp = true; }
                pHP = Math.max(0, pHP - d);
                if(isCrit){turn.log+=`
🌶️ Spicy Hit!!!!!!!! {{DMGLABEL}}: ${d}`;turn.effects.push({type:'player-crit',dmg:d,shrimp:true});}
                else{turn.log+=`
{{DMGLABEL}}: ${d}`;turn.effects.push({type:'player',dmg:d,shrimp:true});}
                if(oppFluffyBlockedShrimp) { turn.log+=`
...
🌶️ 反動で{{ACTOR}}にも${selfDmg}ダメージ！`;
            ...
            } else if(myC.isKaiTate && rollShieldGuard(myC.spd)) {
                turn.log+=`
🛡️ 攻撃を貝の盾でガードした。`;
                turn.effects.push({type:'player-guard'});
            } else {
                const isCrit = Math.random() < getCritRate(oppC.spd, myC.spd);
...
function playBattleTurns(turns, myC, oppC, oppN, role) {
    ...
    function playTurn() {
        ...
        // 🍌そんなバナナカレー「バナナトラップ」演出（オンライン/ルーム戦。ダメージ等はなく音のみ）
        if(resolvedLog.includes('バナナトラップにかかってしまった')) playSoundEffect('sound/manuke.mp3');
        // 種連射演出（オンライン/ルーム戦）
        ...
        // ⛩️京野菜カレー「ぶぶ漬けでも、いかがどす？」演出（オンライン/ルーム戦）
        if(resolvedLog.includes('ぶぶ漬け')) {
            const _actorIsMeKyo = (turn.actorIsHost === (onlineRole === 'host'));
            const _kyoCfg   = _actorIsMeKyo ? KYOYASAI_SKILL_CONFIG : KYOYASAI_SKILL_CONFIG_ENEMY;
            const _hasHRKyo = turn.effects.some(function(e){ return e.type === 'homerun-reflect'; });
            const _intrCfgKyo = _actorIsMeKyo ? HOMERUN_ANIM_CONFIG_ENEMY : HOMERUN_ANIM_CONFIG;
            playTanemakiAnimation(Object.assign({}, _kyoCfg, { interruptConfig: _intrCfgKyo }), _hasHRKyo, function(){});
        }
        // 🧀チーズまみれカレー「チーズキャノン」演出（オンライン/ルーム戦）
        if(resolvedLog.includes('チーズキャノン')) {
            const _actorIsMeChe = (turn.actorIsHost === (onlineRole === 'host'));
            const _cheCfg   = _actorIsMeChe ? CHEESE_SKILL_CONFIG : CHEESE_SKILL_CONFIG_ENEMY;
            const _hasHRChe = turn.effects.some(function(e){ return e.type === 'homerun-reflect'; });
            const _intrCfgChe = _actorIsMeChe ? HOMERUN_ANIM_CONFIG_ENEMY : HOMERUN_ANIM_CONFIG;
            playTanemakiAnimation(Object.assign({}, _cheCfg, { interruptConfig: _intrCfgChe }), _hasHRChe, function(){});
        }
        // 🍤小えびスピードカレー「小エビよ！突撃だ！」演出（オンライン/ルーム戦。ホームランでの打ち返しはない）
        if(resolvedLog.includes('小エビよ')) {
            const _actorIsMeShr = (turn.actorIsHost === (onlineRole === 'host'));
            const _shrCfg   = _actorIsMeShr ? SHRIMP_SKILL_CONFIG : SHRIMP_SKILL_CONFIG_ENEMY;
            playTanemakiAnimation(_shrCfg, false, function(){});
        }
        ...
        normalEffects.forEach(e => {
            ...
            if(isGuest) {
                ...
                else if(ef === 'player-guard') ef = 'enemy-guard';
                else if(ef === 'enemy-guard') ef = 'player-guard';
            }
            ...
            if(ef === 'enemy-guard' || ef === 'player-guard') {
                playSoundEffect('sound/guard.mp3');
                triggerEffect(ef === 'enemy-guard' ? 'enemy' : 'player', 'Guard');
                return;
            }
            ...
            else if(e.cheeseCannon) playSoundEffect('sound/gankowari.mp3');
            else if(e.shrimp) playSoundEffect('sound/ryusuikarei.mp3');
...
function startEventBattleScene(myC) {
    ...
    database.ref('events/tokumori002/currentHp').once('value').then(function(snap){
        let curHp = snap.val();
...
function launchEventBattle(myC, startHp) {
    ...
    liveBattleMyStatusKeys = []; liveBattleOppStatusKeys = [];
    updateBattleStatusIconsFor('pOwnerText', []); updateBattleStatusIconsFor('oppOwnerText', []);
    ...
    if(bgWrap && bgImg) { bgImg.src = "currymonster/tokumori002.jpg"; bgWrap.style.display = "block"; }
    ...
    let pHP = myC.hp;
    let oHP = startHp;
    const oppC = { hp: EVENT_MONSTER_DATA.hp, atk: EVENT_MONSTER_DATA.atk, def: EVENT_MONSTER_DATA.def, spd: EVENT_MONSTER_DATA.spd, isEventBoss: true, foodCategory: 'seafood' };
    let seaHealUsed = false; // 海鮮カレーHP回復フラグ
    ...
    // 🥚ふわとろオム／👑世界三大珍味：水面カレーシャーク・シャーク長男・次男は全員「海鮮」属性のため、
    // バリアが海鮮系統を選んだ場合のみ実際にダメージ軽減が発生する（該当しない属性の場合は演出のみ）。
    ...
    else if(myC.isKyoyasai) setTimeout(()=>playBattleBGM("sound/Kyoto.mp3"),300);
    ...
    // 名前横の状態アイコン：イベント戦はプレイヤー側が毒・幻惑にかかることは無く、モンスター側にのみ発生する。
    // ラタトゥイユの特性（毎ターン回復／肉系軽減）・ふわとろオム／世界三大珍味バリアはプレイヤー自身の名前に表示する。
    const myFluffyIconKeyE = fluffyCategoryToIconKey(myFluffyCategoryE);
    liveBattleMyStatusKeys = (myC.isRatatouille ? ['hpRegen','meatReduce'] : []).concat(myFluffyIconKeyE ? [myFluffyIconKeyE] : []);
    liveBattleOppStatusKeys = (isOppIlluded ? ['illusion'] : []).concat(isOppPoisoned ? ['poison'] : []);
    updateBattleStatusIconsFor('pOwnerText', liveBattleMyStatusKeys);
    updateBattleStatusIconsFor('oppOwnerText', liveBattleOppStatusKeys);
    let myTurn = myC.spd >= oppC.spd;
    let round = 1;
    let playerSkipNextTurn = false; // 鮫ダンスで踊らされている間、プレイヤーの次のターンを1回休みにする
    ...
    function checkEnragedTransition() {
        ...
        if(nowEnraged !== isEnraged) {
            ...
            applyEnragedDisplay();
            if(!myC.isTriCaviar && !myC.isKyoyasai) {
                stopBattleBGM();
    ...
    // 敵フェーズ（シャーク本体→長男→次男の順で最大3ターン）が全て終わったらプレイヤーのターンへ制御を戻す
    function afterEnemyPhase() {
        if(battleAborted) return;
        myTurn = true;
        setTimeout(eventStep, battleDelay(600));
    }

    // シャーク長男のターン（特盛モンスター第二弾専用）。水面カレーシャーク本体のターンが完全に
    // 終了した後、独立した判定で「行動するか／ターンが回ってこないか」を決める、それ自体1つの
    // 独立したターン。ターンが回ってこない場合は表示なしでそのまま次男の判定へ進む。
    // HPバー・名前表示は持たず、プレイヤーの攻撃対象にもならない。
    function runLongSonTurn() {
        if(battleAborted) return;
        const acts = Math.random() >= (isEnraged ? 0.30 : 0.80); // 通常20%／怒り70%で行動
        if(!acts) { runSecondSonTurn(); return; }
        log.innerHTML = `⚠️ シャーク長男が割り込んできた！`;
        playTone(300,'triangle',0.08);
        setTimeout(()=>{
            if(myC.isKaiTate && rollShieldGuard(myC.spd)) {
                playSoundEffect('sound/guard.mp3');
                log.innerHTML += `\n🛡️ 攻撃を貝の盾でガードした。`;
                triggerEffect("player","Guard");
                updateHP(pHP,oHP,oppC.hp,myC.hp);
                battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                round++; setTimeout(runSecondSonTurn, battleDelay(900));
                return;
            }
            let d = Math.max(6, Math.round(SHARK_SON_DATA.long.atk*(0.9+Math.random()*0.2)) - Math.floor(myC.def/2));
            if(myFluffyCategoryE === 'seafood') d = Math.round(d*0.7);
            pHP = Math.max(0, pHP - d);
            playSoundEffect('punch.mp3');
            log.innerHTML += `\n😢 被弾ダメージ: ${d}`;
            triggerEffect("player", d);
            if(myFluffyCategoryE === 'seafood') { log.innerHTML += `\n🥚 ${getBarrierLabel(myC)}により${FLUFFY_CATEGORY_LABEL_E['seafood']}からの攻撃を軽減`; }
            updateHP(pHP,oHP,oppC.hp,myC.hp);
            battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
    ...
    // シャーク次男のターン。長男と同様、独立した判定で行動有無を決める、それ自体1つの独立したターン。
    // 行動しない場合は表示なしでプレイヤーのターンへ制御を戻す。
    function runSecondSonTurn() {
        if(battleAborted) return;
        const acts = Math.random() >= (isEnraged ? 0.30 : 0.80); // 通常20%／怒り70%で行動
        if(!acts) { afterEnemyPhase(); return; }
        log.innerHTML = `⚠️ シャーク次男が割り込んできた！`;
        playTone(300,'triangle',0.08);
        setTimeout(()=>{
            // 通常モード：行動20%を攻撃10%／回復10%で折半。怒りモード：行動70%を攻撃30%／回復40%で配分（40/70）。
            const healChance = isEnraged ? (40/70) : 0.5;
            if(Math.random() < healChance) {
                // 痛いの痛いの飛んでいけ！：水面カレーシャークのHPを600回復
                oHP = Math.min(oppC.hp, oHP + 600);
                playSoundEffect('healing.mp3');
                log.innerHTML += `\n🩹 「痛いの痛いの飛んでいけ！」水面カレーシャークのHPが600回復！`;
                updateHP(pHP,oHP,oppC.hp,myC.hp);
                checkEnragedTransition();
                battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                round++; setTimeout(afterEnemyPhase, battleDelay(900));
                return;
            }
            if(myC.isKaiTate && rollShieldGuard(myC.spd)) {
                playSoundEffect('sound/guard.mp3');
            ...
            let d = Math.max(6, Math.round(SHARK_SON_DATA.second.atk*(0.9+Math.random()*0.2)) - Math.floor(myC.def/2));
            if(myFluffyCategoryE === 'seafood') d = Math.round(d*0.7);
            pHP = Math.max(0, pHP - d);
            playSoundEffect('punch.mp3');
            log.innerHTML += `\n😢 被弾ダメージ: ${d}`;
    ...
    function eventStep(){
        ...
        if(myTurn){
            // 鮫ダンスで踊らされている：この1ターンは行動不能
            if(playerSkipNextTurn) {
                playerSkipNextTurn = false;
                log.innerHTML = `💃 ${playerName}は一緒に鮫ダンスを踊っている。`;
                playSoundEffect('sound/dance.mp3');
                setTimeout(()=>{
                    updateHP(pHP,oHP,oppC.hp,myC.hp);
                    battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
            ...
            if((myC.isWanpaku || myC.isTonTonTon) && isWanpakuMiss()){
                ...
                // ⛩️京野菜カレー専用特技「ぶぶ漬けでも、いかがどす？」：熱々ブレスと同じDEF無視固定80ダメージ
                log.innerHTML+=`\n🍵 ${playerName}の「ぶぶ漬けでも、いかがどす？」！`;
                playTanemakiAnimation(KYOYASAI_SKILL_CONFIG, false, function(){
                    oHP=Math.max(1,oHP-80);
                    eventTotalDamageThisBattle += 80; // 与えたダメージをそのまま積算（敵の回復で相殺されない）
                    log.innerHTML+=`\n🔥 DEF無視80ダメージ！`;
                    playSoundEffect('breath.mp3');
                    triggerEffect("enemy",80);
                    updateHP(pHP,oHP,oppC.hp,myC.hp);
                    checkEnragedTransition();
                    battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=false; round++; setTimeout(eventStep,battleDelay(1100));
                });
                ...
                // 🧀チーズまみれカレー専用特技「チーズキャノン」：DEF無視でATK1〜2倍のダメージ
                log.innerHTML+=`\n🧀 ${playerName}の「チーズキャノン」！`;
                playTanemakiAnimation(CHEESE_SKILL_CONFIG, false, function(){
                    const dChe = Math.round(myC.atk * (1 + Math.random()));
                    oHP=Math.max(1,oHP-dChe);
                    eventTotalDamageThisBattle += dChe;
                    log.innerHTML+=`\n🔥 DEF無視${dChe}ダメージ！`;
                    playSoundEffect('sound/gankowari.mp3');
                    triggerEffect("enemy",dChe);
                    updateHP(pHP,oHP,oppC.hp,myC.hp);
                    checkEnragedTransition();
                    battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=false; round++; setTimeout(eventStep,battleDelay(1100));
                });
                ...
                // 🍤小えびスピードカレー専用特技「小エビよ！突撃だ！」：通常攻撃をATKではなくSPDの数値で行う
                log.innerHTML+=`\n🦐 ${playerName}の「小エビよ！突撃だ！」！`;
                playTanemakiAnimation(SHRIMP_SKILL_CONFIG, false, function(){
                    const isCritShr = Math.random() < getCritRate(myC.spd, oppC.spd);
                    let dShr = isCritShr ? Math.max(8, myC.spd) : Math.max(8, myC.spd - Math.floor(oppC.def/2));
                    dShr = Math.round(dShr*(0.9+Math.random()*0.2));
                    oHP=Math.max(1,oHP-dShr);
                    eventTotalDamageThisBattle += dShr;
                    playSoundEffect('sound/ryusuikarei.mp3');
                    if(isCritShr){ log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 ダメージ: ${dShr}`; triggerEffect("enemy-crit",dShr); }
                    else{ log.innerHTML+=`\n💥 ダメージ: ${dShr}`; triggerEffect("enemy",dShr); }
                    updateHP(pHP,oHP,oppC.hp,myC.hp);
                    checkEnragedTransition();
                    battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=false; round++; setTimeout(eventStep,battleDelay(1100));
                });
                ...
                setTimeout(()=>{
                    ...
                    if(myC.isPoisonApple && Math.random() < 0.5) {
                        if(!isOppPoisoned) { isOppPoisoned = true; eventPoisonLevel = 1; log.innerHTML+=`\n☠️ ${EVENT_MONSTER_DATA.name} は毒にかかった！`; if(liveBattleOppStatusKeys.indexOf('poison')<0) liveBattleOppStatusKeys = liveBattleOppStatusKeys.concat(['poison']); setTimeout(()=>playSoundEffect('poison.mp3'),battleDelay(300)); }
            ...
            // 🍌そんなバナナカレー：プレイヤーが仕掛けたバナナトラップにボスが30%でかかり、このターンは行動不能
            if(myC.isBananaCurry && Math.random() < 0.3) {
                log.innerHTML = `🍌 ${EVENT_MONSTER_DATA.name}はバナナトラップにかかってしまった！`;
                log.innerHTML += `\n🍌 そんなバナナ〜🍌`;
                ...
                if(er<0.60){
                    ...
                    if(myC.isKaiTate && rollShieldGuard(myC.spd)){
                        setTimeout(()=>{
                            ...
                            triggerEffect("player","Guard");
                            updateHP(pHP,oHP,oppC.hp,myC.hp);
                            battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); round++; setTimeout(runLongSonTurn,battleDelay(900));
                        },battleDelay(400));
                        return;
                    }
                    setTimeout(()=>{
                    ...
                    // カレー放水（効果は熱々ブレスと同じ。DEF無視固定ダメージ・ホームラン対象）
                    if(myC.isHomerun && isHomerunReflect(myC.spd, oppC.spd)){
                        log.innerHTML+=`\n🏏 ホームラン！カレー放水を打ち返して無効化！`;
                    ...
                    // 鮫ダンス：プレイヤーの次のターンを1回休みにする
                    setTimeout(()=>{
                        playerSkipNextTurn = true;
                        playSoundEffect('sound/dance.mp3');
                        log.innerHTML+=`\n🦈 ${EVENT_MONSTER_DATA.name}「Shall we 鮫ダンス？」`;
                        updateHP(pHP,oHP,oppC.hp,myC.hp);
                        battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); round++; setTimeout(runLongSonTurn,battleDelay(900));
                    },battleDelay(400));
                }
            } else {
                // ===== 怒りモード（HP50%以下）：通常攻撃20% / 怒りSpicyHit20% / カレー放水30% / 鮫ダンス30% =====
                if(er<0.20){
                    if(isOppIlluded && Math.random() < getIllusionMissRate(oppC.spd, myC.spd)){
                        playSoundEffect('sound/miss.mp3');
                        log.innerHTML+=`\n💨 幻惑で${EVENT_MONSTER_DATA.name}の攻撃が外れた！`;
...
function eventBattleFinish(pHP, oHP, myC, wasBlownAway){
    ...
    // 共有HPをFirebaseに保存
    if(database){
        database.ref('events/tokumori002/currentHp').set(oHP);
        if(playerId) {
            database.ref('events/tokumori002/participants/'+playerId).set({ name: playerName, lastPlayed: Date.now() });
            // このバトルで実際に与えたダメージ量をランキング用に加算（敵の回復で相殺しない）
            if(eventTotalDamageThisBattle > 0) {
                database.ref('events/tokumori002/damages/'+playerId).transaction(function(cur){ return (cur||0) + eventTotalDamageThisBattle; }).then(function(result){
                    if(!result.committed) logSyncEvent('eventDamageWriteNotCommitted', 'tokumori002 dmg=' + eventTotalDamageThisBattle);
                }).catch(function(e){
                    logSyncEvent('eventDamageWriteFailed', 'tokumori002 dmg=' + eventTotalDamageThisBattle + ' err=' + (e && e.message ? e.message : String(e)));
                });
            }
        }
    }
    ...
    // イベント戦は常に敗北扱いの終了処理のため、食品サンプルによる身代わり判定を行う
    if(!tryConsumeCurryOnLoss(selectedCurryIndex)) { selectedCurryIndex=curryStock.length?0:-1; }
    saveGame(); updateFridgeUI(); updateCookSelects(); updateShopButtons();
}
...
// 咖喱図書館：本のボス戦専用。出撃カレーが力尽きた時、ストックの他のカレーの一覧を表示して選ばせる
// （フェスの仕組みを参考にしているが、配色は指定によりフェスのオレンジ系ではなく白黒のシンプルな
// 配色にしてある＝bookBattleCurrySwapOverlay/book-battle-swap-btn側のCSSで対応）。
function promptBookBossCurrySwap() {
    const list = document.getElementById('bookBattleCurrySwapList');
    if(list) {
        list.innerHTML = curryStock.filter(c => !c.isDelivering).map(function(c) {
            const idx = curryStock.indexOf(c);
            const iconsHtml = curryIconsHTML(c.materials, c.spice, '16px');
            return '<button class="book-battle-swap-btn" onclick="chooseBookBossCurrySwap(' + idx + ')">' + iconsHtml + ' ' + c.name + '（HP:' + statDisplayWithTableware('hp', c.hp) + '）</button>';
        }).join('');
    }
    const overlay = document.getElementById('bookBattleCurrySwapOverlay');
    if(overlay) overlay.style.display = 'flex';
}
function chooseBookBossCurrySwap(idx) {
    const overlay = document.getElementById('bookBattleCurrySwapOverlay');
    if(overlay) overlay.style.display = 'none';
    const resolve = bookBossCurrySwapResolve;
    bookBossCurrySwapResolve = null;
    if(resolve && typeof resolve.onSwap === 'function') resolve.onSwap(idx);
}
function giveUpBookBossBattle() {
    const overlay = document.getElementById('bookBattleCurrySwapOverlay');
    if(overlay) overlay.style.display = 'none';
    const resolve = bookBossCurrySwapResolve;
    bookBossCurrySwapResolve = null;
    if(resolve && typeof resolve.onGiveUp === 'function') resolve.onGiveUp();
}
...
function startBattleScene(oppN, oppC, myC) {
    ...
    if (!isBotMatch) {
        ...
        // materialsが指定されていれば食材アイコンを優先表示（咖喱図書館のボス戦等）。無ければ従来通り絵文字。
        const botCurryIcons = oppC.materials ? curryIconsHTML(oppC.materials, oppC.spice, '22px') : '';
        if(botCurryIcons) {
            document.getElementById("oppCurryEmojiText").innerHTML = botCurryIcons;
        } else {
            document.getElementById("oppCurryEmojiText").innerText = oppC.emoji || '';
        }
    }
    ...
    let seaHealUsed = false; // 海鮮カレーHP回復フラグ
    // 日替わりカレー「水曜日の妄想」専用：敵側の海鮮自動回復（既存はプレイヤー側のisSeafoodしか対応していなかったため新規追加）
    let oppSeaHealUsed = false;
    // 咖喱図書館：book5ボス専用「ぷにぷにプレゼント」で付与される、プレイヤー側の1回休みフラグ
    // （フェス/タッグ戦のfestPlayerSkipTurn/skipNextTurnと同じ考え方を1v1戦闘用に持ち込んだもの）
    let mySkipNextTurn = false;
    const log = document.getElementById("battleLog"); log.innerHTML = `⚔️ ${oppN} との決闘開始！
`;

    // BGM（特殊カレーBGMが優先、なければ強敵BGM）
    stopBattleBGM();
    if(isBotMatch && oppC.isBookBoss) setTimeout(()=>playBattleBGM("sound/book_battle.mp3"),300); // 咖喱図書館：本のボス戦専用BGM
    else if(myC.isTriCaviar || oppC.isTriCaviar) setTimeout(()=>playBattleBGM("Specialdinner.mp3"),300);
    else if(myC.isMargherita || oppC.isMargherita) setTimeout(()=>playBattleBGM("Guardare_il_cielo.mp3"),300);
    else if(myC.isKyoyasai || oppC.isKyoyasai) setTimeout(()=>playBattleBGM("sound/Kyoto.mp3"),300);
    ...
    // バトル背景（PC戦のみ。初級/中級で背景画像を切り替える。咖喱図書館のボス戦は背景画像なし＝透過のまま）
    const stageElForBg = document.getElementById("battleStage");
    if(stageElForBg) {
        stageElForBg.classList.remove("battle-bg-easy", "battle-bg-hard", "battle-bg-book");
        if(isBotMatch && oppC.isBookBoss) stageElForBg.classList.add("battle-bg-book");
        else if(isBotMatch) stageElForBg.classList.add(oppC.isHardBot ? "battle-bg-hard" : "battle-bg-easy");
    }
    // 咖喱図書館のボス戦：敵アイコンの丸トリミングを解除するための目印クラス
    const arenaElForBook = document.getElementById("battleArena");
    if(arenaElForBook) arenaElForBook.classList.toggle("book-boss-mode", !!(isBotMatch && oppC.isBookBoss));
    ...
    // 咖喱図書館：book3ボス専用「系統バリア」（戦闘中に自身へ一時的に付与する、特定系統からのダメージ半減）。
    // ふわとろオム等（oppFluffyCategoryB）と違い戦闘開始時ではなく戦闘中に発動し、3ターンで自然消滅する。
    let oppBarrierCategory = null;
    let oppBarrierTurnsLeft = 0;
    // 名前横の状態アイコン：毒・幻惑（この時点で最終確定済み）、ラタトゥイユの特性（毎ターン回復／肉系軽減）、
    // ふわとろオム／世界三大珍味バリア（軽減対象カテゴリ）をまとめて反映する
    const myFluffyIconKeyB = fluffyCategoryToIconKey(myFluffyCategoryB);
    const oppFluffyIconKeyB = fluffyCategoryToIconKey(oppFluffyCategoryB);
    liveBattleMyStatusKeys = (myIsIlluded ? ['illusion'] : []).concat(isPP ? ['poison'] : []).concat(myC.isRatatouille ? ['hpRegen','meatReduce'] : []).concat(myFluffyIconKeyB ? [myFluffyIconKeyB] : []);
    liveBattleOppStatusKeys = (oppIsIlluded ? ['illusion'] : []).concat(isOP ? ['poison'] : []).concat(oppC.isRatatouille ? ['hpRegen','meatReduce'] : []).concat(oppFluffyIconKeyB ? [oppFluffyIconKeyB] : []);
    updateBattleStatusIconsFor('pOwnerText', liveBattleMyStatusKeys);
    updateBattleStatusIconsFor('oppOwnerText', liveBattleOppStatusKeys);
    let myTurn;
...
💚 ${playerName} はネバネバにかかった！${oppN} の先攻攻撃！`;}
    ...
    function step(){
        ...
        if(oHP<=0||round>50){setTimeout(()=>done(pHP,oHP,myC,oppC,false),battleDelay(800));return;}
        if(pHP<=0){
            // 咖喱図書館：本のボス戦専用。出撃カレーが力尽きても、ストックに他のカレーがあれば
            // 次のカレーを出すか、あきらめるか選べるようにする（フェスの仕組みと同様の考え方）。
            if(isBotMatch && oppC.isBookBoss && curryStock.some((c,idx)=>!c.isDelivering && idx!==selectedCurryIndex)){
                curryStock.splice(selectedCurryIndex,1);
                selectedCurryIndex=-1;
                saveGame();
                log.innerHTML+=`\n💔 ${myC.name} は力尽きた…`;
                updateHP(pHP,oHP,oppC.hp,myC.hp);
                battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                bookBossCurrySwapResolve = {
                    onSwap: function(newIdx){
                        const newCurry = curryStock[newIdx];
                        if(!newCurry){ setTimeout(()=>done(pHP,oHP,myC,oppC,false),battleDelay(400)); return; }
                        selectedCurryIndex = newIdx;
                        myC = applyTablewareModifiers(newCurry);
                        pHP = myC.hp;
                        // 前のカレーが受けていたデバフ（毒・幻惑）は新しいカレーには引き継がない
                        isPP=false; poisonLevelP=0; myIsIlluded=false;
                        liveBattleMyStatusKeys=[]; updateBattleStatusIconsFor('pOwnerText', []);
                        const pOwnerEl2=document.getElementById('pOwnerText');
                        if(pOwnerEl2) pOwnerEl2.classList.remove('name-poisoned','name-illuded');
                        document.getElementById("playerCurryNameText").innerText = myC.name;
                        const myIconsHtml2 = curryIconsHTML(myC.materials, myC.spice, '22px');
                        const pCurryIconsEl2 = document.getElementById("pCurryIcons");
                        if(pCurryIconsEl2) pCurryIconsEl2.innerHTML = myIconsHtml2 || myC.visual || '';
                        log.innerHTML+=`\n🍛 ${myC.name} が代わりに出撃した！`;
                        updateHP(pHP,oHP,oppC.hp,myC.hp);
                        battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                        round++; setTimeout(step,battleDelay(900));
                    },
                    onGiveUp: function(){
                        setTimeout(()=>done(pHP,oHP,myC,oppC,false),battleDelay(400));
                    }
                };
                promptBookBossCurrySwap();
                return;
            }
            setTimeout(()=>done(pHP,oHP,myC,oppC,false),battleDelay(800));return;
        }
        if(myTurn){
            // book5ボス「ぷにぷにプレゼント」で付与された1回休み：この行動を消費して1回休みにする
            if(mySkipNextTurn){
                mySkipNextTurn = false;
                log.innerHTML=`😵 ${playerName}は1回休み！`;
            ...
            // 🍌そんなバナナカレー：相手が仕掛けたバナナトラップに30%でかかり、このターンは行動不能
            if(oppC.isBananaCurry && Math.random() < 0.3) {
                log.innerHTML = `🍌 ${playerName}はバナナトラップにかかってしまった！`;
                log.innerHTML += `\n🍌 そんなバナナ〜🍌`;
                playSoundEffect('sound/manuke.mp3');
                setTimeout(()=>{
                    updateHP(pHP,oHP,oppC.hp,myC.hp);
            ...
            if(myIsIlluded&&Math.random()<getIllusionMissRate(myC.spd,oppC.spd)){
                ...
                // ⛩️京野菜カレー専用特技「ぶぶ漬けでも、いかがどす？」：熱々ブレスと同じDEF無視固定80ダメージ
                const willReflectKyo = oppC.isHomerun && isHomerunReflect(oppC.spd, myC.spd);
                log.innerHTML+=`\n🍵 ${playerName}の「ぶぶ漬けでも、いかがどす？」！`;
                playTanemakiAnimation(Object.assign({}, KYOYASAI_SKILL_CONFIG, { interruptConfig: HOMERUN_ANIM_CONFIG_ENEMY }), willReflectKyo, function(){
                    if(willReflectKyo){
                        log.innerHTML+=`\n🏏 ホームラン！${oppN} がぶぶ漬けを打ち返した！`;
                        updateHP(pHP,oHP,oppC.hp,myC.hp);
                        battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                        myTurn=false;round++;setTimeout(step,battleDelay(1100));
                    } else {
                ...
                // 🧀チーズまみれカレー専用特技「チーズキャノン」：DEF無視でATK1〜2倍のダメージ
                const willReflectChe = oppC.isHomerun && isHomerunReflect(oppC.spd, myC.spd);
                log.innerHTML+=`\n🧀 ${playerName}の「チーズキャノン」！`;
                playTanemakiAnimation(Object.assign({}, CHEESE_SKILL_CONFIG, { interruptConfig: HOMERUN_ANIM_CONFIG_ENEMY }), willReflectChe, function(){
                    if(willReflectChe){
                        log.innerHTML+=`\n🏏 ホームラン！${oppN} がチーズキャノンを打ち返した！`;
                        updateHP(pHP,oHP,oppC.hp,myC.hp);
                        battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                        myTurn=false;round++;setTimeout(step,battleDelay(1100));
                    } else {
                        let dChe = Math.round(myC.atk * (1 + Math.random()));
                        oHP=Math.max(0,oHP-dChe);
                        log.innerHTML+=`\n🔥 DEF無視${dChe}ダメージ！`;
                        playSoundEffect('sound/gankowari.mp3');
                        triggerEffect("enemy",dChe);
                        updateHP(pHP,oHP,oppC.hp,myC.hp);
                ...
                playTanemakiAnimation(SHRIMP_SKILL_CONFIG, false, function(){
                    ...
                    let dShr = isCritShr ? Math.max(8, myC.spd) : Math.max(8, myC.spd - Math.floor(oppC.def/2));
                    dShr = Math.round(dShr*(0.9+Math.random()*0.2));
                    let myFluffyBlockedShr = false;
                    if(oppFluffyCategoryB && curryHasCategory(myC, oppFluffyCategoryB)) { dShr=Math.round(dShr*0.7); myFluffyBlockedShr = true; }
                    oHP=Math.max(0,oHP-dShr);
                    playSoundEffect('sound/ryusuikarei.mp3');
                    if(isCritShr){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 ダメージ: ${dShr}`;triggerEffect("enemy-crit",dShr);}
                    else{log.innerHTML+=`\n💥 ダメージ: ${dShr}`;triggerEffect("enemy",dShr);}
                    if(myFluffyBlockedShr){log.innerHTML+=`\n🥚 ${getBarrierLabel(oppC)}により${FLUFFY_CATEGORY_LABEL_B[oppFluffyCategoryB]}からの攻撃を軽減`;}
                    updateHP(pHP,oHP,oppC.hp,myC.hp);
                    battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                    myTurn=false;round++;setTimeout(step,battleDelay(1100));
                });
                ...
                setTimeout(()=>{if(oppC.isKaiTate&&rollShieldGuard(oppC.spd)){playSoundEffect('sound/guard.mp3');log.innerHTML+=`\n🛡️ 攻撃を貝の盾でガードした。`;triggerEffect("enemy","Guard");updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=false;round++;setTimeout(step,battleDelay(1100));}else{const isCrit=Math.random()<getCritRate(myC.spd,oppC.spd);let d=isCrit?Math.max(8,myC.atk):Math.max(8,myC.atk-Math.floor(oppC.def/2));d=Math.round(d*(0.9+Math.random()*0.2));d=Math.round(d*getWanpakuDamageMultiplier());if(oppC.isRatatouille&&isMeatBasedCurry(myC))d=Math.round(d*0.2);if(oppFluffyCategoryB&&curryHasCategory(myC,oppFluffyCategoryB)){d=Math.round(d*0.7);}if(oppBarrierTurnsLeft>0&&oppBarrierCategory&&curryHasCategory(myC,oppBarrierCategory)){d=Math.round(d*0.5);}oHP=Math.max(0,oHP-d);playSoundEffect('punch.mp3');if(isCrit){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 ダメージ: ${d}`;triggerEffect("enemy-crit",d);}else{log.innerHTML+=`\n💥 ダメージ: ${d}`;triggerEffect("enemy",d);}if(oppFluffyCategoryB&&curryHasCategory(myC,oppFluffyCategoryB)){log.innerHTML+=`\n🥚 ${getBarrierLabel(oppC)}により${FLUFFY_CATEGORY_LABEL_B[oppFluffyCategoryB]}からの攻撃を軽減`;}if(oppBarrierTurnsLeft>0&&oppBarrierCategory&&curryHasCategory(myC,oppBarrierCategory)){log.innerHTML+=`\n🛡️ ${oppN}の系統バリア（${FLUFFY_CATEGORY_LABEL_B[oppBarrierCategory]}）により攻撃を軽減`;}if(myC.isPoisonApple&&Math.random()<0.5){if(!isOP){isOP=true;poisonLevelO=1;log.innerHTML+=`\n☠️ ${oppN} は毒にかかった！`;if(liveBattleOppStatusKeys.indexOf('poison')<0)liveBattleOppStatusKeys=liveBattleOppStatusKeys.concat(['poison']);setTimeout(()=>{playSoundEffect('poison.mp3');},300);}else if(poisonLevelO<6){poisonLevelO++;log.innerHTML+=`\n☠️ 毒のダメージが増幅`;setTimeout(()=>playSoundEffect('poison.mp3'),300);}}updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=false;round++;setTimeout(step,battleDelay(1100));}},battleDelay(400));
            } else if(oppC.isTonTonTon){
                playSoundEffect('pig1.mp3');
                setTimeout(()=>{if(oppC.isKaiTate&&rollShieldGuard(oppC.spd)){playSoundEffect('sound/guard.mp3');log.innerHTML+=`\n🛡️ 攻撃を貝の盾でガードした。`;triggerEffect("enemy","Guard");updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=false;round++;setTimeout(step,battleDelay(1100));}else{const isCrit=Math.random()<getCritRate(myC.spd,oppC.spd);let d=isCrit?Math.max(8,myC.atk):Math.max(8,myC.atk-Math.floor(oppC.def/2));d=Math.round(d*(0.9+Math.random()*0.2));if(myC.isWanpaku)d=Math.round(d*getWanpakuDamageMultiplier());if(oppC.isRatatouille&&isMeatBasedCurry(myC))d=Math.round(d*0.2);if(oppFluffyCategoryB&&curryHasCategory(myC,oppFluffyCategoryB)){d=Math.round(d*0.7);}if(oppBarrierTurnsLeft>0&&oppBarrierCategory&&curryHasCategory(myC,oppBarrierCategory)){d=Math.round(d*0.5);}oHP=Math.max(0,oHP-d);playSoundEffect('punch.mp3');if(isCrit){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 ダメージ: ${d}`;triggerEffect("enemy-crit",d);}else{log.innerHTML+=`\n💥 ダメージ: ${d}`;triggerEffect("enemy",d);}if(oppFluffyCategoryB&&curryHasCategory(myC,oppFluffyCategoryB)){log.innerHTML+=`\n🥚 ${getBarrierLabel(oppC)}により${FLUFFY_CATEGORY_LABEL_B[oppFluffyCategoryB]}からの攻撃を軽減`;}if(oppBarrierTurnsLeft>0&&oppBarrierCategory&&curryHasCategory(myC,oppBarrierCategory)){log.innerHTML+=`\n🛡️ ${oppN}の系統バリア（${FLUFFY_CATEGORY_LABEL_B[oppBarrierCategory]}）により攻撃を軽減`;}if(myC.isPoisonApple&&Math.random()<0.5){if(!isOP){isOP=true;poisonLevelO=1;log.innerHTML+=`\n☠️ ${oppN} は毒にかかった！`;if(liveBattleOppStatusKeys.indexOf('poison')<0)liveBattleOppStatusKeys=liveBattleOppStatusKeys.concat(['poison']);setTimeout(()=>{playSoundEffect('poison.mp3');},300);}else if(poisonLevelO<6){poisonLevelO++;log.innerHTML+=`\n☠️ 毒のダメージが増幅`;setTimeout(()=>playSoundEffect('poison.mp3'),300);}}updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=false;round++;setTimeout(step,battleDelay(1100));}},battleDelay(400));
            } else if(myC.hasGold){
            ...
            } else if(isDailyCurryEvaded(oppC.evasionRate)){
                // 日替わりカレー「日曜日の経験」専用：回避率が高く、通常攻撃が当たらない
                setTimeout(()=>{
                    playSoundEffect('sound/miss.mp3');
                    log.innerHTML+=`\n💨 ${oppN} にひらりと躱された！`;
                    updateHP(pHP,oHP,oppC.hp,myC.hp);
            ...
            // 🍌そんなバナナカレー：自分が仕掛けたバナナトラップに相手が30%でかかり、このターンは行動不能
            if(myC.isBananaCurry && Math.random() < 0.3) {
                log.innerHTML = `🍌 ${oppN}はバナナトラップにかかってしまった！`;
                log.innerHTML += `\n🍌 そんなバナナ〜🍌`;
                playSoundEffect('sound/manuke.mp3');
                setTimeout(()=>{
                    updateHP(pHP,oHP,oppC.hp,myC.hp);
            ...
            // book3ボスの「系統バリア」：カウントダウン（発動したそのターンは減らさず、次のターンから
            // 1ずつ減らして3ターン分プレイヤーの攻撃を軽減した後に自然消滅する）
            if(oppBarrierTurnsLeft>0){oppBarrierTurnsLeft--;if(oppBarrierTurnsLeft<=0){oppBarrierCategory=null;}}
            // 日替わりカレー「水曜日の妄想」専用：HP50%以下で1回だけ自動回復（既存の海鮮カレー(myC.isSeafood)と同じ計算式）
            if(oppC.isSeafood && !oppSeaHealUsed && oHP <= Math.floor(oppC.hp * 0.5)) {
                oppSeaHealUsed = true;
                const oppSeaHeal = Math.round(oppC.hp * (oppC.spd >= myC.spd ? 0.4 : 0.3));
                oHP = Math.min(oppC.hp, oHP + oppSeaHeal);
                log.innerHTML += `\n🌊 ${oppN}は波の音に癒された。HP回復+${oppSeaHeal}`;
                playSoundEffect('healing.mp3');
                triggerEffect("enemy-heal", oppSeaHeal);
                updateHP(pHP, oHP, oppC.hp, myC.hp);
            }
            ...
            if(oppIsIlluded&&Math.random()<getIllusionMissRate(oppC.spd,myC.spd)){
            ...
            } else if((oppC.isWanpaku || oppC.isTonTonTon)&&isWanpakuMiss(oppC.wanpakuMissRate)){
                if(oppC.isTonTonTon) playSoundEffect('pig1.mp3');
                playSoundEffect('sound/miss.mp3');
                log.innerHTML+=`\n💨 相手のわんぱくが暴れすぎて攻撃が外れた！`;
                setTimeout(()=>{updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=true;round++;setTimeout(step,battleDelay(1100));},battleDelay(400));
            } else if(oppC.isBookBoss&&oppC.bookChapterNum===2&&Math.random()<0.4){
                // 咖喱図書館：book2ボス（Memories Book）専用行動。まずこのブロックに入る確率が40%、
                // その中で想いの爆発／想いの大爆発を62.5%/37.5%に振り分けるので、結果として
                // 想いの爆発25%・想いの大爆発15%・通常攻撃60%（else節）になる。
                // なお特技「ホームラン」（プレイヤーの技を打ち返す）はoppC.isHomerunフラグとして
                // 既存の反射ロジックに乗せてあるため、ここでは想いの爆発／大爆発自体の実装のみ行う。
                if(Math.random()<0.625){
                    // 想いの爆発：DEF無視・1倍ダメージ。他のボス特技（熱々ブレス等）と同じ演出
                    // （playTanemakiAnimation）を使い、キャラ画像はボス自身の画像（oppC.visual）をそのまま使う。
                    const willReflectBb1 = myC.isHomerun && isHomerunReflect(myC.spd, oppC.spd);
                    log.innerHTML+=`\n💭 ${oppN} の想いの爆発！`;
                    playTanemakiAnimation({
                        charaImg: oppC.visual,
                        charaTop: '55%',
                        charaWidth: '55%',
                        charaMaxScale: 1.2
                    }, willReflectBb1, function(){
                        if(willReflectBb1){
                            log.innerHTML+=`\n🏏 ホームラン！想いの爆発を打ち返して無効化！`;
                            updateHP(pHP,oHP,oppC.hp,myC.hp); battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=true; round++; setTimeout(step,battleDelay(1100));
                        } else {
                            let d=Math.max(8,Math.round(oppC.atk*(0.9+Math.random()*0.2))); // DEF無視（myC.defを引かない）
                            pHP=Math.max(0,pHP-d);
                            playSoundEffect('sound/gankowari.mp3');
                            log.innerHTML+=`\n💭🔥 想いの爆発！DEF無視ダメージ: ${d}`;
                            triggerEffect("player",d);
                    ...
                    // 想いの大爆発：DEF無視・1.5倍ダメージ＋自身にも反動（与えたダメージの10%）。
                    // こちらも他のボス特技と同じ演出＋専用SE（sound/gankowari.mp3）を使う。
                    const willReflectBb2 = myC.isHomerun && isHomerunReflect(myC.spd, oppC.spd);
                    log.innerHTML+=`\n💭 ${oppN} の想いの大爆発！`;
                    playTanemakiAnimation({
                        charaImg: oppC.visual,
                        charaTop: '55%',
                        charaWidth: '55%',
                        charaMaxScale: 1.2
                    }, willReflectBb2, function(){
                        if(willReflectBb2){
                            log.innerHTML+=`\n🏏 ホームラン！想いの大爆発を打ち返して無効化！`;
                            updateHP(pHP,oHP,oppC.hp,myC.hp); battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=true; round++; setTimeout(step,battleDelay(1100));
                        } else {
                            let d=Math.max(12,Math.round(oppC.atk*1.5*(0.9+Math.random()*0.2))); // DEF無視（myC.defを引かない）
                            const selfDmg=Math.max(1,Math.round(d*0.1));
                            pHP=Math.max(0,pHP-d);
                            oHP=Math.max(0,oHP-selfDmg);
                            playSoundEffect('sound/gankowari.mp3');
                            log.innerHTML+=`\n💭💥 想いの大爆発！DEF無視ダメージ: ${d}`;
                            log.innerHTML+=`\n🔥 反動で${oppN}にも${selfDmg}ダメージ！`;
                            triggerEffect("player",d);
                            setTimeout(()=>triggerEffect("enemy",selfDmg),200);
                            updateHP(pHP,oHP,oppC.hp,myC.hp); battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=true; round++; setTimeout(step,battleDelay(1100));
                        }
                    });
                    return;
                }
            } else if(oppC.isBookBoss&&oppC.bookChapterNum===3&&Math.random()<0.4){
                // 咖喱図書館：book3ボス（Memories Book）専用行動。まずこのブロックに入る確率が40%、
                // その中で種連続発射／系統バリアを62.5%/37.5%に振り分けるので、結果として
                // 種連続発射25%・系統バリア15%・通常攻撃60%（else節）になる。
                // なお特技「わんぱく」はoppC.isWanpakuフラグとして既存のロジックにそのまま乗せてあるため、
                // 通常攻撃（else節）に既存のミス判定／ダメージ幅増加が自動的に適用される。
                if(Math.random()<0.625){
                    // 種連続発射（book1と同じ演出・ダメージ計算）
                    if(myC.isHomerun&&isHomerunReflect(myC.spd,oppC.spd)){
                        log.innerHTML+=`\n🏏 ホームラン！種連続発射を打ち返して無効化！`;
                        playBattleSkillAnimation(HOMERUN_ANIM_CONFIG, function(){ updateHP(pHP,oHP,oppC.hp,myC.hp); battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=true; round++; setTimeout(step,battleDelay(1100)); });
                        return;
                    ...
                    // 系統バリア：肉／海鮮／野菜／果物からランダムに1系統を選び、自身に3ターンの
                    // ダメージ半減バリアを付与する（攻撃はせず、この技だけでターンを消費する）
                    setTimeout(()=>{
                        oppBarrierCategory = FLUFFY_CATEGORY_KEYS_B[Math.floor(Math.random()*FLUFFY_CATEGORY_KEYS_B.length)];
                        oppBarrierTurnsLeft = 3;
                        playSoundEffect('healing.mp3');
                        log.innerHTML+=`\n🛡️ ${oppN}は${FLUFFY_CATEGORY_LABEL_B[oppBarrierCategory]}バリアを展開した！（3ターン）`;
                        updateHP(pHP,oHP,oppC.hp,myC.hp);
                        battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                        myTurn=true;round++;setTimeout(step,battleDelay(1100));
                    },battleDelay(400));
                }
            } else if(oppC.isBookBoss&&oppC.bookChapterNum===4&&Math.random()<0.55){
                // 咖喱図書館：book4ボス（Memories Book）専用行動。まずこのブロックに入る確率が55%、その中で
                // 想いの爆発／念の方出／系統バリアを25/55・15/55・15/55に振り分けるので、結果として
                // 想いの爆発25%・念の方出15%・系統バリア15%・通常攻撃45%（else節）になる。
                const bb4Roll = Math.random();
                if(bb4Roll < 25/55){
                    // 想いの爆発：DEF無視・1倍ダメージ（book2と同じ演出＋SE）
                    const willReflectBb4a = myC.isHomerun && isHomerunReflect(myC.spd, oppC.spd);
                    log.innerHTML+=`\n💭 ${oppN} の想いの爆発！`;
                    playTanemakiAnimation({
                        charaImg: oppC.visual,
                        charaTop: '55%',
                    ...
                    return;
                } else if(bb4Roll < 40/55){
                    // 念の方出：固定80ダメージ（DEF無視。ドラゴン料理長の熱々ブレスと同じ形式の固定ダメージ技）
                    const willReflectBb4b = myC.isHomerun && isHomerunReflect(myC.spd, oppC.spd);
                    log.innerHTML+=`\n👁️ ${oppN} の念の方出！`;
                    playTanemakiAnimation({
                        charaImg: oppC.visual,
                        charaTop: '55%',
                    ...
                    // 系統バリア（book3と同じ仕組み：肉/海鮮/野菜/果物からランダムに1系統を選び、
                    // 自身に3ターンのダメージ半減バリアを付与。攻撃はせずこの技だけでターンを消費する）
                    setTimeout(()=>{
                        oppBarrierCategory = FLUFFY_CATEGORY_KEYS_B[Math.floor(Math.random()*FLUFFY_CATEGORY_KEYS_B.length)];
                        oppBarrierTurnsLeft = 3;
                        playSoundEffect('healing.mp3');
            ...
            } else if(oppC.isBookBoss&&oppC.bookChapterNum===5&&Math.random()<0.45){
                // 咖喱図書館：book5ボス（Memories Book）専用行動。まずこのブロックに入る確率が45%、その中で
                // ぷにぷにプレゼント／ぼーっとするを25/45・20/45に振り分けるので、結果として
                // ぷにぷにプレゼント25%・ぼーっとする20%・通常攻撃55%（else節）になる。
                // なお特技「ホームラン」（プレイヤーの技を打ち返す）はoppC.isHomerunフラグとして
                // 既存の反射ロジックに乗せてある。
                if(Math.random() < 25/45){
                    // ぷにぷにプレゼント（カレーフェス実装済みの技を1v1戦闘に移植）：
                    // 通常攻撃の1.2倍ダメージ＋プレイヤーを1回休みにする
                    if(myC.isHomerun&&isHomerunReflect(myC.spd,oppC.spd)){
                        log.innerHTML+=`\n🏏 ホームラン！ぷにぷにプレゼントを打ち返して無効化！`;
                        playBattleSkillAnimation(HOMERUN_ANIM_CONFIG, function(){ updateHP(pHP,oHP,oppC.hp,myC.hp); battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=true; round++; setTimeout(step,battleDelay(1100)); });
                        return;
                    } else {
                        setTimeout(()=>{
                            ...
                            let d=isCrit?Math.max(8,oppC.atk):Math.max(8,oppC.atk-Math.floor(myC.def/2));
                            d=Math.round(d*(0.9+Math.random()*0.2));
                            d=Math.round(d*1.2); // ぷにぷにプレゼント：1.2倍ダメージ
                            pHP=Math.max(0,pHP-d);
                            playSoundEffect('sound/punipuni.mp3');
                            if(isCrit){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 ぷにぷにプレゼント！ダメージ: ${d}`;triggerEffect("player-crit",d);}else{log.innerHTML+=`\n🎁 ぷにぷにプレゼント！ダメージ: ${d}`;triggerEffect("player",d);}
                            mySkipNextTurn = true;
                            log.innerHTML+=`\n😵 ${playerName}は1回休み！`;
                            updateHP(pHP,oHP,oppC.hp,myC.hp);
                            battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                    ...
                    // ぼーっとする（悪ガキサタン君と同じ仕組み）：自分の毒・幻惑を解除する
                    setTimeout(()=>{
                        isOP=false; poisonLevelO=0; oppIsIlluded=false;
                        liveBattleOppStatusKeys = liveBattleOppStatusKeys.filter(k=>k!=='poison'&&k!=='illusion');
                        updateBattleStatusIconsFor('oppOwnerText', liveBattleOppStatusKeys);
                        log.innerHTML+=`\n😶 ${oppN} はなんだか健康になった`;
                        playSoundEffect('poincyo.mp3');
                        updateHP(pHP,oHP,oppC.hp,myC.hp);
            ...
            } else if(oppC.isBookBoss&&oppC.bookChapterNum===6&&Math.random()<0.55){
                // 咖喱図書館：book6ボス（Memories Book）専用行動。まずこのブロックに入る確率が55%、その中で
                // 玉ねぎ汁ピッピ／想いの大爆発／ぼーっとするを25/55・20/55・10/55に振り分けるので、結果として
                // 玉ねぎ汁ピッピ25%・想いの大爆発20%・ぼーっとする10%・通常攻撃45%（else節）になる
                // （ユーザー指定の合計が110%になっていたため、通常攻撃は残り45%として実装）。
                const bb6Roll = Math.random();
                if(bb6Roll < 25/55){
                    // 玉ねぎ汁ピッピ：通常攻撃の0.5倍ダメージ＋プレイヤーを幻惑状態にする
                    if(myC.isHomerun&&isHomerunReflect(myC.spd,oppC.spd)){
                        log.innerHTML+=`\n🏏 ホームラン！玉ねぎ汁ピッピを打ち返して無効化！`;
                        playBattleSkillAnimation(HOMERUN_ANIM_CONFIG, function(){ updateHP(pHP,oHP,oppC.hp,myC.hp); battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=true; round++; setTimeout(step,battleDelay(1100)); });
                        return;
                    } else {
                        setTimeout(()=>{
                            ...
                            let d=isCrit?Math.max(8,oppC.atk):Math.max(8,oppC.atk-Math.floor(myC.def/2));
                            d=Math.round(d*(0.9+Math.random()*0.2));
                            d=Math.round(d*0.5); // 玉ねぎ汁ピッピ：0.5倍ダメージ
                            pHP=Math.max(0,pHP-d);
                            playSoundEffect('punch.mp3');
                            if(isCrit){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 玉ねぎ汁ピッピ！ダメージ: ${d}`;triggerEffect("player-crit",d);}else{log.innerHTML+=`\n🧅 玉ねぎ汁ピッピ！ダメージ: ${d}`;triggerEffect("player",d);}
                            // 幻惑付与（1v1戦闘中に相手の攻撃で幻惑状態になる初のケース。既存のmyIsIlluded判定・状態アイコンをそのまま使う）
                            if(!myIsIlluded){
                                myIsIlluded = true;
                                log.innerHTML+=`\n😵‍💫 ${playerName}は幻惑状態になった！`;
                                if(liveBattleMyStatusKeys.indexOf('illusion')<0) liveBattleMyStatusKeys = liveBattleMyStatusKeys.concat(['illusion']);
                                updateBattleStatusIconsFor('pOwnerText', liveBattleMyStatusKeys);
                            }
                            updateHP(pHP,oHP,oppC.hp,myC.hp);
                            battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                ...
                } else if(bb6Roll < 45/55){
                    // 想いの大爆発：DEF無視・1.5倍ダメージ＋自身にも反動（book2/4と同じ演出＋SE）
                    const willReflectBb6 = myC.isHomerun && isHomerunReflect(myC.spd, oppC.spd);
                    log.innerHTML+=`\n💭 ${oppN} の想いの大爆発！`;
                    playTanemakiAnimation({
                        charaImg: oppC.visual,
                        charaTop: '55%',
                    ...
                    // ぼーっとする（book5と同じ仕組み）：自分の毒・幻惑を解除する
                    setTimeout(()=>{
                        isOP=false; poisonLevelO=0; oppIsIlluded=false;
                        liveBattleOppStatusKeys = liveBattleOppStatusKeys.filter(k=>k!=='poison'&&k!=='illusion');
                        updateBattleStatusIconsFor('oppOwnerText', liveBattleOppStatusKeys);
            ...
            } else if(oppC.isBookBoss&&oppC.bookChapterNum===7&&Math.random()<0.5){
                // 咖喱図書館：book7ボス（Memories Book）専用行動。まずこのブロックに入る確率が50%、その中で
                // チキンヌンチャク／高速連撃／Don't think. Feelを40/50・30/50・30/50に振り分けるので、結果として
                // チキンヌンチャク20%・高速連撃15%・Don't think. Feel15%・通常攻撃50%（else節）になる。
                const bb7Roll = Math.random();
                if(bb7Roll < 0.4){
                    // チキンヌンチャク：通常攻撃1発（等倍）＋その0.5倍の追撃1発。それぞれ独立してクリティカル判定。打撃音2回。
                    setTimeout(()=>{
                        log.innerHTML+=`\n🥢 ${oppN} のチキンヌンチャク！`;
                        const bb7NunchakuHit = (mult, isSecond) => {
                            const isCrit=Math.random()<getCritRate(oppC.spd,myC.spd);
                            let base=Math.max(8,Math.round(oppC.atk*mult));
                            let d=isCrit?base:Math.max(8,base-Math.floor(myC.def/2));
                            d=Math.round(d*(0.9+Math.random()*0.2));
                            if(myC.isRatatouille&&(isMeatBasedCurry(oppC)||isMeatBasedBot(oppN)))d=Math.round(d*0.2);
                            if(myFluffyCategoryB&&curryHasCategory(oppC,myFluffyCategoryB)){d=Math.round(d*0.7);}
                            pHP=Math.max(0,pHP-d);
                            playSoundEffect('punch.mp3');
                            if(isCrit){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 ダメージ: ${d}`;triggerEffect("player-crit",d);}else{log.innerHTML+=`\n😢 被弾ダメージ: ${d}`;triggerEffect("player",d);}
                            if(myFluffyCategoryB&&curryHasCategory(oppC,myFluffyCategoryB)){log.innerHTML+=`\n🥚 ${getBarrierLabel(myC)}により${FLUFFY_CATEGORY_LABEL_B[myFluffyCategoryB]}からの攻撃を軽減`;}
                            if(isSecond){
                                updateHP(pHP,oHP,oppC.hp,myC.hp);
                                battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                                myTurn=true;round++;setTimeout(step,battleDelay(1100));
                        ...
                        bb7NunchakuHit(1, false);
                        setTimeout(()=>{ bb7NunchakuHit(0.5, true); }, battleDelay(350));
                    },battleDelay(400));
                } else if(bb7Roll < 0.7){
                    // 高速連撃：種連続発射をベースに、ヒット数を1〜8回に変更
                    if(myC.isHomerun&&isHomerunReflect(myC.spd,oppC.spd)){
                        log.innerHTML+=`\n🏏 ホームラン！高速連撃を打ち返して無効化！`;
                        playBattleSkillAnimation(HOMERUN_ANIM_CONFIG, function(){ updateHP(pHP,oHP,oppC.hp,myC.hp); battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=true; round++; setTimeout(step,battleDelay(1100)); });
                        return;
                    } else {
                        const hits=Math.floor(Math.random()*8)+1;let damages=[];let spicyCount=0;
                        for(let i=0;i<hits;i++){let sd=Math.max(2,Math.round(oppC.atk*0.4-Math.floor(myC.def/4)));sd=Math.round(sd*(0.9+Math.random()*0.2));if(Math.random()<getCritRate(oppC.spd,myC.spd)){sd=Math.round(sd*2);spicyCount++;}damages.push(sd);pHP=Math.max(0,pHP-sd);}
                        log.innerHTML+=`\n👊 相手の高速連撃 ${hits}連撃！`+(spicyCount>0?` 🌶️ SpicyHit×${spicyCount}！`:'');
                        setTimeout(()=>playSoundEffect('machine-gun.mp3'),50);
                        damages.forEach((sd,i)=>{setTimeout(()=>{triggerEffect("player",sd);if(i===damages.length-1){log.innerHTML+=` 合計: ${damages.reduce((a,b)=>a+b,0)}`;updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=true;round++;setTimeout(step,battleDelay(1300));}},battleDelay(150+i*250));});
                    }
                } else {
                    // Don't think. Feel（悪ガキサタン君のぼーっとすると同じ仕組み）：自分の毒・幻惑を解除する
                    setTimeout(()=>{
                        isOP=false; poisonLevelO=0; oppIsIlluded=false;
                        liveBattleOppStatusKeys = liveBattleOppStatusKeys.filter(k=>k!=='poison'&&k!=='illusion');
                        updateBattleStatusIconsFor('oppOwnerText', liveBattleOppStatusKeys);
                        log.innerHTML+=`\n😶 ${oppN} は「Don't think. Feel」……何かが吹っ切れたようだ`;
                        playSoundEffect('poincyo.mp3');
                        updateHP(pHP,oHP,oppC.hp,myC.hp);
                        battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                        myTurn=true;round++;setTimeout(step,battleDelay(1100));
            ...
            } else if(oppC.isBookBoss&&oppC.bookChapterNum===8&&Math.random()<0.5){
                // 咖喱図書館：book8ボス（Memories Book）専用行動。まずこのブロックに入る確率が50%、その中で
                // 墨攻撃／触手連撃／系統バリアを40/50・30/50・30/50に振り分けるので、結果として
                // 墨攻撃20%・触手連撃15%・系統バリア15%・通常攻撃50%（else節）になる。
                const bb8Roll = Math.random();
                if(bb8Roll < 0.4){
                    // 墨攻撃：通常攻撃の1.2倍ダメージ＋プレイヤーを幻惑状態にする（打撃音＋genwaku.mp3）
                    if(myC.isHomerun&&isHomerunReflect(myC.spd,oppC.spd)){
                        log.innerHTML+=`\n🏏 ホームラン！墨攻撃を打ち返して無効化！`;
                        playBattleSkillAnimation(HOMERUN_ANIM_CONFIG, function(){ updateHP(pHP,oHP,oppC.hp,myC.hp); battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=true; round++; setTimeout(step,battleDelay(1100)); });
                        return;
                        ...
                        setTimeout(()=>{
                            ...
                            let d=isCrit?Math.max(8,oppC.atk):Math.max(8,oppC.atk-Math.floor(myC.def/2));
                            d=Math.round(d*(0.9+Math.random()*0.2));
                            d=Math.round(d*1.2); // 墨攻撃：1.2倍ダメージ
                            pHP=Math.max(0,pHP-d);
                            playSoundEffect('punch.mp3');
                            playSoundEffect('genwaku.mp3');
                            if(isCrit){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 墨攻撃！ダメージ: ${d}`;triggerEffect("player-crit",d);}else{log.innerHTML+=`\n🖋️ 墨攻撃！ダメージ: ${d}`;triggerEffect("player",d);}
                            // 幻惑付与（book6の玉ねぎ汁ピッピと同じ仕組み）
                            if(!myIsIlluded){
                                myIsIlluded = true;
                                log.innerHTML+=`\n😵‍💫 ${playerName}は幻惑状態になった！`;
                                if(liveBattleMyStatusKeys.indexOf('illusion')<0) liveBattleMyStatusKeys = liveBattleMyStatusKeys.concat(['illusion']);
                    ...
                    // 触手連撃：種連続発射をベースに、ヒット数を1〜10回に変更
                    if(myC.isHomerun&&isHomerunReflect(myC.spd,oppC.spd)){
                        log.innerHTML+=`\n🏏 ホームラン！触手連撃を打ち返して無効化！`;
                        playBattleSkillAnimation(HOMERUN_ANIM_CONFIG, function(){ updateHP(pHP,oHP,oppC.hp,myC.hp); battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=true; round++; setTimeout(step,battleDelay(1100)); });
                        return;
                    } else {
                        const hits=Math.floor(Math.random()*10)+1;let damages=[];let spicyCount=0;
                        for(let i=0;i<hits;i++){let sd=Math.max(2,Math.round(oppC.atk*0.4-Math.floor(myC.def/4)));sd=Math.round(sd*(0.9+Math.random()*0.2));if(Math.random()<getCritRate(oppC.spd,myC.spd)){sd=Math.round(sd*2);spicyCount++;}damages.push(sd);pHP=Math.max(0,pHP-sd);}
                        log.innerHTML+=`\n🐙 相手の触手連撃 ${hits}連撃！`+(spicyCount>0?` 🌶️ SpicyHit×${spicyCount}！`:'');
                        setTimeout(()=>playSoundEffect('machine-gun.mp3'),50);
                        damages.forEach((sd,i)=>{setTimeout(()=>{triggerEffect("player",sd);if(i===damages.length-1){log.innerHTML+=` 合計: ${damages.reduce((a,b)=>a+b,0)}`;updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=true;round++;setTimeout(step,battleDelay(1300));}},battleDelay(150+i*250));});
                    }
                } else {
                    // 系統バリア（book3/4と同じ仕組み：肉/海鮮/野菜/果物からランダムに1系統を選び、
                    // 自身に3ターンのダメージ半減バリアを付与。攻撃はせずこの技だけでターンを消費する）
                    setTimeout(()=>{
                        oppBarrierCategory = FLUFFY_CATEGORY_KEYS_B[Math.floor(Math.random()*FLUFFY_CATEGORY_KEYS_B.length)];
                        oppBarrierTurnsLeft = 3;
            ...
            } else if(oppC.isBookBoss&&(!oppC.bookChapterNum||oppC.bookChapterNum===1)&&Math.random()<0.5){
                // 咖喱図書館：本のボス専用行動（book1系統）。まずこのブロックに入る確率が50%、その中で
                // さらに種連射／毒攻撃を五分五分に振り分けるので、結果として
                // 種連続発射25%・毒攻撃25%・通常攻撃50%（else節）になる。
                if(Math.random()<0.5){
                    // 種連続発射（種まき婆ちゃん専用演出は使わず、通常の種連射と同じダメージ計算のみ）
                    if(myC.isHomerun&&isHomerunReflect(myC.spd,oppC.spd)){
                        log.innerHTML+=`\n🏏 ホームラン！種連続発射を打ち返して無効化！`;
                        playBattleSkillAnimation(HOMERUN_ANIM_CONFIG, function(){ updateHP(pHP,oHP,oppC.hp,myC.hp); battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n")); myTurn=true; round++; setTimeout(step,battleDelay(1100)); });
                        return;
                    ...
                    // 毒攻撃：通常攻撃＋毒付与（isPoisonAppleの毒処理と同じロジックを流用）
                    setTimeout(()=>{
                        const isCrit=Math.random()<getCritRate(oppC.spd,myC.spd);
                        let d=isCrit?Math.max(8,oppC.atk):Math.max(8,oppC.atk-Math.floor(myC.def/2));
                        d=Math.round(d*(0.9+Math.random()*0.2));
                        ...
                        playSoundEffect('punch.mp3');
                        if(isCrit){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 ダメージ: ${d}`;triggerEffect("player-crit",d);}else{log.innerHTML+=`\n😢 被弾ダメージ: ${d}`;triggerEffect("player",d);}
                        if(!isPP){isPP=true;poisonLevelP=1;log.innerHTML+=`\n☠️ ${playerName} は毒にかかった！`;if(liveBattleMyStatusKeys.indexOf('poison')<0)liveBattleMyStatusKeys=liveBattleMyStatusKeys.concat(['poison']);setTimeout(()=>{playSoundEffect('poison.mp3');},300);}else if(poisonLevelP<6){poisonLevelP++;log.innerHTML+=`\n☠️ 毒のダメージが増幅`;setTimeout(()=>playSoundEffect('poison.mp3'),300);}
                        updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=true;round++;setTimeout(step,battleDelay(1100));
                    },battleDelay(400));
                }
                ...
                // 種まき婆ちゃん・日替わりカレー「土曜日の束縛」は専用演出あり
                if(oppN === '種まき婆ちゃん' || (oppC.isDailyCurry && oppC.dailyWeekday === 6)) {
                    const willReflect = myC.isHomerun && isHomerunReflect(myC.spd, oppC.spd);
                    log.innerHTML += `\n🌱 ${oppN} の種連続発射！`;
                    playTanemakiAnimation({
                        charaImg: oppC.isDailyCurry ? getDailyCurryDef(oppC.dailyWeekday).anim : 'battle/bt_tanemaki1.png',
                        charaTop: '55%',
                        charaWidth: '60%',
                        charaMaxScale: 1.2
                    }, willReflect, function(){
                ...
                // 🧀チーズまみれカレー専用特技「チーズキャノン」（敵側）：DEF無視でATK1〜2倍のダメージ
                const willReflectCheB = myC.isHomerun && isHomerunReflect(myC.spd, oppC.spd);
                log.innerHTML+=`\n🧀 ${oppN}の「チーズキャノン」！`;
                playTanemakiAnimation(Object.assign({}, CHEESE_SKILL_CONFIG_ENEMY, { interruptConfig: HOMERUN_ANIM_CONFIG }), willReflectCheB, function(){
                    if(willReflectCheB){
                        log.innerHTML+=`\n🏏 ホームラン！${playerName} がチーズキャノンを打ち返した！`;
                        updateHP(pHP,oHP,oppC.hp,myC.hp);
                        battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                        myTurn=true;round++;setTimeout(step,battleDelay(1100));
                    } else {
                        let dCheB = Math.round(oppC.atk * (1 + Math.random()));
                        pHP=Math.max(0,pHP-dCheB);
                        log.innerHTML+=`\n🔥 DEF無視${dCheB}ダメージ！`;
                        playSoundEffect('sound/gankowari.mp3');
                        triggerEffect("player",dCheB);
                        updateHP(pHP,oHP,oppC.hp,myC.hp);
                        battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                        myTurn=true;round++;setTimeout(step,battleDelay(1100));
                    }
                ...
                // 🍤小えびスピードカレー専用特技「小エビよ！突撃だ！」（敵側）：通常攻撃をATKではなくSPDの数値で行う
                log.innerHTML+=`\n🦐 ${oppN}の「小エビよ！突撃だ！」！`;
                playTanemakiAnimation(SHRIMP_SKILL_CONFIG_ENEMY, false, function(){
                    const isCritShrB = Math.random() < getCritRate(oppC.spd, myC.spd);
                    let dShrB = isCritShrB ? Math.max(8, oppC.spd) : Math.max(8, oppC.spd - Math.floor(myC.def/2));
                    dShrB = Math.round(dShrB*(0.9+Math.random()*0.2));
                    let oppFluffyBlockedShrB = false;
                    if(myFluffyCategoryB && curryHasCategory(oppC, myFluffyCategoryB)) { dShrB=Math.round(dShrB*0.7); oppFluffyBlockedShrB = true; }
                    pHP=Math.max(0,pHP-dShrB);
                    playSoundEffect('sound/ryusuikarei.mp3');
                    if(isCritShrB){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 😢 被弾ダメージ: ${dShrB}`;triggerEffect("player-crit",dShrB);}
                    else{log.innerHTML+=`\n😢 被弾ダメージ: ${dShrB}`;triggerEffect("player",dShrB);}
                    if(oppFluffyBlockedShrB){log.innerHTML+=`\n🥚 ${getBarrierLabel(myC)}により${FLUFFY_CATEGORY_LABEL_B[myFluffyCategoryB]}からの攻撃を軽減`;}
                    updateHP(pHP,oHP,oppC.hp,myC.hp);
                    battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));
                    myTurn=true;round++;setTimeout(step,battleDelay(1100));
                });
                ...
                playSoundEffect('pig1.mp3');
                setTimeout(()=>{if(myC.isKaiTate&&rollShieldGuard(myC.spd)){playSoundEffect('sound/guard.mp3');log.innerHTML+=`\n🛡️ 攻撃を貝の盾でガードした。`;triggerEffect("player","Guard");updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=true;round++;setTimeout(step,battleDelay(1100));}else{const isCrit=Math.random()<getCritRate(oppC.spd,myC.spd);let d=isCrit?Math.max(8,oppC.atk):Math.max(8,oppC.atk-Math.floor(myC.def/2));d=Math.round(d*(0.9+Math.random()*0.2));if(oppC.isWanpaku)d=Math.round(d*getWanpakuDamageMultiplier());if(myC.isRatatouille&&(isMeatBasedCurry(oppC)||isMeatBasedBot(oppN)))d=Math.round(d*0.2);if(myFluffyCategoryB&&curryHasCategory(oppC,myFluffyCategoryB)){d=Math.round(d*0.7);}pHP=Math.max(0,pHP-d);playSoundEffect('punch.mp3');if(isCrit){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 ダメージ: ${d}`;triggerEffect("player-crit",d);}else{log.innerHTML+=`\n😢 被弾ダメージ: ${d}`;triggerEffect("player",d);}if(myFluffyCategoryB&&curryHasCategory(oppC,myFluffyCategoryB)){log.innerHTML+=`\n🥚 ${getBarrierLabel(myC)}により${FLUFFY_CATEGORY_LABEL_B[myFluffyCategoryB]}からの攻撃を軽減`;}if(oppC.isPoisonApple&&Math.random()<0.5){if(!isPP){isPP=true;poisonLevelP=1;log.innerHTML+=`\n☠️ ${playerName} は毒にかかった！`;if(liveBattleMyStatusKeys.indexOf('poison')<0)liveBattleMyStatusKeys=liveBattleMyStatusKeys.concat(['poison']);setTimeout(()=>{playSoundEffect('poison.mp3');},300);}else if(poisonLevelP<6){poisonLevelP++;log.innerHTML+=`\n☠️ 毒のダメージが増幅`;setTimeout(()=>playSoundEffect('poison.mp3'),300);}}updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=true;round++;setTimeout(step,battleDelay(1100));}},battleDelay(400));
            } else if(oppC.isTonTonTon){
                playSoundEffect('pig2.mp3');
                setTimeout(()=>{if(myC.isKaiTate&&rollShieldGuard(myC.spd)){playSoundEffect('sound/guard.mp3');log.innerHTML+=`\n🛡️ 攻撃を貝の盾でガードした。`;triggerEffect("player","Guard");updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=true;round++;setTimeout(step,battleDelay(1100));}else{const isCrit=Math.random()<getCritRate(oppC.spd,myC.spd);let d=isCrit?Math.max(8,oppC.atk):Math.max(8,oppC.atk-Math.floor(myC.def/2));d=Math.round(d*(0.9+Math.random()*0.2));d=Math.round(d*getWanpakuDamageMultiplier());if(myC.isRatatouille&&(isMeatBasedCurry(oppC)||isMeatBasedBot(oppN)))d=Math.round(d*0.2);if(myFluffyCategoryB&&curryHasCategory(oppC,myFluffyCategoryB)){d=Math.round(d*0.7);}pHP=Math.max(0,pHP-d);playSoundEffect('punch.mp3');if(isCrit){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 ダメージ: ${d}`;triggerEffect("player-crit",d);}else{log.innerHTML+=`\n😢 被弾ダメージ: ${d}`;triggerEffect("player",d);}if(myFluffyCategoryB&&curryHasCategory(oppC,myFluffyCategoryB)){log.innerHTML+=`\n🥚 ${getBarrierLabel(myC)}により${FLUFFY_CATEGORY_LABEL_B[myFluffyCategoryB]}からの攻撃を軽減`;}if(oppC.isPoisonApple&&Math.random()<0.5){if(!isPP){isPP=true;poisonLevelP=1;log.innerHTML+=`\n☠️ ${playerName} は毒にかかった！`;if(liveBattleMyStatusKeys.indexOf('poison')<0)liveBattleMyStatusKeys=liveBattleMyStatusKeys.concat(['poison']);setTimeout(()=>{playSoundEffect('poison.mp3');},300);}else if(poisonLevelP<6){poisonLevelP++;log.innerHTML+=`\n☠️ 毒のダメージが増幅`;setTimeout(()=>playSoundEffect('poison.mp3'),300);}}updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=true;round++;setTimeout(step,battleDelay(1100));}},battleDelay(400));
            } else {
                // 通常攻撃
                setTimeout(()=>{if(myC.isKaiTate&&rollShieldGuard(myC.spd)){playSoundEffect('sound/guard.mp3');log.innerHTML+=`\n🛡️ 攻撃を貝の盾でガードした。`;triggerEffect("player","Guard");updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=true;round++;setTimeout(step,battleDelay(1100));}else{const isCrit=Math.random()<getCritRate(oppC.spd,myC.spd);let d=isCrit?Math.max(8,oppC.atk):Math.max(8,oppC.atk-Math.floor(myC.def/2));d=Math.round(d*(0.9+Math.random()*0.2));if(oppC.isWanpaku)d=Math.round(d*getWanpakuDamageMultiplier());if(myC.isRatatouille&&(isMeatBasedCurry(oppC)||isMeatBasedBot(oppN)))d=Math.round(d*0.2);if(myFluffyCategoryB&&curryHasCategory(oppC,myFluffyCategoryB)){d=Math.round(d*0.7);}pHP=Math.max(0,pHP-d);playSoundEffect('punch.mp3');if(isCrit){log.innerHTML+=`\n🌶️ Spicy Hit!!!!!!!! 💥 ダメージ: ${d}`;triggerEffect("player-crit",d);}else{log.innerHTML+=`\n😢 被弾ダメージ: ${d}`;triggerEffect("player",d);}if(myFluffyCategoryB&&curryHasCategory(oppC,myFluffyCategoryB)){log.innerHTML+=`\n🥚 ${getBarrierLabel(myC)}により${FLUFFY_CATEGORY_LABEL_B[myFluffyCategoryB]}からの攻撃を軽減`;}if(oppC.isPoisonApple&&Math.random()<0.5){if(!isPP){isPP=true;poisonLevelP=1;log.innerHTML+=`\n☠️ ${playerName} は毒にかかった！`;if(liveBattleMyStatusKeys.indexOf('poison')<0)liveBattleMyStatusKeys=liveBattleMyStatusKeys.concat(['poison']);setTimeout(()=>{playSoundEffect('poison.mp3');},300);}else if(poisonLevelP<6){poisonLevelP++;log.innerHTML+=`\n☠️ 毒のダメージが増幅`;setTimeout(()=>playSoundEffect('poison.mp3'),300);}}updateHP(pHP,oHP,oppC.hp,myC.hp);battleLogHistory.push(log.innerHTML.replace(/<br\s*\/?>/gi,"\n"));myTurn=true;round++;setTimeout(step,battleDelay(1100));}},battleDelay(400));
            }
...
function forceForfeit() {
    ...
    if(wasEventMode && database) {
        // 以前は#oHpTextの表示テキストをパースしていたが、CSS側で非表示（display:none）にされると
        // innerTextが空文字になり同期が失敗するため、updateHP()が毎回更新している変数を直接参照する
        const curOHP = Math.max(0, Math.round(lastRenderedOppHp));
        database.ref('events/tokumori002/currentHp').set(curOHP);
        if(playerId && eventTotalDamageThisBattle > 0) {
            database.ref('events/tokumori002/damages/'+playerId).transaction(function(cur){ return (cur||0) + eventTotalDamageThisBattle; }).then(function(result){
                if(!result.committed) logSyncEvent('eventDamageWriteNotCommitted', 'tokumori002(forceForfeit) dmg=' + eventTotalDamageThisBattle);
            }).catch(function(e){
                logSyncEvent('eventDamageWriteFailed', 'tokumori002(forceForfeit) dmg=' + eventTotalDamageThisBattle + ' err=' + (e && e.message ? e.message : String(e)));
            });
        }
        if(playerId) database.ref('events/tokumori002/participants/'+playerId).set({ name: playerName, lastPlayed: Date.now() });
    }
    // バトル画面を終了してアリーナから退場
    endBattleScene();
    // 負けメッセージをモーダルで表示
    const oldEXP = playerEXP; playerEXP += 1;
    // 離脱＝強制敗北扱いのため食品サンプルによる身代わり判定を行う
    if(!tryConsumeCurryOnLoss(selectedCurryIndex)) { selectedCurryIndex = curryStock.length ? 0 : -1; }
    saveGame(); updateFridgeUI(); updateCookSelects(); updateMatchCurrySelects();
    setTimeout(() => checkLvUp(oldEXP, playerEXP), 300);
    if(!isBotMatch && currentRoomId && database) {
...
// forceForfeit()（棄権時のイベントボスHP同期）が、表示テキストのパースに頼らずここから直接読めるように保持しておく。
// #oHpTextはCSS側で非表示（display:none）にされる場合があり、その場合innerTextは空文字になるため、
// DOM表示ではなく毎回このHPの生値そのものを更新しておく方が安全。
let lastRenderedOppHp = 0;
// 通常PC戦・イベント戦（両方ともupdateHPとpOwnerText/oppOwnerTextを共有）の名前横に表示する状態アイコン。
// 各バトル関数側で状態が変わるたびにこの2つの配列を更新しておけば、updateHP()が呼ばれるたびに自動で反映される。
let liveBattleMyStatusKeys = [];
let liveBattleOppStatusKeys = [];
...
// ダメージポップの表示位置を、実際の敵/自分アイコンの位置から動的に計算する
// （フェス・タッグ戦のfestTriggerDamagePop/tagTriggerDamagePopと同じ考え方に統一）。
// 以前は#battleStage内の固定px/%指定だったため、画面幅や表示内容（bookボス戦のUI差し替え等）
// によって実際のアイコン位置とズレることがあった。アンカー要素が見つからない・非表示の場合は
// 従来の固定値にフォールバックする。
function positionDamagePopOnAnchor(pop, anchorEl, stageEl, verticalFraction) {
    if(!anchorEl || !stageEl) return false;
    const stageRect = stageEl.getBoundingClientRect();
    const anchorRect = anchorEl.getBoundingClientRect();
    if(anchorRect.width === 0 || anchorRect.height === 0) return false;
    pop.style.left = (anchorRect.left - stageRect.left + anchorRect.width/2 - 20) + 'px';
    pop.style.top = (anchorRect.top - stageRect.top + anchorRect.height * (verticalFraction === undefined ? 0.15 : verticalFraction)) + 'px';
    return true;
}
// 特盛りイベント戦は#oVisual（丸アイコン）を使わず#eventBgImg（横長の敵画像）を表示しているため、
// イベント戦中はそちらをアンカーにし、画像の縦方向中央（0.5）にダメージポップを出す。
// 通常戦は従来通り#oVisualの上寄り（0.15）に表示する。
function getEnemyDamagePopAnchor(stageEl) {
    const isEventModeBattle = !!(stageEl && stageEl.classList.contains('event-mode'));
    if(isEventModeBattle) {
        return { el: document.getElementById('eventBgImg'), verticalFraction: 0.5 };
    }
    return { el: document.getElementById('oVisual'), verticalFraction: 0.15 };
}
function triggerEffect(target, damage) {
    ...
    if(target==="enemy"||target==="enemy-gold"||target==="enemy-poison"||target==="enemy-crit"){
        ...
        const enemyAnchor1 = getEnemyDamagePopAnchor(stageEl);
        if(!positionDamagePopOnAnchor(pop, enemyAnchor1.el, stageEl, enemyAnchor1.verticalFraction)){ pop.style.top="130px"; pop.style.left="25%"; }
        ...
        pop.innerHTML = "+" + damage;
        if(!positionDamagePopOnAnchor(pop, document.querySelector(".player-zone"), stageEl)){ pop.style.top="240px"; pop.style.left="45%"; }
        pop.style.color="#2ecc71";
        pop.style.animation = "popUp 0.6s forwards";
    }else if(target==="enemy-heal"){
        pop.innerHTML = "+" + damage;
        const enemyAnchor2 = getEnemyDamagePopAnchor(stageEl);
        if(!positionDamagePopOnAnchor(pop, enemyAnchor2.el, stageEl, enemyAnchor2.verticalFraction)){ pop.style.top="130px"; pop.style.left="25%"; }
        pop.style.color="#2ecc71";
        pop.style.animation = "popUp 0.6s forwards";
    }else{
...
function done(pHP, oHP, myC, oppC, wasBlownAway) {
    ...
    if(isWin){
        ...
        let g=30,e=5,loot="";
        if(isBotMatch&&oppC.isDailyCurry){
            // 日替わりカレー：勝利で当日分の権利を消費（翌日まで再挑戦不可）。報酬は曜日/段階ごとの専用テーブルから付与。
            dailyCurryLastWinDate = getJSTDateString();
            const dReward = grantDailyCurryReward(oppC.dailyWeekday, oppC.dailyTier);
            g = dReward.goldGain; e = dReward.expGain; loot = dReward.lootHtml;
            incrementGlobalStat('battle/bot');
        }
        else if(isBotMatch&&oppC.isMaharaja){
        ...
        else if(isBotMatch&&oppC.isBookBoss){
            // 咖喱図書館：本のボス戦専用の固定報酬（Exp+50, G+50, ノーマル具材3個+ノーマルスパイス1個）
            g=50; e=50;
            const normalIngPool=Object.keys(masterIngredients).filter(k=>masterIngredients[k].shop===0 && isIngredientAvailable(k));
            const normalSpicePool=Object.keys(masterSpices).filter(k=>masterSpices[k].shop===0 && isIngredientAvailable(k));
            let lootHtmlParts=[];
            for(let i=0;i<3;i++){
                if(normalIngPool.length===0) break;
                const itm=normalIngPool[Math.floor(Math.random()*normalIngPool.length)];
                inventory[itm]=(inventory[itm]||0)+1; discoveredItems[itm]=true;
                const d=masterIngredients[itm]; const ico=d&&d.icon?`<img src="${d.icon}" style="width:1.2em;height:1.2em;vertical-align:middle;object-fit:contain;">`:'';
                lootHtmlParts.push(`${ico}${itm}`);
            }
            if(normalSpicePool.length>0){
                const sItm=normalSpicePool[Math.floor(Math.random()*normalSpicePool.length)];
                inventory[sItm]=(inventory[sItm]||0)+1; discoveredItems[sItm]=true;
                const sd=masterSpices[sItm]; const sIco=sd&&sd.icon?`<img src="${sd.icon}" style="width:1.2em;height:1.2em;vertical-align:middle;object-fit:contain;">`:'';
                lootHtmlParts.push(`${sIco}${sItm}`);
            }
            loot = lootHtmlParts.join(' ');
            // 再戦（周回）の場合は、他の報酬と同じようにテキストへ「想いの欠片+10個」を追加する。
            // 想いの欠片自体はgame.js側では管理していないため、実際の加算はonBookBossWinフック
            // （story.js側）に委ねる。ここではあくまで表示テキストの組み立てのみ行う。
            if(oppC.isBookBossRebattle) {
                loot += (loot ? ' ' : '') + '💭 想いの欠片+10個';
            }
            if(typeof window.onBookBossWin === 'function') {
                try { window.onBookBossWin(oppC.bookChapterNum, !!oppC.isBookBossRebattle); } catch(e) {}
            }
        }
